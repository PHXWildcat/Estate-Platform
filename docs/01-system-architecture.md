...
5. **Vault Service (Zone A).** Stores only opaque ciphertext blobs and encrypted item metadata. Implements SRP-style authentication so the vault password never transits to the server. Emergency access via time-delayed key escrow: the recovery key is split (Shamir) between user-designated contacts and a platform share that releases only after a configurable waiting period + notification storm to the owner. **A released grantee may then READ the owner's items (M27), and this is the one place in Zone A where the caller and the data's owner differ.** The credential does not change to allow it: the grantee presents their own vault session — which the guard still binds to them alone — and the authority to read somebody else's ciphertext comes from an `emergency_access_policies` row in `status = 'released'`, re-checked against the settlement gate on every read. The server hands over ciphertext it still cannot open; the key was reconstructed on the grantee's device from shares it never held. Zone A's guarantee is unchanged — a server compromise still yields ciphertext only — and what changed is who may fetch that ciphertext, which is an authorization fact rather than a key-custody one. docs/03 §6uu.
...
10. **Notification Service.** Multi-channel (SES, SNS/Twilio, push, in-app), per-event-type user preferences, encrypted message center for sensitive content (email/SMS carry only "you have a message" pointers, never estate content).
    *M9 delta (2026-08-04, approved deviations recorded in CLAUDE.md):* shipped as `apps/services/notifications` with a **synchronous HTTP port**, not the consumer-tier placement in the diagram above — the M6/M7 fail-closed capability gates (503 `notifications_unavailable`) are request/response by nature, and per-send outcomes feed the callers' delivery bookkeeping; Kafka fan-in can arrive later for non-gating kinds. Recipient addresses are **event-carried**, never read across clusters: identity feeds the service's own encrypted store (`notifications/kek`) at registration and login, so no email-ciphertext read path exists anywhere. Email (SES v1) is the only live channel so far; SMS/push/in-app and the message center remain open scope.
...
**AuthN ladder:** Passkeys are the default and the marketing push; TOTP and hardware keys supported; SMS OTP allowed only as a recovery factor with risk-score gating (SIM-swap is a primary ATO vector for this user base). Adaptive engine scores every auth and sensitive action on device fingerprint, IP intelligence, impossible travel, velocity, and session behavior; scores map to step-up requirements. **Step-up MFA (fresh, ≤5 min) is mandatory for:** vault open, document generation, data export, trustee/executor changes, beneficiary changes, deletion requests, emergency-access configuration, adding an authentication factor to an account that already has one (M16), and changing the account password (M17). The last two are CONDITIONAL on the account already holding a verified factor, and the condition is forced rather than chosen: step-up requires a factor to prove, so an unconditional rule would make both the first factor and — for an account with none — the password itself permanently unchangeable. Both are enforced in the service rather than by a route decorator, because the condition is account state a decorator cannot see; `test/factor-routes.spec.ts` exists so a service-level gate is not invisible to the fences that scan decorators. **A password RESET is deliberately outside this list** (M17 PR3): its caller has forgotten the password and holds no session, so there is nothing to step up FROM — the mailed code is the whole proof, and for an account with a verified factor that is a real weakening, recorded in docs/03 §6m rather than softened here.

*This list does NOT include login, and docs/00's authentication paragraph — which says it does — is the older of the two. Login requiring a fresh step-up would mean every sign-in needs a second factor, which is a product decision nobody has taken, contradicts the passkey-first ladder above, and is not what any shipped route does. docs/00 is left as the requirements-era statement of intent; this list is what the platform enforces.*
