...
| M44 | The step-up refusal discriminator, and the client that still shares one | **ADDED BY M27 PR6**, which fixed this on the extension and asked what else was in the category. Four clients call `POST /v1/auth/stepup`; `apps/bff` and `apps/operator-web` key 401 on the TOKEN and tell a rejected code apart from a dead session, and `apps/vault-web` does not — it collapses every 401 into `UNAUTHENTICATED`, so its step-up prompt answers an expired session with "try the current one". Splitting 401 there is not a one-line change: `UNAUTHENTICATED` is consumed at four sites on that origin and `app.ts`'s key-change path SYNTHESISES it locally to mean the local verification refused, so a careless split reproduces the inherited-discriminator defect M27 PR6 exists to close. Also carries two extension-side items: `vault-screens.ts` takes a raw bearer at all eight of its call sites and never routes through `withSession`, so an access token that ages past fifteen minutes makes every screen say "This device is no longer connected" about a live pairing; and the unreachable `INVALID_CODE` disjunct in that same file's revocation predicate. Three residuals in docs/03 §6aaa; needs a drive against the running vault origin |
