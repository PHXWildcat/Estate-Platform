...
| # | Milestone | Status |
|---|---|---|
| M21 | TB7 operator platform, minimum slice | **IN PROGRESS.** Approved 2026-08-17, section above. PR0–PR4e shipped; PR5 — documents evidence content plus the legal-hold lift ceremony — is NOT YET SHIPPED, so the milestone is live and its eighteen residuals are debt on a live owner. **M40 PR0 corrected what this row was said to be:** the M40 row called its old `**APPROVED**` the same stale-prose defect the M23 row records about itself, and it is the OPPOSITE one — M23 described a finished milestone as in progress; this described a live milestone by saying nothing. The eighteen are still a defect, because PR5 does none of them, and that is what the later M40 PRs adjudicate |
