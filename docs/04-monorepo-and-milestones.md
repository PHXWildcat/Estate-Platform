...
1. `DECRYPT_FIELD_PREFIXES` in @estate/contracts: the first dotted token of
   ...
   `distributions` is registered although settlement had no read route when this
   was written, so a decrypt ever appearing under it attributes to settlement
   rather than to the unknown class. *(M23 PR4b then shipped that read route and
   M48 PR3 gave the prefix two reviewed bound rows; the registration reasoning
   above is what it was at M18, and it is why the attribution was already right
   when the route arrived.)* The fence (`packages/contracts/test/decrypt-field-prefixes.spec.ts`)
...
bound 0 (`unknown_prefix` / `unmodeled_principal`: a read path nobody reviewed
is itself the anomaly). *(A third zero, `encrypt_only`, stood here until M48
PR3 deleted the class — see docs/03 §6jjj. Left as a record of what PR2
shipped, not rewritten into the present tense.)* Emits
...
fixed the sweep, settled it at **18** `[OWNER: M21]` residuals across ten
subsections of `docs/03` — **17** across nine after M40 PR1 re-tagged one to M41,
and **0** since M40 PR2 adjudicated the rest (docs/03 §6fff) — with TB7 or "the
operator platform" named on further lines still, counted by the command docs/03
§4's opening paragraph states rather than restated here — §4, not §6, because
that is where the sentence carrying the command actually sits. It has grown by roughly one per milestone since M14. The
...
never downward while it was being MEASURED, which is the argument restated: the
number rose each time somebody actually looked, so every figure taken on trust
was an undercount. It has since moved down twice and neither time by measuring —
M40 PR1 re-tagged one bullet to M41, and M40 PR2 adjudicated the remaining
seventeen to zero. Both are items LEAVING the category rather than a recount of
it, so the upward-only argument still holds for every MEASUREMENT that made it.
...
own. The tag is the count, anchored on the BULLET —
`grep -c '^- \*\*\[OWNER: M21\]\*\*'`. The unanchored form counts PROSE
mentions of the tag as well, and M40 PR1 added one: after PR1 re-tagged the
documents oracle to M41 the anchored count was SEVENTEEN while the unanchored one
still answered eighteen, agreeing with the superseded figure for the wrong reason.
(Both now answer differently again — M40 PR2 took the anchored count to zero —
which is the point: the spelling of the grep decides what it is counting, and only
the anchored one has ever been counting residuals.)
This paragraph names a figure only because the figure is the point.
...
(JIT elevation, session recording and KMS grant suspension are **E1** — they need
real IAM; peer approval is **M46**, and M40 PR2 corrected this sentence, which had
swept all four into E1 while the very next sentence deferred to `docs/03` §4,
where peer approval was never E1), operator-assisted account recovery (**M46**),
or the
...
**The State cell reports the BLOCKER, not the work** — see M40 PR4 (docs/03
§6hhh) for why the milestone vocabulary is the wrong shape to copy here. A
milestone that reads `**COMPLETE.**` makes a residual naming it STALE, because
the work is done. An escalation that reads `**CLEARED.**` makes a residual
naming it SCHEDULABLE, because the work can finally start and nobody has
started it. Same mechanism, opposite meaning, so it gets its own two words.

| | Item | Blocker | State |
|---|---|---|---|
| E1 | The M5 cloud half — AWS org, billing, CI OIDC role | Money | **BLOCKED.** |
| E2 | Legal + tax reference review | Procurement — **two** datasets, two reviewers | **BLOCKED.** |
| E3 | Plaid sandbox credentials | Procurement; gates *production*, not building | **BLOCKED.** |
| E4 | SMS carrier + 10DLC; APNs/FCM | Procurement, multi-week lead | **BLOCKED.** |
| E5 | Penetration-test firm | Procurement + a deployed target (rides E1) | **BLOCKED.** |

**The queue.** Numbering is proposed. **Every row's Status cell opens with one
token from a closed vocabulary** — `**PLANNED.**`, `**IN PROGRESS.**` or
`**COMPLETE.**` — and narrative follows it in the same cell. That token is where
`threat-model-residuals.spec.ts` reads "has this milestone shipped?", and it
asserts one is present on every row, so a row cannot answer by saying something
adjacent or by saying nothing (M40 PR0, docs/03 §6ddd). The token says what
stage the work is at; it does not say the stage is TRUE, and nothing mechanical
here can — the two candidate cross-checks were measured and both were narrower
than their claim, which §6ddd records.

| # | Milestone | Status |
|---|---|---|
| M21 | TB7 operator platform, minimum slice | **IN PROGRESS.** Approved 2026-08-17, section above. PR0–PR4e shipped; PR5 — documents evidence content plus the legal-hold lift ceremony — is NOT YET SHIPPED, so the milestone is live. **M40 PR2 (2026-08-26) ADJUDICATED ALL OF ITS RESIDUALS — docs/03 §6fff — and M21 now owns ZERO** (eighteen when M40 PR0 measured; seventeen after PR1 re-tagged §6e to M41; nought after PR2). Not because the work was done: ten of the seventeen were never M21's, five are accepted on their own bounds, and the remainder went to M46 — an owner docs/03 §4 had been naming as "M21 follow-on" without it being a milestone at all. **M40 PR0 corrected what this row was said to be:** the M40 row called its old `**APPROVED**` the same stale-prose defect the M23 row records about itself, and it is the OPPOSITE one — M23 described a finished milestone as in progress; this described a live milestone by saying nothing. That defect — residuals naming a live owner whose one outstanding PR does none of them — is now discharged; PR5 remains M21's only outstanding work |
...
| M24 | Dashboard, computable subset | **COMPLETE.** Approved 2026-08-21, section below; PR0–PR4 shipped. **The lifecycle token is M40 PR0’s:** this cell read APPROVED for as long as the milestone had been finished, so every derivation over this column counted M24 as live. The flip-trigger was re-checked at kickoff and had STILL not fired — no demo date, no signed customer — so starting is the owner's recorded decision, not the trigger firing; the grounds are in the M24 section and docs/06 rather than silently replacing this row's earlier "a proposal, not a decision". Owned two docs/03 §6v residuals and CLOSED BOTH — PR1 into §6qq, PR2 into §6rr — so docs/03 carries ZERO `[OWNER: M24]` tags. **This cell claimed the two in the present tense until M40 PR0:** M27 PR5 had already struck M24 from the M40 row and from docs/03 §6uu for asserting a count nobody checked, and both corrections said plainly that M24 owns none — while the row that asserts the OWNERSHIP went unread, because a sweep that greps for counts does not see a claim phrased as a verb. Ships the shared client read cache with its first consumer |
| M25 | Crypto-shredding execution path | **COMPLETE.** Scoped 2026-08-21, section below. PR0-PR5 shipped: the boundary, the capture redaction, the decision record, the destroy leg, the owner's surface, the security review. `destroyDek` now has one production caller and identity's domain erases; the other participants do not, and the ledger says which. PR5 closed the out-of-envelope category for identity's cluster (`password_hash`, `email_changes.new_email_bidx`) — every other domain's leg owns enumerating its OWN, and docs/03 §6pp says why building that list from column names is how PR4's sweep got it wrong. Count the participant domains from `packages/contracts/test/erasure-domains.spec.ts`, never from a number in this table |
| M26 | Forensic audit completeness | **PLANNED.** `auth_events` writes 4 of 9 columns; append-only, so history is permanently incomplete. **NAME COLLISION, flagged by M24 PR0 rather than inherited:** M25 re-owned the erasure fan-out "to M26" in nine `[OWNER: M26]` residual tags across docs/03 §§6kk–6pp — seven unreached domains, the search-token purge, the pre-shred notification ordering — while this row scopes M26 as audit completeness, which owns four earlier `[OWNER: M26]` tags of its own. Two bodies of work share one name; whoever scopes M26 splits them FIRST, and neither absorbs the other by default. **M40 PR3 ADDED ONE TAG TO THE AUDIT-COMPLETENESS HALF, EXPLICITLY** (docs/03 §6dd, re-owned from M23): `setDistributionStatus` accepts three targets and emits on exactly one, so moving a distribution to `in_progress` or to `disputed` writes nothing to the trail, and `disputed` is reachable from `completed` — an unlogged undo of a paid-out distribution, callable by an executor. `AUDIT_ACTIONS` carries four distribution actions and none for either unaudited target. It is named here as well as there because the split this row demands has to put it somewhere, and a shipped write verb that emits nothing is audit completeness rather than erasure fan-out. **NOT FIXED BY M40**, which is an adjudication milestone: the fix widens the closed `AUDIT_ACTIONS` vocabulary, and a new action must reach the audit consumer before its producer. **CLOSED BY M49 PR1 (2026-09-02), which is why the measurement restated above is now HISTORY rather than a description of the tree** — and the honest form of that sentence names WHICH clauses moved, because a first draft of it said "every clause is false at HEAD" and that is an over-correction. Three clauses still hold: the verb still accepts three targets, `disputed` is still reachable from `completed`, and it is still callable by an executor — which had better be true, since the row now records `completed → disputed` as an edge and that edge has to exist to be recorded. What changed is the part the residual was about: all three targets emit, `AUDIT_ACTIONS` carries six distribution actions rather than four, and the edge is on the row. Kept because a row that quietly rewrites its own past is unreviewable, and because this is the class of sentence M40 PR0 opened this whole sweep over: a measurement restated beside the mechanism that derives it, in a second file, where nothing reads it. **THE NAME COLLISION IS UNAFFECTED AND STILL OWED**: the audit-completeness half loses one item and keeps the rest, and whoever scopes M26 still splits the two bodies of work first |
| M27 | Emergency-access reader + vault item restore | **COMPLETE.** Scoped 2026-08-22, section below; PR0–PR6 shipped, PR4 folded into PR3b. **The lifecycle token is M40 PR0’s:** this cell read SCOPED after the milestone finished, and no regex hunting a completion word could have found one, because there was none to find. M27 was scoped on two halves, BOTH NOW CLOSED: release reconstructed the master key and wiped it, so the ceremony spent the escrow and delivered nothing — **PR3a closed it**, release now admitting `status IN ('waiting','released')`; and `deleteItem` was refused to the extension audience BECAUSE overwrites were "recoverable", a security argument resting on a restore surface nobody had built — **PR1b closed it**, the versions reader and version restore being that surface. **M40 PR0 put those two clauses into the past:** they were written in the kickoff's present tense and left there, which merely read as stale beside `SCOPED` and reads as a contradiction beside `COMPLETE`. The milestone SECTION deliberately keeps the kickoff tense because it is a scoping record; this row is a status row and cannot. The seven `[OWNER: M27]` tags in docs/03 were FIVE distinct residuals, none of them either half, and PR0 re-owned all five rather than let the headline scope keep living only in this row's prose |
| M28 | Owner-initiated sharing (§5.5 / §6s) | **PLANNED.** `beneficiary.cedar` is loaded and structurally unmatchable |
| M29 | Passkey sign-in / passwordless discovery | **PLANNED.** Both authenticate routes carry `SessionGuard` |
| M30 | In-app notification feed + channel preferences | **PLANNED.** Additive only; must never satisfy `deliversToRealChannels`. **M27 PR5 note: this row records two of the four residuals that now name M30** — M27 PR3a and PR3b each added one, and a row that undercounts its own tags cannot be sized from them, which is the defect M40 exists to sweep and M39's row has already been caught committing twice. **Gained two docs/03 residuals from M27 PR0**, both because the feed is the carrier that can close them without reversing a control: the §5.2 one-tap DENY token (docs/03 §6c — an email-borne deny is a per-recipient URL, which `templates.spec.ts` refuses outright and §6c cites as a CLOSED leakage half, and an SMS one needs E4), and `emergency.reminder`, declared since M9 and emitted by nothing because no service has a scheduler |
| M31 | Plaid link surface | **PLANNED.** Needs E3 |
| M32 | Subscription manager, manual half | **PLANNED.** After M25 |
| M33 | Global search | **PLANNED.** After M25 — the index must purge with the DEK |
| M34 | Operability instrumentation | **PLANNED.** Zero of six named tools present; precedes load testing |
| M35 | Load / chaos / DR / penetration | **PLANNED.** Needs E1 + E5 |
| M36 | Plaid-assisted subscription detection | **PLANNED.** Three stacked prerequisites, **middle one absent** — no transaction data path exists |
| M37 | Passkey provisioning (Estate as authenticator) | **PLANNED.** Reverses M16's central control |
| M38 | Referral marketplace | **PLANNED.** Zero code, zero DDL, regulatory surface |
| M39 | Zone A hardening — the SRP abuse bound and rollback detection | **PLANNED.** **ADDED BY M27 PR0**, and the addition is forced rather than tidy: `threat-model-residuals.spec.ts` derives its owner vocabulary from THIS TABLE in both directions, so re-owning a residual to anything not listed here is refused — which is the point of a closed vocabulary. Two §6a residuals, both vault-local and both unblocked: a caller holding a GENUINE step-up can still burn SRP handshakes (three tags, one residual, tracked at docs/03 §6a, §6j and §6k), and full-history rollback is detectable only by client-side last-seen state. Neither is either half of M27's name, and carrying them there was how M27 looked bigger than it is. **A THIRD residual joined M39 in the same commit and this row said "two" until the M27 PR0 review counted them:** §6uu's grantee-read detection gap — `vault.emergency.items_read` is watched by no detector, because `decrypt-rate-detector.ts` pins its counted set to `crypto.field.decrypted` alone. It is neither a §6a bullet nor vault-local (the detector is audit's), so the milestone is three residuals across two services, not two in one. **A FOURTH joined from M27 PR3b**, found by driving the real app: `apps/vault-web` carries no step-up propagation budget, so a user who completes step-up faster than `HttpSessionVerifier`'s positive cache expires is told they did not complete it (docs/03 §6zz). It lands here because the fix is the third copy of a shape `apps/web` and `apps/operator-web` already have, and deciding between a third copy and one shared module is a client-architecture call, not a line inside a Zone A read |
| M40 | Residual ownership re-sweep | **COMPLETE.** Added by M27 PR0, which found the complementary hole to the one it was scoped to fix. PR0 closed the case of an UNTAGGED residual hiding outside the fence's corpus; this is the case of a TAGGED one whose owner has since shipped. **PR0 (2026-08-25) closed the DERIVATION half — docs/03 §6ddd.** Every row of this table now opens its Status cell with one token from a closed vocabulary (`**PLANNED.**` / `**IN PROGRESS.**` / `**COMPLETE.**`), so a row can no longer answer the completion question by saying something else or by saying nothing. Applying that rule moved the derived completed set from three to SIX: M24, M27 and M44 had all finished under a cell that said `APPROVED`, `SCOPED`, or `… and the row is now complete.` in lower case, which the case-sensitive scan could not read. None of the three owns a residual, so the stale count is unchanged at thirteen — but the next one would have been invisible too. Thirteen residuals in the fence's corpus name M22 or M23, whose docs/04 rows read COMPLETE, and `OWNERS` is derived from every row of this table including the closed ones — so a tag naming a finished milestone passes the vocabulary check exactly as a live one does. The count is now pinned by `threat-model-residuals.spec.ts` so it cannot grow in silence; deciding, one at a time, whether a later slice of the same programme still owes each item is the sweep this row owns. **PR0 REFUTED THIS ROW’S OWN ACCOUNT OF M21,** which is the second time this row has been wrong about a milestone it names. It said M21’s eighteen were invisible "because that row says APPROVED and never gained a completion status — which is the same stale-prose defect the M23 row already records about itself". M21 has NOT shipped: docs/04’s M21 PR split records PR5 — documents evidence content plus the legal-hold lift ceremony — as NOT YET SHIPPED, and no commit names it. So M21’s silence was HONEST and the defect is the OPPOSITE of M23’s: M23’s row described a FINISHED milestone as in progress, and M21’s described a LIVE one by saying nothing. M21 now declares **IN PROGRESS.**, its residuals are live debt, and they stay outside the stale count. They remain a defect for a different reason — PR5 does none of them, so they name an owner with no plan to do them — and adjudicating that is a later M40 PR. **PR1 (2026-08-25) adjudicated the FIRST item of the sweep — docs/03 §6eee.** The read-before-authz category sat under two owners; §6e's documents half is re-owned from M21 to M41, both halves cross-cite, and the membership is settled at eight routes in two services with the rest of the repo swept and clean. One item down. **PR2 (2026-08-26) ADJUDICATED M21'S SEVENTEEN, ONE AT A TIME — docs/03 §6fff — taking M21 to ZERO.** Ten were never M21's; five are accepted on their bounds; NOTHING is closed — a draft marked two bullets CLOSED on a decorator's presence and the review showed the decorator WIDENS rather than binds, so both stay open under M46; two bullets carried more than one item and were SPLIT, one of them naming three properties, giving two of them owners its own tag did not name and the third no owner at all, while wearing a single tag. The headline is an owner that was not a milestone: docs/03 §4 deferred peer approval and operator-assisted account recovery to an owner it called "**M21 follow-on**" (only peer approval carried the fuller "not the minimum slice") — no queue row, no `OWNERS` entry, nothing to schedule — surviving because §4 is prose outside the §6 corpus the fence reads, while docs/04's own scope statement contradicted it by sweeping the same items into E1. **M46** is that owner made real, and **M45** takes the two fence-corpus items. Remaining: the thirteen stale-owned (M23 twelve, M22 one), which is PR3. The number is still spelled out here and in several other sentences, and reducing those copies is not done. **PR0 RECORDED THESE AND DID NOT FOLD THEM IN**, on the rule that a sweep which fixes what it trips over stops being reviewable. Each was re-derived before being written here. (a) The M32 and M33 rows both read "After M25", and M25 is COMPLETE — a cleared prerequisite still reading as pending, which is this column's stale-prose class again, now on the blocker rather than the status. (b) docs/03's `Pre-existing across all six `StepUpPrompt` callers` residual cites "M23 PR4c" inside an [ACCEPTED] bullet; M23 shipped PR1, PR2, PR3, PR4a and PR4b, in docs/04's headings and in git alike, and no *M23* PR4c exists anywhere in the repo (M21 PR4c and M22 PR4c are both real, which is exactly why the milestone qualifier has to be in the sentence) — a false sentence about another file, of the class no fence here can reach because it is neither a tag nor in §6. (c) M21 PR2.5 shipped as #112 and appears in NO bullet of M21's PR split, so the split does not enumerate its own milestone in either direction. (d) M44 is the only COMPLETE milestone with no `###` record section at all — its per-PR record lives in this table's own row — and whether "the record lives in the row" is a convention worth stating is undecided rather than assumed. (e) The M39 and M43 rows both say the fence derives its owner vocabulary "from THIS TABLE in both directions"; it reads `^\| (M\d{2}|E\d) \|` across BOTH of docs/04's tables, the escalations one included, which is why `OWNERS` can bless `E1`. True in both directions and about every milestone owner, but it under-describes its own source — and PR0 now asserts that two-table boundary as a mechanism, so the two sentences describing it should agree with it. (f) `threat-model-residuals.spec.ts`'s own DOCSTRINGS carry seven stale derived counts, re-measured here so a later PR need not: they say docs/03 "carries 107 residual bullets" (238 were tagged when M40 PR0 measured it — 113 ACCEPTED, 97 OWNER, 28 CLOSED; **247 after M40 PR2** — 120 ACCEPTED, 99 OWNER, 28 CLOSED), that "NINETEEN are one owner — E1" (E1 owned 20 then, and 21 after M40 PR2), that `MIN_RESIDUALS` "sits at 95 against 212 actual bullets", that the corpus is "107 bullets, 89 reached by region and 18 by language", that the out-of-corpus census is "132 prose sentences", and that a new region idiom would be "a sixteenth" (`REGION_MARKERS` holds 13, so a fourteenth). Two neighbouring counts are correct because they say WHEN they were measured — "the counts measured at M21 PR0" — which is the distinction worth copying. None of the seven affects a gate; all seven are the second-copy-that-rots defect sitting in the docstrings of the fence built to catch it, which is why they are recorded here rather than swept in a PR about the Status column. **M27 PR5 struck M24 from that sentence:** it read "M21's eighteen and M24's", and M24 owns ZERO residual tags, so the row scoping a residual-ownership sweep was itself miscounting one of the two milestones it named. **PR3 (2026-08-26) ADJUDICATED THE THIRTEEN STALE-OWNED — docs/03 §6ggg — and M22 and M23 now own none.** Thirteen bullets became SEVENTEEN items, two of them carrying three things each: eight ACCEPTED, four to M45, two to M46, two to M47 (a row PR3 opens), one to M26, nothing closed. Five of the thirteen were still TRUE — every assertion in them re-verified — and eight carried at least one false clause. ALL THIRTEEN were rewritten, so "held as written", the phrasing an earlier draft of this row used, measures the editing and not the decay. THE FINDING IS A SHAPE RATHER THAN A COUNT: two of the thirteen deferred on a stated PRECONDITION that a later PR met without anyone re-deciding — docs/03 §6dd's distribution bullet was left as found "because the route has no consumer anywhere", and M23 PR4b then built exactly that consumer without adding the audit emit, turning a latent defect into a live one. The deferral is M21 PR4's (docs/03 §6dd is that PR's delta) and the PR that spent it is M23's — a DIFFERENT milestone, which is what makes the shape hard: the PR that discharges a precondition has no reason to have read the section stating it. A deferral that names its own trigger is only as good as whatever watches the trigger, and nothing did. PR3 also answered an assertion that could not survive its own success: with the stale set empty, the pinned owner-set and count comparisons become `[] vs []` and `0 === 0`, which pass identically if the parser breaks. Both KEEP their place — they are still the thing anyone cares about — and are now paired with a POSITIVE CONTROL that feeds a synthetic bullet through the same predicate and asserts it FIRES, with a negative twin on a live milestone. Its coverage is stated from a COUNTERFACTUAL rather than from intention, because PR3's first account of it was wrong: breaking the completed-set derivation is caught anyway by `MIN_COMPLETED_MILESTONES` one assertion above, and what the control uniquely catches is the predicate failing while its inputs stay healthy — same mutation, control present fails, control removed passes. **PR3 RECORDED TWO MORE AND DID NOT FOLD THEM IN.** (g) `apps/services/vault/test/session-audience.spec.ts` reasons about "a 24th route added tomorrow" in the comment justifying its derived half, while the assertion three lines below pins the refused set at TWENTY-ONE and `EXTENSION_ROUTES` holds seven — twenty-eight, so the next one is the twenty-ninth. The number in the ASSERTION is current and the number in the PROSE explaining it is five behind, which is the second-copy-that-rots defect in the one place it is least visible: a comment whose whole job is to explain why the assertion is trustworthy. No gate reads it. (h) The same file states a rule and breaks it. Its last test carries a comment headed "NO COUNT IN THE NAME", arguing that a test name is not checksummed and so must name the PROPERTY and let the list be the count — and TWO names above it carry a count anyway: `exactly seven handlers admit an extension session` and `every route that is not one of the seven refuses the extension`, four and one `it` blocks earlier. The second is harmless, because its body pins `toHaveLength(21)` and an eighth admitted route makes that fail. The FIRST is not: its assertion is set equality against `EXTENSION_ROUTES`, so growing that list keeps the test green and leaves only the name wrong — the precise failure the comment describes, in the file that describes it. Both are in a package PR3 does not touch, and folding them in would put an unrelated package in an adjudication diff. **PR4 (2026-08-27) IS THE SECURITY REVIEW — docs/03 §6hhh — AND M40 FINISHES OWNING ZERO RESIDUALS.** A milestone that shipped no runtime code has no new attack surface, so the review's subject is the DECISIONS: the FOURTEEN `[ACCEPTED]` bullets M40 wrote or amended in other milestones' sections — thirteen newly marked (PR2's five, PR3's eight) plus §6cc's operator-console bullet, `[ACCEPTED]` since `9101c12`, whose body PR3 edited. Its own six delta-section residuals are excluded on the rule that a section does not review itself. **NINE STAND. FOUR ARE IMPRECISE — right disposition, wrong clause. ONE REOPENS.** The one that reopens is §6g's *Unlinking does not revoke what the link already enabled*, and its argument never reached the surface it was defending. It named TWO survivors of an unlink and one gate; there are SEVEN behind two gates, and `assertCaseVisible` (`apps/services/settlement/src/admin.service.ts:1110`) takes no Cedar decision at all — it admits on the bare `kase.reported_by === actor` snapshot, so a linked contact who filed a death report and was then UNLINKED keeps five case reads. All five sit POST-verification, which is exactly the window that bullet's own last sentence says the owner's void does not reach, so the acceptance's third leg never covered them. Four — `timeline`, `listStages`, `listTasks`, `listDistributions`, the whole of `recordOperatorRead`'s surface union — emit NOTHING, because that helper opens `if (!isOperator) return;`. The fifth, `distributionAmount`, decrypts the estate's distribution amounts under the DECEDENT's DEK and returns plaintext money, and is the only one of the five that is audited. **THE BULLET IS SPLIT RATHER THAN RE-TAGGED WHOLE:** the money route is a defect and goes to **M48**, a row this PR opens, together with the four silent reads' missing trail; the case-exists reads stay `[ACCEPTED]` as evidence a reporter is meant to have. The fix shape is already in the file — the same `if` re-derives the EXECUTOR's linkage live through `isExecutorOf` two lines below the reporter's frozen column, so the platform enforces a link's lifetime for one party and not the other on the same estate. **PR4 TURNED PR3'S HAND-FOUND SHAPE INTO A MECHANISM.** PR3 caught, by hand, one deferral whose stated precondition a later PR had spent. PR4 swept for others and found NONE — then made the absence DATA rather than trusting the sweep: `PRECONDITIONS` holds the five deferrals justified by an ABSENCE, each declaring the absence, the corpus that would falsify it, what "filled" looks like, and where it is deliberately not counted, and the probe reads the TREE rather than other prose. A clean sweep is distinguishable from a broken one because the sweep's POSITIVE CONTROL is PR3's own case, which returns SPENT through the same probe. **AND THE ESCALATION HALF, WHICH NOTHING WATCHED EITHER.** Residuals owned by `E1` or `E4` wait on a blocker recorded only in prose, so the escalations table gains a **State** column — five rows, all `**BLOCKED.**` — with two assertions: every escalation row declares a state, and no residual names an escalation whose blocker has CLEARED. The vocabulary is deliberately NOT the milestone one, because the two mean opposite things about a residual: `**COMPLETE.**` makes one STALE (the work is done and the tag is a lie), while `**CLEARED.**` makes it SCHEDULABLE (the work can start and nobody has). **PR4 RECORDED FOUR MORE AND DID NOT FOLD THEM IN**, on the rule PR0 and PR3 both used, and the sharpest is about this milestone. (i) §6b's cross-reference repair — written by M40 PR2 — names the sibling twice over, as *Operator actions remain unbounded* at §6bb, "owned by **M23** today". M40 PR3, the next PR of the same milestone and the SAME DAY, retitled that bullet to *Operator READS are bounded, counted and reviewed by nothing* and re-owned it to **M46**. Both halves of the repair were falsified within a day of being written, by us, and the sentence still stands. (j) §6dd's `config.secureCookies` bullet asserts of a two-edge PAIR a figure measured on ONE edge: "three NAMED assertions red … Both edges are the same" is three reds on `operator-web` and exactly one on `vault-web`, which has no `cookies.spec.ts` at all — the half-applied rule again, inside the bullet **M47** exists because of. (k) §6o's parenthetical points at "§6m question 8"; §6m has no numbered list and does not contain the word "question" — a dead pointer introduced at M17 PR5 (`1ff424a`) and carried through M40 PR2's rewrite of the very line it sits on. (l) §6d's "in five independent spellings" is an undercount, and the interesting part is that two independent reviewers derived SIX and EIGHT using different definitions of a spelling — a number whose value depends on what you count, sitting in prose beside a mechanism that could derive it. **THE LIFECYCLE TOKEN IS THIS PR'S OWN:** leaving this row `**IN PROGRESS.**` after its last PR would be exactly the M23 defect — a finished milestone described as live — committed by the milestone that exists to sweep it |
| M41 | Plaid item refusal, and the read-before-authz sweep | **PLANNED.** **ADDED BY M27 PR1a**, which fixed this defect in the vault and asked what else was in the category. `plaid`'s `sync` and `revoke` read an item by id, answer 404 when it is missing, then call `assertCan`, which answers 403 when it is not yours — the same existence oracle M27 PR1a closed on `vault_items` and `emergency_access_policies`. It is a row rather than a line in that PR because reaching from a vault change into the Plaid service is scope creep to propose, not to perform. `ai-assistant` was checked in the same pass and is already correct (its authz throws `NotFoundException`), so the sweep this row owns is: every service that reads a row by id before its authz gate, checked against that precedent. **M27 PR5 counted the category and the row named one of its two members:** `documents` has SIX id-bearing routes in the identical shape — 404 for a missing id, 403 from Cedar for one that is not yours — against `plaid`'s two, and every other service under `apps/services/` is clean by a named mechanism. **M40 PR1 (2026-08-25) CLOSED THE SPLIT — docs/03 §6eee.** The documents half was tagged `[OWNER: M21]` in docs/03 §6e — M12's delta section, carrying the M4 review's open follow-up — the plaid half was here, and nothing connected them, so whoever scoped either row saw half the work. Both halves are now `[OWNER: M41]` and cross-cite each other, so one grep returns the category. PR1 also swept the whole repo for the shape and settled the membership at **EIGHT routes in two services and nowhere else** — documents six, plaid two — derived from the handlers and confirmed member-by-member by a refute-by-default pass, with `apps/bff`, both proxy origins, `apps/web` and `packages/` all checked and clean. **PR1 corrected two of this row's own reasons,** each right about the conclusion and wrong about why. (i) `settlement` was credited to `assertCanOrNotFound`; that helper is called at exactly THREE sites in a service with thirty-one routes, nineteen of them carrying a path id. What keeps the REST out is different: `SettlementAdminService` takes no Cedar PEP at all — there is no `SettlementAuthz` in its constructor — and reaches a uniform 404 through `assertCaseVisible`, `administrableCaseFor` and `requireAdministeredDecedentFor`, while others are excluded by ORDER, because `OperatorGate.assertIn` throws before any lookup happens. An audit that checked helper usage alone would have called most of the service unprotected and been wrong about all of it. (ii) Neither member is browser-reachable today, but that is a property of TWO CLIENTS rather than of the edge: `apps/bff` collapses a bare 403 on two of its six downstream clients, three of the other four branch only on a token-qualified 403 and let a bare one through, while the fourth reads a bare 403 as the assistant being switched off — an outage-shaped answer rather than a leak; and both proxy origins forward the upstream status untouched, with the operator console rendering 403 and 404 as two codes with two sentences. The sweep this row owns therefore has a second question inside it — which services hold the shape, and which of them sit behind a surface that would show it |
| M42 | Cross-user free text, and everywhere else it crosses | **PLANNED.** **ADDED BY M27 PR5**, which asked what CATEGORY PR3b's escrow `label` belongs to. That label is the first field in the product one user authors and a DIFFERENT user renders, and it is validated harder than its eighty characters suggest: `EscrowLabelSchema` (`apps/services/vault/src/schemas.ts`) trims, bounds, and REFUSES `\p{Cc}` and `\p{Cf}` — the second because a right-to-left override in a name shown on somebody else's screen is a spoofing primitive rather than a typo. A repo-wide scan finds that refusal in exactly ONE place, which is that one: the rule exists on a single member of its category, the shape M27 kept finding. The sweep this row owns is enumerating the fields whose author and audience differ and adjudicating each. `documents.title` (`z.string().min(1).max(200)`, no such refusal) is the first candidate, and whether its readers are ever someone other than its author is the question that decides it. It is a row rather than a line in PR5 because the answer is per field across four services, and because "add the same `.refine` everywhere" is the wrong shape — vault migration 007's CHECK carries the control-character half, and only the rendering audience justifies the BiDi half |
| M43 | One derivation per closed vocabulary | **PLANNED.** **ADDED BY M27 PR5.** The repo runs several closed vocabularies and fences them unevenly, so what it costs to extend one depends on which one it is. `OWNERS` in `threat-model-residuals.spec.ts` is derived from THIS TABLE in both directions. `NOTIFICATION_KINDS` is checked against the `notification_sends_kind_check` CHECK by an int spec that inserts every member and compares the resulting SET. `AUDIT_ACTIONS` has no DDL mirror at all and is fenced per service, one-directionally: each emitter is asserted to be a member, so a member nobody emits is invisible. The gap this row is named for is the second CHECK: the vault cluster keeps its own hand-written `emergency_access_notifications_kind_check` over an overlapping vocabulary, and migration 009's own header records the residual out loud — "the DDL still has to be written on both sides". M13 shipped a kind to one list and not the other; the send succeeded and the record of it did not, so owners were mailed while their trail said they were not. The fence written after that closed one of the two pairs. This row owns the other, and the general form: every closed vocabulary derives its mirrors, or names in its own file which mirror is hand-kept and why |
| M44 | The step-up refusal discriminator, and the client that still shares one | **COMPLETE.** Added by M27 PR6, which fixed this on the extension and asked what else was in the category. **PR1 (2026-08-25) CLOSED the vault-origin half — docs/03 §6bbb.** The risk this row recorded was overstated: splitting 401 there looked like it would touch four `UNAUTHENTICATED` consumers and a local synthesis (there are in fact two — `app.ts`'s unlock `.catch` and `vault-session.ts`'s `changePassword` arm — which this row and §6aaa both undercounted, and an earlier draft of this rewrite silently repaired the number instead of saying so), but splitting out ONLY `invalid_code` touches none of them, because the vault service's sole 401 token is `srp_failed` and `invalid_code` is reachable on exactly one route that client calls. The fix was a DELETION — `messageFor` owns the sentence and the prompt's special case is gone — which is the opposite of M27 PR6's fix for the same defect, because the extension's `invalid_code` serves two ceremonies and this origin's serves one. PR1 also widened `copy.spec.ts` from the vault service alone to every upstream the runtime's `PROXY_ROUTES` names, and added the first e2e OBSERVATION of what `POST /v1/auth/stepup` answers. **PR2 (2026-08-25) CLOSED THE REST — docs/03 §6ccc, and the row is now complete.** `VaultScreensDeps` took `bearer: string`, a value read once at mount and spent for the life of the popup; it now takes a CAPABILITY (`call`), so the vault half holds no credential and cannot hold a stale one. The defect was the TYPE rather than the eight call sites — every consumer of that string was correct and the answer was still wrong. The same snapshot turned out to be present a second time, on the disconnect button, where a rotation between render and click would have spent a dead refresh token and left a browser looking signed out over a live session. The unreachable `INVALID_CODE` disjunct in `session.ts`'s `withSession` is deleted, and its claim is now DERIVED: the fence walks the refresh route out of identity and drives each refusal through the real client, so a disjunct naming something the route cannot produce reddens. One mutation SURVIVED — dropping `rotateSession` left all eleven popup cases green — and was answered with a test rather than a weakened mutation |
| M45 | Fence corpus breadth — every source-scanning fence states its reach | **PLANNED.** **ADDED BY M40 PR2**, which adjudicated M21's seventeen residuals and found two asking one question. docs/03 §6r: the route-gate fence covers the `assets` service only, so the other EIGHT services' step-up coverage is whatever their controllers happen to say — `apps/services` holds ten directories and nine of them have controllers (`audit` has none), which is the number §6r's own bullet states. docs/03 §6y: the corpus lesson from `rate-bounds.spec.ts` — a fence that scanned a single file while claiming to cover a service — was applied to that one fence and never swept across the others. Both are the shape this repo already names: **a fence whose input is narrower than its claim goes green for the same reason it is wrong.** Scope: enumerate every source-scanning fence, make each one DECLARE its corpus, and assert that declaration against the tree so a scan cannot silently narrow. Distinct from M43, which is about a VOCABULARY having one derivation; this is about a SCAN having a stated reach. The two rows meet where a fence scans a vocabulary. |
| M46 | TB7 follow-on — the operator authorization model | **PLANNED.** **ADDED BY M40 PR2**, and it exists because an owner was named without being a milestone and stood that way for nine days and six shipped milestones (the `**COMPLETE.**` rows in the table below: M22, M23, M24, M25, M27, M44) — introduced by M21 PR0 (`9885362`, #109, 2026-08-17) and re-owned here. docs/03 §4's deferred-TB7 list gives every item an owner, and two of them name **"M21 follow-on"** — peer approval of an operator action, and operator-assisted account recovery; only the first carries the fuller **"not the minimum slice"**. That is not a milestone: no queue row, no `OWNERS` entry, nothing to schedule. It survived because §4 is PROSE, outside the §6 bullet corpus the residuals fence reads — the blind spot §6uu already records. docs/04's own M21 scope statement contradicted it on ONE of them, sweeping four items into "(all E1 — they need real IAM)" and then deferring to §4 as the authority; three of the four agree, and §4 says peer approval is not E1. **This row is that owner, made real.** Scope: peer approval / M-of-N on the §5.1 chain; operator-assisted account recovery; allowlist case scoping (`OperatorGate` decides nothing about a CASE — operator status is global and unscoped); a real operator role vocabulary, since one role carries every operator verb and nothing can express "may review, may not approve distributions"; a PEP for `admin.service.ts`'s task/stage/distribution routes, which `settlement.cedar` has no resource types for; the allowlist READER plus the revocation notice that M21 PR3b shipped the console without; and allowlist EXPIRY. **Explicitly NOT E1:** none of these needs AWS IAM. M-of-N over the stage chain is a second-approver column and a CHECK in settlement's own cluster, the shape that service has already shipped four times over (docs/03 §6b enumerates them with their migrations), and expiry is one column on `settlement_operators` plus one conjunct in the `revoked_at IS NULL` predicate `operators.repo.ts` already runs. What is genuinely E1 stays E1 — JIT elevation, session recording, and KMS grant suspension. |
| M47 | The isolated origins, hardened as a pair | **PLANNED.** **ADDED BY M40 PR3**, which adjudicated the thirteen residuals owned by milestones that had already shipped and found a rule applied to one member of its category. `operator-web` and `vault-web` were built from one template and have drifted with nothing comparing them. docs/03 §6dd recorded THREE shapes as "operator-edge items": the malformed-`Host` 500, a `config.secureCookies` nothing reads, and `APP_ORIGIN` validated with `z.string().url()` rather than as an ORIGIN. All three exist on the vault edge too — three shapes across two origins is SIX instances, and the bullet named three. Two of the three are accepted on their bounds; `APP_ORIGIN` is not, because `.url()` admits a path, a query, credentials and a `javascript:` scheme, and the accepted value reaches a sink on both edges — an `href` set via `setAttribute` on the operator origin, `location.assign` on the vault one. Different sinks for the same value, which is the drift this row is about. docs/03 §6ee joins it: the `form-action` origin split is DETECTED and never PREVENTED, a deployment can serve an origin the built CSP does not permit, and `images.yml` checks build-arg propagation rather than parity. Scope: one `.refine` per edge; bake the permitted origin into the client and refuse before a code reaches the DOM; and DERIVE what the two origins share, so a seventh instance of a template shape cannot join the category unnoticed. **Explicitly NOT E1** — E1 is the AWS half, blocked on money, and none of this needs AWS anything. Distinct from M45: a zod schema is not a scan and has no corpus to declare |
| M48 | The link that outlives the unlink | **COMPLETE.** **ADDED BY M40 PR4**, the security review (docs/03 §6hhh), which re-adjudicated the thirteen `[ACCEPTED]` dispositions M40 wrote and found one whose argument never covered the surface it was defending. `assertCaseVisible` in `apps/services/settlement/src/admin.service.ts` admits on the bare snapshot `kase.reported_by === actor`, so a linked contact who filed a death report and was then UNLINKED keeps five case reads. Four of them — `timeline`, `listStages`, `listTasks`, `listDistributions` — emit nothing, because `recordOperatorRead` early-returns `if (!isOperator)`. The fifth is `distributionAmount`, which decrypts the estate's distribution amounts under the DECEDENT's DEK and returns plaintext money; it is the only one of the five that is audited. §6g's ACCEPTED bullet named TWO survivors and one gate (`caseResource`'s Cedar snapshot); there are seven behind two gates, and `assertCaseVisible` takes no Cedar decision at all. All five sit POST-verification, which is exactly the window that bullet's own last sentence says the owner's void does not reach — so the acceptance's third leg never covered them. THE FIX SHAPE IS ALREADY IN THE FILE: the same `if` re-derives the executor's linkage live via `isExecutorOf` two lines below the reporter's frozen column, so the platform enforces a link's lifetime for one party and not the other, on the same estate. **PR1 (`de91ae3`, #173) RE-DERIVES THE REPORTER'S LINK AT READ TIME**, and in doing so RE-ADJUDICATES THIS ROW'S OWN SENTENCE. M40 PR4 wrote that "the case-exists reads are evidence and stay"; they are not case-exists reads. `listDistributions` returns the beneficiary contact, the asset, the approving operator and whether an amount exists; `timeline` returns the full case history; `listStages` returns the access ladder. That is estate administration, and the vocabulary undersold what it left ACCEPTED. Seeing that your own report exists is `getCase` and `listMyCases`, which sit on DIFFERENT gates and which PR1 does not touch — so §6g's evidence argument survives intact: an unlinked reporter still sees the case and still sees that they authored it. **The alternative was measured, not waved off — and PR1a had to RE-ARM the measurement PR1 cited.** Refusing only the money route breaks M23 PR4b's set-equality argument, encoded in `admits exactly the callers listDistributions admits`: a reader who may see a distribution exists is the reader who may see its amount, so only shrinking both sets together keeps it. PR1 reported that the money-only alternative REDDENS that test. It did — against the fixtures as they stood when the sentence was written. PR1 then repaired `verifiedCase` to LINK its reporter (a reporter intake cannot produce), which left that comparison unable to see the link dimension at all: measured on the tree PR1 shipped, the money-only alternative leaves it GREEN. **PR1a states the arm where an unlink can be expressed** — `admit the same SET after an unlink` — and the money-only alternative reddens it again, as does removing the gate entirely. **PR1 ALSO FOUND A THIRD GATE AND AN EIGHTH SURVIVOR** that §6g and §6hhh both missed — `CasesRepo.listForUser`'s raw-SQL `OR reported_by = $1` — and KEEPS it frozen deliberately, stating why in the repo rather than leaving it uncounted: it is exactly what makes the evidence argument true. **THE HARNESS HAD TO BE REPAIRED FIRST.** `FakeCoreReads` had no `unlink` and its `isExecutorOf` ignored links entirely, while the real query JOINs a live contact — so an unlinked executor kept the estate in tests and lost it in production, and M48's subject was inexpressible. Two fixtures also designated a reporter without linking one, a state intake cannot produce (`report` 404s unless `isLinkedContact` holds), which is why the test named for the reporter's access was evidence about nothing. **DRIVEN IN A REAL BROWSER AGAINST THE STACK, WITH THE COUNTERFACTUAL RUN.** A linked contact reported a death through the product (`reportDeath`), read `estateDistributions` on the case and got `200 []`; the owner then unlinked them through the real one-click `unlinkContact`; the same browser session re-read and got `NOT_FOUND`. **The probe was then shown able to FAIL**: the settlement image was rebuilt with the fix reverted, proven pre-fix by reading `dist/admin.service.js` out of the running container — zero occurrences of `isLinkedContact`, which the fix adds exactly once to that file, the service-wide string being no discriminant because `settlement.service.js` and `core-reads.repo.js` carry it in both builds — and the identical request from the identical unlinked session returned `200 []` again — the defect reproduced live. Rebuilt with the fix, it returns `NOT_FOUND`. Two controls held throughout: `settlementCases` still returned the case (the third gate, kept deliberately, so the reporter never loses sight of the report they filed), and `reportableEstates` was empty in both runs, so the link state was constant and only the code differed. **RECORDED, NOT FOLDED IN, AND NOW DEMONSTRATED RATHER THAN REASONED:** the same unlinked reporter successfully called `attachCaseEvidence` and drove `evidenceCount` to 1 — a removed contact WRITING to a dead person's evidence trail. That is §6g's `[ACCEPTED]` Cedar half, and it is the positive control proving this fix did not over-apply; whether it should stay accepted is a question M48 raises and does not answer. **PR2 (`360e95e`, #175) IS THE TRAIL**, and it removes a premise rather than only an early return. `recordOperatorRead` opened `if (!isOperator) return;`, so four of the five reads PR1 had just put behind a live link check recorded only platform staff, and the fifth — `distributionAmount` — produced no case-trail row for anybody at all. `caseViewed`'s own docstring argued that narrowness was the decision, on the ground that the other readers are "people reading their own case". True of the DECEDENT; false of the EXECUTOR, who administers somebody else's estate and whose equivalent read the assets service has audited as `asset.estate.viewed` since M7 PR2 (`031972c`) — an event settlement's own `executorCases` cites by name as the reason it needs none of its own. So: the early return goes, `actorType` is DERIVED from the flag `assertCaseVisible` already had to compute, `asOperator` is a REQUIRED parameter so a new caller that omits it fails to compile rather than silently recording an operator as a user, `distributionAmount` joins the case trail it had sat beside since M23 PR4b (and emits ABOVE the null-amount return, so a successful read of an amount-less row is not the one gated read that records nothing), and `getCase`'s inline second copy of the condition is unwrapped. **DERIVING `actorType` ALSO SPENT M23 PR2's REASON FOR A SEPARATE OMISSION**, which had to be re-decided rather than inherited: `executorCases` emitted nothing solely because `worklistViewed` hardcoded `actorType: 'operator'` and a new `AUDIT_ACTIONS` member would cost a consumer deployment. Both halves die here, so it emits `worklist: 'executor'` — and that read is the one worth having, since it lists estates belonging to OTHER PEOPLE. **`listMyCases` is NOT audited, and that is a reversal inside this PR.** It was written, then measured and taken back out: it answers `OpenSettlementCaseBanner`, mounted in `AppShell` with its effect keyed on `pathname`, so the emit wrote one permanent row per page navigation for every authenticated user — §6cc's audited-volume-is-a-UI-constraint rule, the rule that stops the operator console polling, and it would have drowned the cross-case reconnaissance signal `settlement.queue.viewed` carries. The reason now lives in `settlement.service.ts` at the line where the next person would add it. **THE SURFACE UNION IS NOW FENCED AGAINST THE SOURCE**: it existed twice by hand, which is how `distributionAmount` joined the gate and neither union, and the fence compares what the reads RECORD against what the union DECLARES, as SETS, so neither file can satisfy it alone. Its corpus is one file per gate — `admin.service.ts` for `assertCaseVisible`'s callers and `settlement.service.ts` for `getCase`'s direct emit — because the first draft was narrower than its claim: `'case'` was injected into the expected set by hand, so deleting `getCase`'s emit left both assertions green. Scanning the second file is what makes that mutation red, and the scanner strips block comments, joins each method body before matching, and floors its own call-site counts so a gated read whose declaration it fails to recognise cannot vanish into the method above it. **THE HARNESS CAME FIRST AGAIN**: `FakeFieldCrypto` accepted an `actorType` and dropped it, so the decrypt half was UNPROVABLE — reverting the harness repair stops the proof compiling, which is the sharpest statement of why. docs/03 gains §6iii and §6g's M48 bullet is restated to its live subject before being closed: clause (2) said "this caller", inheriting clause (1)'s removed contact, when the parties the silence covered were the decedent, a still-linked reporter and the executor. **PR3 IS THE LOOSE ENDS, AND BOTH WERE THE SAME SHAPE** — a control that was right when written and falsified by a later milestone, with nothing watching for the falsification (docs/03 §6jjj). **The `distributions` encrypt-only bound**: `ENCRYPT_ONLY_PREFIXES` held exactly ONE member for its whole life, on the ground that settlement amounts are write-only; the claim was true when written (M18 PR2, `eef5606`, crediting M18 PR1 with correcting an earlier comment that asserted a route which did not then exist), and M23 PR4b (`4e125e1`) then shipped `distributionAmount`, so from 2026-08-21 every dual-control amount check an operator made breached the loudest class in the table at count 1 — the `asset`/`sentinel` defect in a different token (that row went loud as `unmodeled_principal` for want of a row, this one as `encrypt_only` for an absolute claim that had gone stale), a reviewed path raising the alarm reserved for unreviewed ones. `boundFor` returned the encrypt-only zero ABOVE the principal test, so PR2's `actorType` work could not clear it either. The class is **DELETED, not emptied** — an empty map leaves a branch `boundFor` cannot take, a `BoundName` member nothing can produce and a fence that loops zero times, all green and all meaningless — which also makes `undecidedPrefixes` stricter, since every registered prefix must now carry a reviewed NUMBER rather than a prose sentence. **TWO rows replace it, one per principal class**, because both reach the site and modelling one would leave the other breaching at count 1 on a reviewed path; the operator's ceiling is the TIGHTER one, on reach rather than on the act. Membership is DERIVED ACROSS A PACKAGE BOUNDARY — the audit spec reads `admin.service.ts` for `actorType: isOperator ? 'operator' : 'user'` and demands a named bound for both arms, so reverting PR2's derivation reddens a bound test in another service. That cross-package read needed `apps/services/audit/turbo.json`, and the repo's own turbo-inputs fence named the omission before the paragraph describing it was written. **`attachCaseEvidence` by an unlinked reporter**: `report` refuses at intake unless `isLinkedContact` holds now, and PR1 made the five gated reads re-derive the same fact — this write was the last member of that category the rule had not been applied to. The category is DERIVED rather than asserted: six routes authorize through `caseResource`'s frozen `reported_by`, Cedar grants a reporter exactly `read` and `evidence_add`, so a reporter reaches two and only one writes. `getCase` stays open deliberately — §6g's evidence argument, and closing it would remove no information while pushing the read onto a route that emits nothing. The refusal is a DISTINCT token, not the uniform 404: the reporter still sees this case in their own list, so a 404 on the attach would be a control firing wearing the face of an outage. **AND THE STACK E2E NOW DRIVES THE ROUTE THAT HAD NEVER BEEN DRIVEN.** The bounds file credits the suite's zero-anomaly gate with keeping fail-closed-loud defaults out of legitimate journeys; for `distributions` that credit was unearned, because no journey had ever reached the amount route — the gate asserts over anomalies raised inside its own run's window, so the traffic that would have raised one was never generated and the gate could not have gone red. PR3 adds the journey: intake, review, the waiting period advanced by SQL (the one reality an e2e cannot honour — the DDL floors it at five days), verification, a recorded distribution, and the amount read back by BOTH principal classes, asserted as a SET of `(actor_id, actor_type)` pairs so two rows attributed to the executor cannot satisfy a length check. **Proven able to fail**: reverting the operator bound to zero and rebuilding the audit image reddens the burst test's zero-anomaly assertion naming `distributions_operator`, while that same test's own `mfa_methods_user` poll stays green as the positive control. **THE MEASURED PEAKS MOVED WITH IT** — both rows go from 0 to 1, and 1 is a FLOOR proving reach rather than a workload sample, which both notes and the field's docstring now say, because sizing by arithmetic on that 1 would put the executor ceiling at 50. **AND THE COUNT GATE MOVED IN THREE PLACES**: dev 33/4 → 34/4 and production 23/14 → 23/15, BOTH MEASURED on a stack brought up in each mode rather than derived from the `describeDev` wrapper; and `assert-stack-counts.test.mjs` turned out to be the THIRD consumer of a number its own workflows say to derive at exactly that point, so its fabricated result and its two profile pairs are now read from the call sites. That removed an accidental tripwire — the pinned fixture had been the only thing catching one workflow's number moving without the other — so it is replaced by an explicit assertion that the two DEVELOPMENT pairs agree, proven by mutating `images.yml` and watching only that case go red. **Explicitly NOT E1** — no AWS anything. Distinct from M46: that row is the OPERATOR authorization model, and this caller is not an operator. **PR3 carries no sha of its own**, and deliberately: every other PR here is cited by its SQUASH-MERGE commit, which no commit can know about itself — PR1's `de91ae3` was written by PR1a (`2e4b2aa`), and PR3 is M48's last, so there is no successor to fill a placeholder. A literal `TBD` left standing is the rot this row exists to avoid |
| M49 | The transitions a status machine makes without saying so | **IN PROGRESS.** **ADDED BY M49 PR1**, which fixed docs/03 §6dd's distribution item and asked what else was in its category. The answer is that the shape is not settlement's: a table with a `status` CHECK, a verb that moves it, and an audit vocabulary that covers some of the transitions. **PR1 (2026-09-02) DID SETTLEMENT — docs/03 §6kkk.** `setDistributionStatus` permitted five edges and audited one; `settlement_cases` had two rungs — `active` and `distributing` — reached only as a side effect of another act and emitting nothing of their own. Four members join the closed `AUDIT_ACTIONS` vocabulary, chosen by maps the emitter indexes rather than by conditionals, and every row carries the EDGE rather than the target — `completed → disputed` undoes a payout and `approved → disputed` does not, and only the prior status tells them apart. A fence derives its corpus from settlement's own DDL (three tables carry a `status` CHECK) and DRIVES every edge, with the stage machine — already total — kept as its positive control. **NOTHING WAS RETIRED**: folding the three targets into one `status_changed` action would have turned every historical `settlement.distribution.completed` row into `event_hash_mismatch`, the token real tampering produces, because `ChainVerifier` re-parses stored rows against the LIVE schema and the table is under `REVOKE UPDATE, DELETE`. **PR1 ALSO FIXED `settlement.stage.requested`**, found by asking what else was in the envelope category: three of the four `settlement_access_stage` emitters named the decedent and the REQUEST named nobody, so "who asked for access to this estate" was the one rung unanswerable from the dead person's side. **PR2 (2026-09-04) ASKED THE SAME QUESTION OF A CONTROL — docs/03 §6lll.** Not "did it record what happened" but "can it say whether it ran": `gate-image-scan.mjs` read an absent `matches` key as zero findings, so `{}`, `{"error":"database is invalid"}`, `[]` and a schema rename of `matches` each printed `application (blocking): 0` and exited 0 — a vulnerability gate answering for a scan that never happened. It now refuses anything not proving a scan occurred, on what grype's own report carries (`matches` an array, `descriptor.db.status.valid`, a bounded `built`), under a token and an exit code distinct from a real finding so the caller retries the refusal and never the finding. The step gained the three attempts its SBOM sibling has had since a download endpoint 503'd, and `gate-image-scan.mjs` — the only script under `.github/scripts` with no companion, which `ci.yml` said out loud — gained the test that found all four shapes. Two probes in the web smoke test had the same defect and are fixed with it: a curl `000` and a failed header read each printed the exact control finding they exist to print, an outage wearing the face of the control. **PR3 (2026-09-08) IS THE EDGE, ONE LEVEL BELOW PR1 — docs/03 §6mmm.** PR1 made three machines audit every TARGET they permit; four statements inside them can move a row from more than one prior status, and THREE of those four produced events that recorded the destination only (the fourth is `setDistributionStatus`, which PR1 had already given a `from`). Rejecting a case from `waiting_period` unlocks a living person's identity account and lifts the documents legal hold while rejecting from `verifying` does neither; revoking a stage from `approved` takes access away while from `requested` it refuses an ask never answered; closing from `verified` means an estate closed with no stage ever approved and nothing recorded. One row shape each, and `settlement.case.closed` carried `detail: {}`. **NO NEW ACTION** — four optional keys on four existing envelopes, an edit `ChainVerifier` tolerates because `detail` is an open record; making a key REQUIRED is the edit it does not, since the verifier re-parses stored rows against the LIVE schema and a parse failure is `event_hash_mismatch`. **THE READ HAPPENS AT THE WRITE**, and that is a correctness property rather than a style: `confirmVerification` moves the row to `verified` and then unwinds it through the same `markResolved` INSIDE ONE TRANSACTION, so the status the service read at the top is `waiting_period` in both arms and a caller reporting it writes a false, permanent edge in the arm where a living owner came one commit from being entombed. **THE BOUNDARY WAS MEASURED RATHER THAN ASSUMED, AND IT IS NARROWER THAN A FIRST DRAFT CLAIMED**: a `SELECT` on the line above the UPDATE returns the same value — reimplementing the method that way leaves every test green, a survivor meaning the edit is not load-bearing — because under READ COMMITTED the CTE is exactly as stale as a pre-read and both rest on the caller's row lock, demonstrated on 16.15 with two connections and EvalPlanQual. The CTE is preferred because it cannot DRIFT back up the method. The false edge is also not undetectable: the version trigger is `FOR EACH ROW` and each of these statements matches one case by primary key, so the interlock arm leaves two prior images where the other leaves one — recoverable by an inference over an unchained table, which is the construction §6kkk rejects. `settlement.int.spec.ts` drives the case where the status a top-of-method read would report, the in-transaction status and the post-update status all DIFFER, against Postgres, because the in-memory doubles agree with any of them. **THE RULE IS DERIVED AND SO IS ITS POSITIVE CONTROL**: a scan of EVERY source file in the service finds NINE statements writing a `status` column and classifies each by the from-predicate the runtime compares against — five single-valued and owing nothing, four multi-valued and owing a `from` — keyed on `<table>:<what is assigned>` rather than on a method name a rename could hide. `StagesRepo.decide` is exempt because the scan READ its predicate, not because the fence says so, and an unreadable predicate classifies as owing a `from`, which fails closed. Twelve edges are driven and compared as SETS, and every recorded `from` must be a member of the table's own DDL vocabulary — the assertion that catches a DOUBLE left answering a boolean, which every `=== null` guard in the service would accept and `AuditEventSchema` would store as `true`. **THE SCAN'S FIRST THREE SHAPES WERE ALL WRONG AND ITS OWN REVIEW BROKE EACH ONE LIVE**, which is the part worth carrying: it matched only `*.repo.ts` in a directory that already holds a `dek.repository.ts` issuing an UPDATE; it required `RETURNING`, `status` first in the SET clause and uppercase keywords, so a plausible two-status cascade was simply absent and the count never moved; and it probed for the first `status = '<literal>'` it could find, so `(status = 'a' OR status = 'b')` passed as single-valued, as did an `= ANY($n)` under a `--` comment naming one status. All three went green, and so did a fourth status table whose `CHECK` body wrapped over three lines. **THIRTEEN MUTATIONS, ONE SURVIVOR, ONE POSITIVE CONTROL** — eight in the change's own battery and five raised by its review, each of the five green before the fence was repaired; the survivor is the pre-read reimplementation above, classified as not load-bearing rather than argued away. **ONE MUTATION SAYS WHICH LAYER**: defeating the CTE reddens the integration spec and nothing else, and reporting the caller's earlier read reddens the unit fence and nothing else — the unit fence proves the wiring, only Postgres proves the statement. **AND §6kkk'S OWN RESIDUAL WAS WRONG TWICE IN ONE SENTENCE**, corrected at the source: it named a `markVoided` that does not exist, and enumerated two `markResolved` call sites where there are three — omitting the one whose from-set contains `verified`, the value the whole fix is about. **NO SURFACE CHANGED, CHECKED RATHER THAN ASSUMED**: the operator console's case timeline is built from COLUMNS and reads no audit event, so nothing renders `detail.from` and there was no journey to drive — which is itself a residual, since the human surface keeps exactly the fusion the trail just lost. **PR3 RECORDS EIGHT RESIDUALS**, and the split is worth stating because a first draft of this sentence got it wrong: THREE were raised by the adversarial review and did not exist before it, THREE MORE were written beforehand and materially CORRECTED by it, and two survive as drafted. In order: `advanceStatus`'s second spelling; two discarded compare-and-set booleans, the worse of which is followed by the one irreversible transition in the machine; the `requested` revoke arm with no shipped consumer; the timeline's identical fusion; two category members outside settlement (`sessions.mfa_level` and `documents.execution_status`) recorded nowhere; identity's `SettlementLockService.setState`, which reports a NON-TRANSACTIONAL read where the statement knows the answer; and two **M45** items — the scan's remaining bounds (one service, one column name) and `revokeStage`'s dual control, where deleting either layer alone leaves the package green. **PR4 (2026-09-08) TOOK THE VAULT'S EMERGENCY-ACCESS LADDER — docs/03 §6nnn**, widened by PR3's census: §6kkk named `rearm` and `revoke`, and `markDenied` and `markReleased` were in the same state and named in neither section. (That census also REMOVED a member — profile's `contact.link.claimed` is an attribution defect, not an edge one, and both statements in its ceremony pin a single prior state.) **PR3'S DERIVATION DID NOT TRANSFER, AND THAT IS THE FINDING**: every status write in the vault service is `WHERE id = $1`, so there is no from-predicate to read and PR3's scan would have demanded a `from` on all five statements — fail-closed and wrong, because `markRequested` genuinely admits one prior. The vault narrows its priors in TYPESCRIPT, and the fence reads the guard chain instead: an allow-list guarding a throw, a refusal list in the method or in a helper it calls, or no comparison at all, with a fourth shape unreadable and owing a `from`. The vocabulary is a DIFFERENCE rather than a list — the DDL CHECK minus the statuses nothing writes (`requested` is in the CHECK and in `PolicyStatus` and no statement assigns it) minus the status no lock can observe (`markRevoked` also sets `deleted_at`; both locking reads filter `deleted_at IS NULL`). Thirteen edges are driven against Postgres and compared as SETS. **NO COMPARE-AND-SET GOES WITH THE CAPTURE**: `deny` and `revoke` admit all four live priors deliberately, because M27 PR3a made admitting deny on a released policy what keeps re-collectable release legal under the rule that the protective action must never be harder than the permissive one — so a predicate admitting all four is a no-op and one that narrows inverts that rule in the change citing it. **AND NO `to` KEY**: five statements, five literal targets, one emitter each, so a `to` would be a second copy of what the action id determines; the verb-to-target map is asserted instead, which is how the two ids whose verb disagrees with the status their write produces (`requested` writes `waiting`, `rearmed` writes `configured`) stay visible. §6kkk's vault sentence was WRONG IN TWO WAYS and is corrected at the source — six statuses admitted but five produced, and four fusing statements rather than two pairs. On the emitters it was RIGHT, and an earlier draft of this row was wrong to call it false: the action id does record the target, and what the emitters lacked was the source. §6mmm's forward reference was wrong too: it said denying a `released` policy "stops nothing", when since M27 PR3a a denial there ends the arrangement's ability to hand over more and the grantee read answers 403. **§6nnn RECORDS ELEVEN RESIDUALS** — nine M49, two M45 — of which FOUR did not exist before the PR's own survey and its adversarial review: `softDeleteAllForOwner` retiring every live policy with no status write and two callers that disagree about auditing it, `vault.emergency.configured` being vault-scoped so N creations and N retirements fuse, `request_blocked` already carrying the prior status under the name `reason`, and — found by sweeping the service for other members of the category rather than by planning — `vault.recovery_key.published` fusing a first publication with a key SUBSTITUTION, on a ceremony whose own migration comment asserts a later key change is detectable. The coverage floor was NOT ratcheted and that is deliberate: measured 93.91/79.44/95.68/94.13 against 93/78/95/94, where functions and lines each sit one uncovered unit above their floor already, and branches could go to 79 only by leaving two — which is the "just below the measurement" setting PR3 argued against. **WHAT PR1 RECORDED AND DID NOT FOLD IN**, on the rule M40 PR0 set — TEN residuals, six of them found by the PR's own adversarial review rather than planned. This row's remaining scope is the machines and envelopes. FIVE OF §6kkk'S SEVEN ARE NOW CLOSED — two by PR3 (`settlement.stage.revoked` not saying which arm it revoked, and the case machine's terminal edges, where rejecting in `waiting_period` unlocks a living person's account and rejecting in `verifying` does not, on one row shape) the vault's emergency-access ladder by PR4, the erasure driver's release and terminal rung by PR5, and `plaid_items`' `error` and its recovery by PR6. **PR5 (2026-09-08) TOOK THE ERASURE LADDER — docs/03 §6ooo**, which is §6kkk's OTHER half: PR3 and PR4 fixed events that could not say which transition they recorded, and this one fixes transitions with no event at all. `erasure_requests.status` has five writing statements and four transitions, and of the four exactly ONE was audited — the owner's withdrawal; the driver's three (claim, release, completion) were silent, and the second pre-existing member records the INSERT that creates the row rather than a transition. It looked covered because the destructive steps are audited by CONSEQUENCE, but a RESUME TAKEN AFTER THE CLOSE skips every one of them, so a driver picking up such a half-finished erasure could run its whole leg and leave nothing behind — a resume taken BEFORE it re-runs the leg and files all three, which the same patch's drive asserts, and an earlier draft of this row stated the skip without that qualification. **PR3'S DERIVATION TRANSFERS HERE INTACT AND THE ANSWER IS NOT TRIVIAL**: every statement carries a from-predicate in SQL, three pin ONE prior and owe nothing, and only the claim admits two — `pending` begins a destruction, `executing` resumes one — so one statement of four carries a `from` and the other THREE single-prior statements are the fence's derived positive controls. Plain `RETURNING` cannot answer it, since both arms land on `executing`, so the prior comes from PR3's CTE pre-image with the lock taken inside the CTE. **THREE MEMBERS JOIN `AUDIT_ACTIONS`** — the first vocabulary growth since PR1, bringing back the consumer-before-producer constraint, and `erasure_completed` cannot fire until all eight domains gain transport, which is the argument for adding it now rather than later. Both compare-and-sets are now read, **AND `releaseClaim`'S IS NOW EXECUTED BY A TEST, WHICH IT WAS NOT.** Only that one: `completeIfAllDone` was already driven against Postgres at the base commit by `completes only once every domain reports`, which asserts its WINNING arm, so making it throw would always have reddened. An earlier draft of this row said BOTH were unexecuted, generalising a measured fact about one method to two inside the row that records a milestone about test coverage. The drive named for the release did not reach one: it moved the account BEFORE the sweep, so the claim's own `u.status = ANY($3)` refused the row and the leg never started — `runDueErasures` answering 0 was that refusal, not a claim handed back. Making `releaseClaim` throw unconditionally left EVERY test in the package green — 561 of them, this PR as it stood before these three drives existed, against 553 at the base commit and 564 now, which is how a statement carrying a new audit event turned out to be executed by nothing. The reachable setup is the RESUME arm, which carries no status allowlist by design, so an owner who becomes ineligible AFTER a claim is re-claimed and then stopped by `closeAndUnlinkEmail` restating that allowlist inside its own `WHERE`. THREE tests now cover what the renamed drive never reached, one per layer: the statement answering false on a row that already moved, against Postgres; the `if (released)` guard, against a double forced to false; and the release driven end to end with the audit row it files. The misnamed drive is renamed for the refusal it genuinely proves. **§6ooo RECORDS FIFTEEN RESIDUALS** — nine M49, three M45, three ACCEPTED. FIVE of those fifteen did not exist before the PR's own adversarial review, and the sharpest is not a documentation gap: a CLAIM HAS NO OWNER, so two overlapping sweeps can both hold one request, and the loser's release moves the row to 'pending' and files `auth.account.erasure_released` — an event meaning nothing was destroyed — while the winner is still shredding the DEK. The stranding predates this PR; the FALSE RECORD is new, which makes it this PR's to record even though the remedy is a claim lease and therefore docs/02's to approve. The fence found ONE defect in itself before it found any in the ladder: it read the claim's table name as `OF`, because `FOR UPDATE OF r` precedes the real `UPDATE`, and dropped the one statement the PR is about while reporting three healthy-looking ones — the SET comparison is what surfaced it. A second suspected defect turned out not to be one: the backtick theory was wrong, and the stripper written for it is not load-bearing, which §6ooo records rather than crediting it with a fix. A third residual is the fence's own bound — the claim's from-set is not actually read, since its `WHERE` moved into the CTE. **PR6 (2026-09-09) TOOK THE PLAID ITEM LADDER — docs/03 §6ppp**, the fourth service and the first where PR3's derivation and PR4's AGREE: no write of `plaid_items.status` pins a prior in SQL (all four are `WHERE id = $1 AND deleted_at IS NULL`) and none narrows in TypeScript (the service reads `.status` once, to render it), so every one of the four owes a `from` by both rules at once. Three live priors — the DDL's four minus `revoked`, written only beside `deleted_at` and filtered by every writer — and four targets make twelve edges, and the trail could say none of them: `login_required` and `revoked` carried no prior, `error` had NO EVENT AT ALL, and `healthy` was recorded only as the `synced` that ships every successful sync, so a recovery and a routine sync were one row. **ONE MEMBER JOINS `AUDIT_ACTIONS`**, `plaid.item.errored`, and the recovery goes on `synced` as `from` rather than on a second member — `from: 'error'` on a `synced` row is an item coming back and `from: 'healthy'` the no-op, the edge recorded on the write and not the temporal join §6kkk rejected; a `recovered` member would sit behind `prior !== 'healthy'` and give one write two spellings (not, as a first draft of this row said, the consequence-guard shape PR5 found wrong — a guard on the prior is a guard on exactly what it records). **THE PRIOR COMES FROM THE WRITE**, by PR3's CTE pre-image with the row lock taken inside the CTE, as PR5 spelled it for identity, because every caller's row was read OUTSIDE its transaction — `revoke()` calls Plaid between the two — and `setStatus`/`markRevoked`, which returned `void`, now answer the prior or `null`; all four sites READ the null before emitting, and for the `healthy` write that meant moving it to the FRONT of the sync's transaction — `revoke()` locks the item then its accounts, the sync locked the accounts then the item, and the PR's review DROVE that cycle to a 40P01 with the step-up-gated revoke as the victim; the same reorder makes a sync that meets a revoke under the lock write nothing instead of resurrecting the accounts the revoke had just retired, which a first draft of this row had recorded as a residual and let `synced` announce with `from` absent, on an arm the review found executed by no test. **THE FENCE'S EXEMPTION SET IS EMPTY, AND IT SAYS SO**: nothing in the corpus owes nothing, so its "owes nothing" answer is proven only by SYNTHETIC controls, labelled as such — weaker than PR3's five or PR4's one — and one of those controls found a defect in the fence on its first run, a clause cutter stopping at the first `)`. It closes PR5's own bound for this service by reading every `WHERE` a statement contains, the CTE's included. Twelve edges are driven against Postgres and compared as SETS, the race is driven on two connections (a rival holds `revoke()`'s transaction uncommitted while the owner's sync waits on the lock), the emitters have a spec of their own over a real producer, `plaid.item.errored` is pushed through the real ingestor by one added step in the plaid e2e journey, and an EIGHT-mutation battery reddened the layer predicted for each, with the one predicted survivor at the Postgres layer alone: passing the caller's stale `item.status` for the write's answer stays green end to end because no drive races the two — the mirror of PR3's own caller's-earlier-read mutation, which reddened its unit fence and nothing else, and NOT of PR3's battery survivor, which was a different edit (a `SELECT` above the `UPDATE`) judged not load-bearing — and is caught by the fence's pairing test, by its pin that the service reads `.status` once, and by a unit drive the review wrote for exactly this disagreement, which flips the row from inside the gateway stub so the caller's pre-read and the write's answer differ. **NO SURFACE EXISTS** — neither `apps/web` nor `apps/bff` mentions plaid — so the browser-drive rule does not apply, said rather than skipped. The coverage floor was RATCHETED from 60/55/40/60 to 70/62/50/69 on the corpus the file's header names — the run WITHOUT a database, 74.27/66.66/54.47/73.02 on this commit — after a first draft calibrated on the with-Postgres run (89.26/76.7/91.86/88.84) and went red on every machine without one; the floor is now a lower bound in both configurations. **THE PR'S REVIEW REWROTE THE FENCE AS WELL AS THE SERVICE**: six mutations left all fourteen of its tests green — a writer in a method whose first template literal was not SQL, a fifth status added by a later migration, a guard on a destructured `status`, a detail key renamed away from `from`, a negated predicate read as a PIN, and a stale pre-read laundered through the transaction's own return. Four more followed: a rung spelled `INSERT … ON CONFLICT … DO UPDATE SET status`, invisible to every reader in the file INCLUDING the raw floor advertised as their backstop, in an idiom this service already uses on its accounts table; reversing `revoke()`'s two statements, which reintroduced the deadlock with the whole suite green because the two-connection drive hand-rolls revoke's half rather than reading it; renaming the repository property the reach reader was anchored on; and — found re-running the battery against the final tree — a second `@Injectable` in `src` injecting the same `ItemsRepo` under any other property name, which the fix for the rename walked straight past, because it resolved that name once out of the service file. Each reddens a named test now, beside a RAW count of `UPDATE plaid_items` and, for the reach reader, a raw floor of its own: the writer's name called on any receiver at all. An unattributable writer is a FAILURE, not an absence. **§6ppp RECORDS TEN RESIDUALS** — seven M49, one M45, two ACCEPTED — two of them driven by the review's interleaving lens rather than planned — overlapping syncs rewind the cursor (the write-back compares nothing), and a sync can commit `healthy` over a newer `login_required` that Plaid never resends — with a third interleaving finding, a webhook-driven write's VERSION ROW naming the owner as actor while its audit row now names the platform, FOLDED into the attribution bullet that already existed rather than added beside it, which is why nine went to nine. Also: a webhook-driven sync is attributed to the OWNER as a `user` while its decrypt files as `service` and its failure now as `system`; two webhook codes land on one status and one event that cannot tell them apart; a dead token answers `500 internal_error` on both routes, an outage face on a remediable condition, and Plaid RETRIES a 500 — each retry another `error → error`; the domain topic carries the same four transitions with no edge, now PROBED for a consumer appearing; a latent `dek_id` mismatch on `accounts`, unreachable until `@estate/crypto` learns to rotate, ACCEPTED with a probe watching for exactly that; and — found by the PROSE pass after the code review — that `plaid.item.errored` still has no DEPLOYED-consumer journey, since the plaid e2e bridges producer to ingestor in-process and `stack.e2e.spec.ts` does not drive plaid at all, which is the same gap §6ooo records for PR5's three members. What is left of §6kkk, BESIDE §6mmm's eight adds, §6nnn's eleven, §6ooo's fifteen and §6ppp's ten: the operator console timeline, which still cannot show the two rungs PR1 put on the trail; and profile's `contact.link.claimed`, null beside a sibling two lines away that names the owner. TWO, not three: `plaid_items`' `error` and its recovery left this list in PR6, as the erasure driver's release and terminal rung had in PR5 — whose `releaseClaim` and `completeIfAllDone` booleans are returned and READ rather than discarded, the construction PR6 gives `setStatus` and `markRevoked`. **THAT LIST WAS ONE SHORT UNTIL M49 PR2**: it enumerated six where §6kkk carried seven, and the vault ladder was the absent one — PR1's own prose pass caught the identical omission in its commit message and did not re-check this row, which is the failure mode that pass exists to catch. Three went to other owners — the operator-breadth fence cannot see the FIVE methods that ask `gate.is` rather than `gate.assertIn`, two of them writes, and PR1's own fence is keyed on a column NAME so a lifecycle column spelled otherwise is invisible to it (both **M45**); and whether an operator moving a distribution should count toward the ceiling is undecided (**M46**), now carrying a `PRECONDITIONS` probe so the PR that routes `/status` to the console is told which decision it retired |
| M50 | The supply chain the pipeline does not pin | **PLANNED.** **ADDED BY M49 PR2**, which hardened the image scan gate and then asked what else was in its category. Six of §6lll's nine residuals are this row: CI references twelve distinct third-party actions and pins NONE of them to a digest, while `postgres:16@sha256:…` in ci.yml is digest-pinned on a throwaway test database — the repo knows the idiom and spends it where it buys least. Both scanners are exact-tag but not digest-pinned; `node:22` floats inside the job whose stated subject is that the artifact must not depend on its build environment. Eight of thirteen jobs carry no `timeout-minutes`, and the two genuinely longest DO carry one — the ceiling went where someone was already thinking about duration, not where it is missed. `pnpm install --frozen-lockfile` is the most-repeated network fetch written as a `run:` step, at five call sites, none retrying — and no `uses:` action retries either. And a redirect creates its output file whether or not the command succeeds, so an exhausted retry loop uploads a 0-byte artifact GREEN — `if-no-files-found` never fires on either image analysis step, which makes the `warn`/`error` difference against the extension archive not the distinction it appears to be. The through-line is the one M49 PR2 established one layer down: a step that cannot say whether it did its job reports success by default |
...
> **SUPERSEDED BY M48 PR2 (docs/03 §6iii).** The hardcode is gone —
> `worklistViewed` derives `actorType` from the caller — so reusing
> `settlement.queue.viewed` states nothing false and costs no consumer
> deployment, and both halves of the paragraph above are spent. `executorCases`
> emits `worklist: 'executor'` now. The read was also mis-described as "their
> own worklist": it lists estates belonging to OTHER PEOPLE that this caller
> administers, which is the disclosure `asset.estate.viewed` has recorded on the
...
### M27 — emergency-access reader + vault item restore (scoped 2026-08-22)

**Why this milestone, in one fact.** The emergency-access ceremony is SHIPPED
and, completed, it delivers nothing while spending itself. A grantee who waits
out the period, is not denied, and passes the settlement gate reaches
`apps/vault-web/src/client/app.ts`, which reconstructs the owner's master key
on their device, calls `wipe()` on it, and tells them the arrangement "is now
spent"; `emergency.service.ts` then refuses re-release at four sites with
`already_released`. In the ONE scenario §5.2 exists for — the owner dead or
...
audience BECAUSE an overwrite is "recoverable" — a security argument resting
...
bullet, cited from §6a, §6j and §6k), and not one of them is either half.
Carrying them was how M27 looked bigger than it is, and how its actual
headline scope came to live only in a queue-row sentence. PR0 re-owned all
five:

- The one-tap deny token and `emergency.reminder` → **M30**. An email-borne
  deny is a per-recipient URL, which `templates.spec.ts` refuses outright and
  which docs/03 §6c cites as a CLOSED half of the M6 leakage item; an SMS one
  needs E4. An IN-APP deny needs neither, and M30 builds the in-app feed.
  `emergency.reminder` needs something to fire it and `ScheduleModule|@Cron(`
  matches zero files repo-wide, so M27 would have carried it untouched.
- The SRP abuse bound and full-history rollback detection → **M39**, a queue
  row this PR adds. Both are §6a Zone A hardening, both vault-local, both
  unblocked, neither is a reader or a restore.
- Token-gated reset → **`[ACCEPTED]`**. Every compensating control the bullet
  names has shipped; what remains is the permanent fact that a lost password
  cannot be proved. There is no work owed, which is what the tag means.

**§5.2 vault subsets stay deferred and the reader ships all-or-nothing**
(docs/03 §6uu, owned by M28). A released grantee reads every item, because the
master key they reconstruct opens every item key.

#### What PR0 changes

- **`vault_items.deleted_reason`** (migration `004`), CHECK-tied to
  `deleted_at`. `reset` soft-deletes with the SAME stamp `deleteItem` uses and
  then replaces the keyset, so those blobs are cryptographically dead; nothing
  distinguished them. A restore list built as `WHERE deleted_at IS NOT NULL`
  would have offered rows that can never decrypt, failing as a silent AEAD
  error on click. The version trigger could not have helped:
  `app.change_reason` is read by `packages/db/src/conventions.ts` and SET BY
  NO SERVICE, in any cluster, so `vault_items_versions.reason` has been NULL
  for every row ever captured.
- **`test/restorable-corpus.spec.ts`**, which derives the vocabulary from the
  DDL across the whole migrations directory (one file may define it — the M24
  `categories.test.ts` lesson, where a fence reading one checksummed migration
  was frozen with it), asserts the restorable/unrestorable partition is total
  with both arms populated, and anchors "every writer records why" on the SQL
  rather than on a method name.
- **The residuals fence learns its own reach.** docs/03 §6j:1623 recorded
  "there is no restore surface", assigned it to TB7, carried no tag, and was
  invisible to `threat-model-residuals.spec.ts` for five milestones — TB7 then
  shipped as M21 without it. The bullet moves into §6j's declared residual
  region tagged `[OWNER: M27]`, and the fence now declares, per lead-in, the
  132 §6 bullets its corpus does NOT reach, so a residual written where that
  one hid changes a count and goes red. Two parser defects surfaced doing it:
  bold lead-ins whose run wraps across lines were read as half a sentence (one
  such lead-in in §6pp had gone unclassified since M25), and `## 7. Validation
  program` — which sits BETWEEN §6hh and §6ii — never ended §6, so its four
  bullets were attributed to §6hh.
- **`AUDIT_ACTIONS`** gains `vault.emergency.items_read` and
  `vault.item.restored`, a milestone before their producers, because the
  vocabulary is closed and an older consumer drops what it does not know with
  no mark on the producer.
- docs/01 (the Zone A boundary), docs/03 §6uu, this section, docs/06.

**Two things PR0 deliberately does NOT ship.** The `vault.read_by_grantee`
notification kind waits for the reader PR that emits it: adding it here would
mean a kind, a template, and TWO DDL CHECKs (the notifications service's and
vault's own `emergency_access_notifications`) with nothing to send them. And
`RESTORABLE_REASONS` ships with zero callers — PR1's restore list is its first
— which is the one place this PR breaks the ship-a-capability-with-its-caller
rule; it is here because the fence's claim needs a subject, and the
alternative is PR1 inventing the classification beside the query that reads
it.

**Off the M24/M25 PR0 shape, deliberately.** Both precedents were docs plus
exactly one fence and no runtime code. This PR0 carries a migration, two repo
signatures and two call sites, because the fence it owes DERIVES from the DDL
— the column has to exist for the fence to have an input, and a column with a
paired CHECK and no writer is a broken table.
...
- **PR6 — the extension's step-up refusals**, landing a finding PR5 recorded and
  did not fix. A wrong authenticator code reached the popup as PAIRING copy —
  "create a new one in Estate under Security", about a number that is read
  rather than created — while a genuinely revoked device was told to try the
  current code. The two sentences were swapped because the discriminating branch
  was copied from `apps/vault-web`, where `UNAUTHENTICATED` is the right code
  because that origin maps EVERY 401 to it, into a client that splits 401 three
  ways. The 429 guessing cap had no branch at all and read as an outage,
  inviting the retry it was refusing; the extension was the fourth client on
  this ceremony and the last without that member. The refusal corpus is now
  walked out of identity's own source rather than listed. docs/03 §6aaa,
  residuals owned by M44.
