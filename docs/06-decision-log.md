...
- 2026-08-27 — the four reads M40 PR4 called "case-exists reads" are not case-exists reads, and the vocabulary undersold what the adjudication left ACCEPTED. `listDistributions` returns the beneficiary contact, the asset, the approving operator and whether an amount exists; `timeline` returns the full case history; `listStages` returns the access ladder. Seeing that your own report exists is `getCase` and `listMyCases`, on different gates. So M48 PR1 takes all five reads from an unlinked reporter rather than only the money route — and the alternative was MEASURED, not argued: refusing only `distributionAmount` reddens `admits exactly the callers listDistributions admits`, a deliberate SET comparison encoding M23 PR4b's argument that a reader who may see a distribution exists is the reader who may see its amount. Shrinking both sets together is the only option that keeps it.
