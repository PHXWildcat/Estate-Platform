...
Architectural and process decisions, newest last. Moved verbatim out of `CLAUDE.md`
on 2026-08-19: it had reached 8,386 lines (~159k tokens) and was auto-loaded into
every session and every subagent, which is what killed five of seven review agents
on 2026-08-19. The durable rules it contained now live in `CLAUDE.md` and
`.claude/rules/`; this file is the full record and the archive of the reasoning.

Append new entries here, in the format: `- YYYY-MM-DD — decision — rationale`.
...
- 2026-08-13 — M18 PR2 SHIPPED: the decrypt-rate detector inside the audit
  ...
  the zero default can never be reached by forgetting. **SUPERSEDED IN PART
  2026-09-02 (M48 PR3):** the third zero is gone. `distributions` acquired a
  read route in M23 PR4b and had been breaching on every legitimate reveal
  since, so the encrypt-only class is deleted and the prefix carries two
  reviewed bound rows. `undecidedPrefixes()` is correspondingly STRICTER — a
  visible decision now means a bound row, never a sentence. The rest of this
  entry stands as written. (2) THE DETECTOR IS
...
- 2026-08-17 — AND THE RESIDUAL FENCE PR0 SHIPPED WAS UNDER-COLLECTING, found by
  ...
  the fourth upward move of a figure that had never once been revised down
  (five → twelve → fourteen → eighteen). [M40 PR1, 2026-08-25, revised it down
  for the first time — to seventeen, by RE-TAGGING one bullet to M41 rather than
  by recounting. The upward-only claim was true of measurement and is not true of
  membership.] PR0 stated its bound as "a residual
...
- 2026-08-20 — fences are anchored on what the RUNTIME reads, never on an identifier a caller chose — the audience fence grepped `SESSION_AUDIENCE_METADATA` while `CallerGuard` resolves the string it holds, so `@SetMetadata('estate:session-audiences', [...])` widened any route in six services and matched nothing; the same shape defeated the `OperatorsRepo` write fence (keyed on the property name `operators`) and the gate's handle rule (keyed on the literal `tx`).
- 2026-08-20 — a fence that cannot READ a construct must refuse it, not skip it — `compose-parity` silently ignored `<<: *anchor` merge keys, hiding every merged variable from both the parity assertion and the operator-web credential scan; it throws now, and doing so immediately revealed six service blocks whose comment lines were already going unread.
- 2026-08-20 — comment stripping in a fence is a PARSE, not a regex — `source.replace(/\/\*[\s\S]*?\*\//g, '')` treats a `/*` inside a string as a comment opener and deletes to the next closer; `ts.createScanner` was measured and still fails on regex literals, so `ts.createSourceFile` is the standard. 21 copies remain outside `auth-guard` as an accepted residual (docs/03 6ff).
- 2026-08-20 — an anti-vacuity floor belongs at every LEVEL, and a `for...of` over an empty collection is the quietest vacuum there is — the operator-web credential scan asserted nothing when its corpus was empty, and the residual fence's `count === 0` could not see a region that went blank AFTER its first bullet.
- 2026-08-20 — route-decorator vocabularies are DERIVED from `RequestMethod`, never hand-listed — two fences carried copies that disagreed (five verbs and nine); Nest 11 ships sixteen, so even the list called complete was short by the seven WebDAV verbs.
- 2026-08-20 — a control that lives in an unexported `main()` is a control no test can reach — the operator CLI's fail-closed broker gate was pinned by nothing while its spec's docstring claimed to prove it; extracted as `auditTransportFor` and pinned directly, with a test that says WHICH of the two layers it proves (this repo's refusal, not kafkajs rejecting an empty broker array).
- 2026-08-20 — an exemption reason is a claim about the world and rots like one — `EXEMPT_TB7_OPERATOR` said the operator platform was deferred, forty lines above thirteen routes the shipped console consumes; split into three true reasons and the constant deleted, the same standard applied to `EXEMPT_RECOVERY_SURFACE` in M20 PR3.
- 2026-08-20 — when a lookup cannot move before the authz gate, make the REFUSALS uniform instead — `setDistributionStatus` needs the case to evaluate the executor arm, so it cannot gate first like `approveDistribution`; every refusal reachable without authority now answers the same 404 an unknown id gets, while a caller WITH authority still gets `case_not_verified`, because fail-closed means de-escalate and not "refuse everything".
- 2026-08-20 — a suite that passes unchanged when a security fix lands is telling you the path was never tested — all 237 settlement tests stayed green when the three-way refusal became uniform, because no test had ever exercised a refusal on that verb; that is why the oracle survived two reviews.
- 2026-08-20 — a fact about a column belongs in the CATALOG, not in a `--` comment in the migration that created it — `granted_by`'s forensic meaning inverted when PR1 made `--by` mandatory, and 001 is checksummed and immutable; migration 005 sets `COMMENT ON COLUMN` so `\d+` and `col_description()` show the truth, which is what an investigator queries — the identity 004 precedent.
- 2026-08-20 — a corrected sentence is worth little until a test holds the DIRECTION — the catalog fence asserts "did NOT come through the ceremony" rather than the column name, because a comment that merely mentions `granted_by` passes a presence check while still being backwards.
- 2026-08-20 — an operator's BREADTH across estates is a control surface and volume is not — thirty actions on one case is a thorough operator, one action on each of thirty cases in an hour is a pattern; `settlement_operator_actions` counts `COUNT(DISTINCT case_id)` in a rolling window, so a rate limit on actions (which would have penalised the first and missed the second) is explicitly the wrong shape.
- 2026-08-20 — the breadth bound WARNS and does not refuse, and that is a deviation with a reason — settlement's human review is mandatory and time-sensitive, and a ceiling of 12/hour is a guess with no production data behind it; refusing on a guess makes a control that wears the face of an outage, so the first slice makes the pattern visible and the refusal (or a step-up re-challenge, the better shape) waits for numbers.
- 2026-08-20 — PROTECTIVE operator actions are never counted toward a ceiling — deny, reject and revoke write nothing to the ledger and the two vocabularies are asserted disjoint, because *the protective action must never be harder than the permissive one*: an operator near the ceiling must never hesitate to withdraw access.
- 2026-08-20 — coverage of a control is DERIVED from the AST, never hand-listed — `operator-breadth-fence.spec.ts` takes every method calling `this.gate.assertIn` as its corpus and fails on any that neither records nor carries a written exemption; it caught its own first defect, that `review.approved` and `verification.confirmed` were declared permissive kinds nothing ever wrote — two of six dead on arrival, invisible to any count.
- 2026-08-20 — an exemption from a control carries its REASON in the fence, not just its name — `reportProviderSignal` is exempt because it owns no transaction and a post-commit ledger row can be lost (under-counting, the fail-open direction), and that sentence is what a reviewer checks; a bare name would have read as settled.
- 2026-08-20 — the attribution a control counts is the AUTHORITY that admitted the caller, never the id on the row — `insertCase` takes `countBreadthFor` as its own field rather than deriving it from `reportedBy`, because an operator who is also a linked contact reporting through the contact path acted as a contact; deriving it type-checks perfectly and is wrong on exactly that input, so the test exercises the arm where the two facts DISAGREE.
- 2026-08-20 — a coverage fence must follow intra-class DELEGATION or it punishes the correct fix — `reportProviderSignal` records through `insertCase`, the helper that owns the transaction, and a fence reading only a method's own text would have called the one transactionally-correct verb an orphan; the call graph is read from the AST and closed transitively, and the resolver itself is pinned by a mutation that makes it answer `true` for everything.
- 2026-08-20 — a mutation that inserts a duplicate object key is UNFAITHFUL, not a surviving defect — the last key wins in JavaScript, so the mutant was byte-different and behaviourally identical and reported SURVIVED; the rule holds that a survivor means weak test, unfaithful mutation, or a change that is not load-bearing, and here it was the second.
- 2026-08-20 — a task whose test reads files OUTSIDE its package must declare them as turbo `inputs`, or the gate caches green while its only input changes — `packages/contracts`' residual fence reads `docs/03`/`docs/04`, and a docs edit that violated the tag grammar four ways passed locally as `cache hit, replaying logs` and failed in cold CI; the glob is deliberately wider than the two files read today, because too broad only re-runs a fast test while too narrow replays a stale pass on a disarmed gate.
- 2026-08-20 — `turbo run --dry=json` is NOT a usable observer of input hashing — it reported an identical hash across a real input change in both the fixed and unfixed configurations, while actual runs showed the hash moving; measure cache behaviour with real runs and a matched control that must stay IDENTICAL.
- 2026-08-20 — the hand-list of affected specs was short by three packages and eight specs, which is why this is a fence — twelve specs across eight packages were found by grepping for `__dirname, '..', '..', '..'`; deriving the same question from the AST found ELEVEN packages, and `apps/web` alone reached into four other packages from five different test files that nobody had noticed. `turbo-test-inputs.spec.ts` takes every test file in every workspace package as its corpus and asserts the set that NEEDS the declaration is exactly the set that CARRIES it, so both a missing declaration and one left behind after a spec stopped reaching outside go red.
- 2026-08-20 — an explicit `$TURBO_ROOT$` glob is NOT gitignore-aware, and that is the difference between a cache that never hits and one that lies — `["$TURBO_DEFAULT$", "$TURBO_ROOT$/**"]` alone gave two DIFFERENT hashes on two identical consecutive runs, because it sweeps `node_modules`, `dist`, `.turbo` and `.next`; appending one line to a gitignored `dist/index.js` moved the hash on its own, which is the direct proof. The canonical declaration therefore re-applies .gitignore as `!` exclusions, and the fence checks that hand against `git ls-files --others --ignored` rather than against a comment. The two failure directions are deliberately asymmetric: a missing EXCLUSION makes the hash unstable (loud and slow), a missing INPUT makes it stale (silent and wrong).
- 2026-08-20 — the declaration is one canonical list in every package rather than a precise one per package — a precise list is a second copy of the paths the spec already names and is free to drift from it, while several of these specs sweep `apps/**` and `packages/**` at runtime anyway, so the whole tree IS their honest input set; the fence holds the list as data and names any `turbo.json` that disagrees, including on `extends: ["//"]`, without which a package-level config REPLACES the root task and silently drops `dependsOn: ["^build"]`.
- 2026-08-20 — a workspace DEPENDENCY already propagates its hash, so a control that probes one proves nothing — `apps/operator-web`'s test hash moved on an edit to `packages/auth-guard/src/verifier.ts` even with the fix removed, because auth-guard is a declared dependency and `^build` carries it; the honest control probes a package it reads but does NOT depend on (`apps/services/settlement`), and there the unfixed hash was byte-identical across the same edit. The first reading looked like "the fix does nothing" and was a broken observer, not a broken fix.
- 2026-08-20 — the escape detector follows the LITERAL prefix and stops at the first non-literal argument, which is a stated limit rather than an oversight — `join(__dirname, '..', 'src', file)` resolves to the package's own `src` and is correctly INTERNAL; treating an unresolvable tail as an escape put `apps/services/identity` on the list for reading its own controller and would have demanded a whole-repo hash for every PBKDF2 suite in it. A non-literal that escapes at runtime is the accepted residual, and it is small because the literal prefix carries the `..` segments in every case in the tree today.
- 2026-08-20 — a BFF edge is a milestone-sized thing when it is the FIRST one to a service — `apps/bff/src` held five downstream clients and none was settlement, so every reporter/owner route had been complete and unreachable since M7; M22 PR3's real content is the edge, and the screens are the small part on top of it.
- 2026-08-20 — the OWNER's half of a surface ships before the REPORTER's, and the ordering is the rule and not a preference — filing a death report is already one tap by design (the controls sit on what a report can CAUSE, not on filing it), so shipping the reporting screen first would put the permissive capability in front of ten million people while the protective one still requires curl, which is precisely the inversion "the protective action must never be harder than the permissive one" exists to prevent.
- 2026-08-20 — `assertCanOrNotFound`'s exemption list mixed two routes that are not the same case, and only one of them earned it — the docstring parked `void` beside the owner's own `manage`, but `manage` builds its resource from the CALLER'S OWN id and cannot deny, while `void` locks a row by a caller-supplied id and then asks Cedar; so `void` answered 403 where an absent case answered 404, and any account able to step up on itself could ask whether a death case existed for a given UUID — the M21 PR4d oracle, on the one route a victim uses against a fraudulent case naming them.
- 2026-08-20 — asking WHAT ELSE IS IN THE CATEGORY found the second one — `addEvidence` has the same shape as `void` and reads like an operator path because it calls `gate.is`, which MEASURES operator-ness and refuses nobody; the three genuine operator writes call `gate.assertIn`, which refuses before any lookup, so a stranger there learns nothing either way. The distinguishing fact is a measurement-versus-refusal difference of one method name.
- 2026-08-20 — pinning an exception TYPE is not testing a refusal, and it is why this survived — the two existing `void` tests asserted `rejects.toThrow(ForbiddenException)`, which is green whether or not the answer distinguishes anything; the property is that two refusals are IDENTICAL, so the new tests compare status and body as values and a `refusalOf` helper exists because `toThrow` cannot express it. Nothing had exercised an `addEvidence` refusal at all.
- 2026-08-20 — a surface renders a settlement case from `resolution`, never from `status` — the DDL CHECK forces `(resolution IS NOT NULL) = (status = 'rejected_fraud')`, so an owner who kills a fraudulent case about themselves lands the row in a status spelled `rejected_fraud`; printing `status` tells the person who just used a protective control that fraud was found against them, and `owner_voided` versus `operator_rejected` is the only field that distinguishes the two outcomes.
- 2026-08-20 — an unrecognised status renders as a vague sentence, never as its raw token — settlement's vocabulary has grown twice, and a surface that prints an unknown enum verbatim shows a person the word `deceased_pending` about themselves.
- 2026-08-20 — the waiting-period setting lives on `/security`, as a SIBLING of `SecurityPanel` rather than a section inside it — it is emergency-access-configuration class and belongs beside the other standing protections on an account, while `SecurityPanel` is 1500 lines owning one coherent thing (credentials and sessions) and folding a settlement round trip into it would give that panel a second downstream and a second reason to re-render; `AccountSecurity` composes them, which is what it is for.
- 2026-08-20 — the settlement case surface gets NO `AppNav` entry, and the reason is not the operator page's — `/operator` is unlisted because a role-blind handoff should not be advertised; this one is unlisted because it is EMPTY for virtually everybody and a permanent nav item reading "Death cases" in an estate app is a standing memento mori for ten million people who will never have one. It is reached the way the fact arrives: an app-shell banner that appears only while a case naming you is open, plus a pointer on `/security` for anyone who comes looking unprompted.
- 2026-08-20 — that banner is deliberately NOT dismissible, which is a departure from the usual courtesy — a dismissed banner hides a live case from the only person entitled to kill it, and the case does not stop advancing because the banner was closed; it disappears when the case is closed and only then.
- 2026-08-20 — three settlement refusals earned three BFF codes because they have three remedies — `CASE_OPEN` is a control firing (the window is frozen precisely because someone is trying to use it), `CASE_NOT_VOIDABLE` is "too late to self-rescue" and is deliberately NOT the document `INVALID_TRANSITION` whose copy names a document, and `SETTLEMENT_UNAVAILABLE` says the transition rolled back and nothing happened. On a kill switch that last one decides an outcome: an owner who reads an outage as a refusal stops trying, and the fraudulent case stays alive on the difference.
- 2026-08-20 — the BFF projects a settlement case NARROWER than the service shapes it, and each omission is a decision — `decedentUserId` becomes the boolean `aboutMe` (computed against the session, so no user id reaches the browser to answer a yes/no), `reportedBy` is dropped outright because a bare UUID tells a person nothing and resolving it to a name is a cross-cluster read this edge has no business making, and evidence is COUNTED with `z.array(z.unknown())` so no document id or provider match id has a field here to land in — absence over filter.
- 2026-08-20 — `voidable` is a hint about whether to OFFER an action, never authority to perform it, and the drift direction is stated — settlement re-decides under a row lock, so a `VOIDABLE_STATUSES` that drifts wider shows a button earning a clean refusal while one that drifts narrower HIDES A LIVE KILL SWITCH; the copy therefore names the three OPEN statuses rather than enumerating terminal ones, and a fence reads both the service source and the BFF declaration because `apps/bff` cannot import from a Nest service package.
- 2026-08-20 — a null `identity.session` is a refusal everywhere except the `session` query itself — carrying on with an unresolved caller would compare an empty id against every `decedentUserId`, decide no case is about the caller, and render an empty reassuring page to someone who may have a live case against them; the failure mode of a silent default here is hiding the control, not showing a wrong number.
- 2026-08-20 — the platform could not tell a person which estates NAME them, and every consequence of that landed on the bereaved — `redeemContactLink` returns `Ok!`, nothing in profile/BFF/web reads the link backwards, and settlement's `reportable-estates` is therefore a list of bare UUIDs; a capability you hold and cannot enumerate is not one you can exercise responsibly, so M22 PR4a adds the contact→owner read before the reporter surface that depends on it.
- 2026-08-20 — disclosing an owner's legal name to their linked contacts is proportionate because of three MECHANISMS, not three assurances — the owner minted the code under step-up and handed it over out of band (you cannot appear in someone's contacts by asking), the owner ends it with one ungated click (the protective action stays the easier one, applied here to a read), and the name is ALL that is disclosed: not the address, DOB, SSN, assets, or the other people named.
- 2026-08-20 — the audit record precedes the plaintext on any read that IS a disclosure — `estatesNaming` emits before the decrypt loop, because an event written afterwards is one a crash can lose while the plaintext already exists; asserted as an ORDER in the test, since both orderings return identical data and only one is correct.
- 2026-08-20 — a disclosure event carries the READER and a COUNT, never the subjects — naming the owners in `contact.link.estates_read` would write the very relationship whose disclosure it records into the trail that exists to police it; the per-row `crypto.field.decrypted` events carry the owner as DEK subject, so the join is available to an investigator with authority for both halves and to nobody with only one.
- 2026-08-20 — "a stolen code must not become a read" was half-true after PR4a, so the sentence says less now — redemption still discloses nothing (worth keeping: it stops redemption being an oracle), but the LINK it creates is enumerable on the next request. Accepted rather than overlooked: a stolen code already confers the far heavier capability of opening a death case (docs/03 §6b), so what stands between a thief and harm is unchanged — the owner is NOTIFIED on claim and unlinking is one click. The control is revocation, not concealment.
- 2026-08-20 — the linked-estates panel offers NO control, deliberately — a contact who could remove themselves from an estate would be editing someone else's plan silently, at the moment it matters least to them and most to the owner; if the relationship should end it ends from the owner's side, where they can see what they are changing.
- 2026-08-20 — a null owner name is rendered in words, never as "Unknown" — the null means that owner has never saved a profile, and a placeholder invented at the BFF or in the UI is a layer with no key asserting what the key holder declined to say.
- 2026-08-20 — the mandatory human review had no evidence in front of it: `CaseDto` has carried `evidence` since M7 and the operator console's `parseCase` dropped the field, so docs/03 §5.1's control 2 was exercised against a status, a timeline and two opaque ids. Nothing was red because the defect was an ABSENT read rather than a wrong one — the shape of gap no test can report, since there is no assertion to fail. M22 PR4b adds the panel, and pins `parseEvidence` against `EvidenceEntry` in BOTH directions: the other four shapes in that fence are subset checks because a client may legitimately ignore a DTO field, and evidence is the one field this console exists to put in front of a human.
- 2026-08-20 — the console's evidence view is FLAT rather than a mirror of settlement's discriminated union, because both union behaviours are wrong on a review screen — failing the row removes a death case from a worklist over an evidence kind added last week, and skipping an unparseable entry tells a reviewer there is one piece of evidence when there are two. The type token renders as itself when unrecognised, and `addedBy`/`addedAt` are required because every arm carries them, so an entry this build predates still reports that it exists, who attached it and when.
- 2026-08-20 — the console says an entry came from "the account that reported this case" and never that it came from an operator: reporter-versus-operator is the distinction a reviewer wants, and this origin CANNOT honestly make it — it is role-blind by construction, `settlement_operators` being resolved inside the transaction that acts. The narrower claim that is exactly true is the one rendered; the alternative is a trust label minted by the surface that has no way to check it.
- 2026-08-20 — the evidence panel shows what was attached and never its contents, and says so on screen — a document opens through the documents service under its own audited read, and pulling estate documents into the lowest-trust origin in the product is a decision with its own audit volume, not a detail of a panel. A reviewer told nothing would be left wondering whether they are looking at everything.
- 2026-08-20 — a reporter could attach `{type:'provider_match'}` with a matchId of their own invention, landing it in `verification_evidence` beside entries operators file from real provider signals and indistinguishable to the human reviewing them. The reporter-facing SOURCE enum had excluded `data_provider` since M7 for exactly this reason; the evidence union was the same claim through another door, and it took building the panel where the two appear side by side to see it. A rule applied to one member of a category is a rule half-applied.
- 2026-08-20 — that one rule is enforced at two layers because only one route serves both capacities: `ReportCaseSchema` narrows to documents STATICALLY (its authority is the linked-contact check, and `report()` already states that a reporter who is also an operator "acted as a contact here"), while `addEvidence` decides DYNAMICALLY on the `isOperator` it already measures. Narrowing the shared `EvidenceInputSchema` instead would have broken the operators the type exists for.
- 2026-08-20 — `document_evidence_only` is checked BEFORE the status check and named for the rule rather than for `provider_match`: the two refusals have opposite remedies and only one can ever change — this one is permanent and about the caller, `invalid_transition` is transient and about the row — and answering the transient one first sends a reporter back later for something that will never work. Written as an allowlist, so a third evidence type is refused to non-operators until somebody decides it is reporter-safe.
- 2026-08-20 — reporter intake is NOT step-up gated and the argument is affirmative, not an omission: filing ADDS scrutiny rather than authority — the case locks nothing, the owner is notified on every channel, and they void it with one ungated click — so a gate would fall on a grieving contact on a borrowed device while stopping nothing a token thief wants. What replaces it is a REVIEW STEP, which is not a credential and refuses nobody: one click, existing because the consequence is legible to us and not to the person clicking. Somebody who is alive gets told, on every channel, that they were reported dead.
- 2026-08-20 — the reporting mutation names an estate by CONTACT ID, never `decedentUserId`, and the BFF resolves it by re-reading settlement's own reportable list. Two things at once: PR3's rule that no raw user id reaches the browser survives on the one surface that had a reason to break it, and the mutation checks entitlement against the service instead of trusting an argument. A contact id not on the list answers the same uniform 404 settlement would, without the request ever reaching it.
- 2026-08-20 — `reportDeath` takes no `source`: a report carrying a death certificate IS `death_certificate_upload` and one without IS `trusted_contact`, and the service already enforces that implication by refusing the former with no document. Deriving it at the BFF deletes a parameter that could only ever be wrong — prefer an absence to a filter.
- 2026-08-20 — the reporter's picker takes its SET from settlement and its NAMES from profile, and the direction matters: settlement re-checks the link inside `report()`'s transaction, so it is the list that predicts what the server will accept, while profile is the only service holding the owner's name. The join is a LEFT join — an estate with no profile row keeps its place and loses only its name, because dropping it would hide an estate this caller may genuinely report on behind somebody else's unfilled profile.
- 2026-08-20 — settlement spends one `invalid_transition` on the kill switch and on the evidence attach with opposite remedies, so the BFF's `mapError` takes the calling route's own meaning for it, and a route that declares none falls through to the generic status error rather than inheriting whichever sentence was written first. Fail closed: a route added without thinking about this says something vague, never something confidently wrong. Same reasoning splits `CASE_ALREADY_REPORTED` from `CASE_OPEN` — one word apart, opposite audiences.
- 2026-08-20 — the linked-estates panel's "offers no control at all" narrows to "no control over the LINK" (M22 PR4c). The reason PR4a gave was always about a contact silently editing someone else's plan; reporting a death edits nothing and grants nothing. It is the only place in the product that names these estates, so it is the only honest place to act on one — and the action LEAVES: a link to a page with a review step, never a button that files from a list of names.
- 2026-08-20 — no nav entry for `/people/report`, and the reason differs from `/security/cases`'s. That page is unlisted because it is empty for virtually everybody and a standing "Death cases" item is a memento mori for ten million people; this one is unlisted because a permanent "Report a death" entry would advertise the PERMISSIVE action more prominently than any protective one, which is the inversion this milestone exists to prevent. No id in the URL either: a link naming whose death you were about to report would sit in history and in any `Referer` leaving the origin.
- 2026-08-20 — sealed documents are never offered as evidence. A Zone A document is opaque ciphertext the server cannot read and no reviewer ever could, so attaching one is evidence guaranteed to tell nobody anything — an action that succeeds and accomplishes nothing, which is worse than one that is refused.
- 2026-08-20 — M21 PR4d fixed the pre-authority refusal ladder on `setDistributionStatus` and its comment asserted "every sibling verb gates first". The sentence was false when written — `requestStage`, `completeTask` and `recordDistribution` all looked the row up first — so the defect survived two milestones behind prose claiming it did not. A rule applied to one member of a category is a rule half-applied, and a comment that justifies an omission by asserting a fact about the rest of the tree is a test nobody runs. The claim is deleted; the mechanism is `administrableCaseFor`, which all three now call.
- 2026-08-20 — `administrableCaseFor` takes a NULLABLE case id so `completeTask` can pass `locked?.case_id ?? null`: a missing task and a task on somebody else's estate must leave through the same line, and an early `if (locked === null) throw` above the authority test is exactly the kind of harmless-looking return that answers before anyone has been authorised.
- 2026-08-20 — `case_not_verified` deliberately still reaches the estate's OWN executor. Fail closed means de-escalate, not withhold from the person entitled to the answer: their remedy differs from a 404's and only they can be told. Not `assertCaseVisible`, which admits the decedent and the reporter as well — neither may administer an estate.
- 2026-08-20 — `requireAdministrableCase` survives for the three operator paths that pass `OperatorGate.assertIn` first, with its precondition written into its docstring rather than assumed. This helper looks safe and is not; that is how all three executor verbs came to leak. Forcing six callers through one shape would have made the operator paths pay for a check they had already passed.
- 2026-08-20 — `recordDistribution` seals the amount outside the transaction (a KMS round trip does not belong inside an open one) and used to do it before authorising, so a stranger holding a case id could spend a KMS operation on the DECEDENT's own DEK with a plaintext of their choosing. Authority now precedes the seal, and the in-transaction check stays: a pre-transaction read and the transaction it guards are separated by every commit that lands between them, so the gap needs a test that revokes authority inside it — reverting the locked check alone left every existing test green.
- 2026-08-20 — the executor's worklist is a THIRD settlement listing, not a widened `listMyCases`. That query is `decedent_user_id = $1 OR reported_by = $1` and the web renders it as two panels split on `aboutMe`; a third OR arm would have put somebody else's estate into both, under a heading saying it is yours. Same reasoning M21 PR3b gave for splitting `administrable` off `queue`. It filters on `ADMINISTRABLE_STATUSES`, the constant the operator's worklist already uses, so a ninth case status cannot reach one list without reaching the other.
- 2026-08-20 — a pre-verification case is invisible to a designated executor. Until an operator verifies the death there is nothing to administer, and listing it would announce a death report about somebody who may well be alive — a disclosure that belongs to the reporter and the notification channels, not to a worklist.
- 2026-08-20 — the executor worklist emits NO audit event. Reusing `settlement.queue.viewed` would have stamped `actorType: 'operator'` on an executor's read, and a false actor type on an append-only trail is worse than no event; a new `AUDIT_ACTIONS` member costs a consumer deployment ahead of its producer and buys a record that somebody read their own worklist. The reads that disclose something already audit themselves where they happen. **SUPERSEDED 2026-08-27 (M48 PR2):** `worklistViewed` derives `actorType` instead of asserting it, so the reuse states nothing false and needs no new member — both stated reasons are spent, and an omission whose only reason has been removed is not still argued. `executorCases` emits `worklist: 'executor'` with `actorType: 'user'`. The third sentence was also wrong on its own terms: this worklist lists estates belonging to OTHER PEOPLE, not the caller's own, which is exactly the read `asset.estate.viewed` has recorded on the assets side since M7 PR2.
- 2026-08-20 — `estateInventory` resolves its case id to a `decedentUserId` against settlement's own list, and `estateStages` deliberately does NOT. The first has to: assets identifies an estate by user id and no user id may reach the browser, so the resolve is both the translation and the authority check. The second must not: settlement's `assertCaseVisible` admits the decedent, the reporter, the executor and an operator, and a resolve-first check at the BFF would refuse three of the four on a route built for all of them.
- 2026-08-20 — the executor surface offers exactly ONE requestable rung — the first stage with no live request whose predecessors are all approved. That is `stage_exists` and `assertPredecessorApproved` restated at the surface to decide whether to OFFER, never to decide authority, which settlement re-decides under a row lock. A revoked stage re-locks the inventory it opened: access is a grant, not a fact, and a screen that cached "approved" would be the one place in the product that disagrees with the authorization system.
- 2026-08-20 — the settling-estates panel renders nothing at all when the read FAILS, which is the one deliberate exception to "a failed read is not an empty one". It sits on the overview page, so the alternative is an error card about death cases on the first screen after sign-in for ten million people who are settling nothing. An executor who sees no panel loses nothing they can act on; every other reader would be shown a failure about a surface that does not concern them.
- 2026-08-20 — `STAGE_ALREADY_REQUESTED` is its own error code even though the UI never offers the action that earns it. Left unmapped, `stage_exists` fell through to the generic status error, so a race rendered "we are already reviewing this" as "something went wrong on our side" — a control firing wearing the face of an outage, which is the one substitution this repo's error rules forbid outright.
- 2026-08-20 — the executor's stage request raises `StepUpPrompt` rather than rendering `STEPUP_REQUIRED` as copy. Found by driving the app: the screen said "this action needs a fresh identity check" and offered nothing to click, on the one action the surface exists for. No unit test could see it — the fixtures decide what the session is, so the suite was green about a screen that stranded every real user. The prompt REPLACES the request button (two live paths to one action let somebody answer the prompt while a second request is in flight) and the retried action carries the stage it was refused with, per the frontend rule that a retried action never re-reads its arguments from current state.
- 2026-08-20 — an integration spec that UPDATEs a versioned row must set `search_path` to its scratch schema. The `<table>_capture_version` triggers insert into `<table>_versions` UNQUALIFIED, so the name resolves at execution time against the caller's path; a local `PG_TEST_URL` pointed at the running stack's `core` database finds those tables in `public` and passes, while CI's clean database fails. Confirmed by running the same code against both. The rule that follows: a green int run against a database that already holds the product's tables is not evidence about a clean one — re-run against a fresh database before believing it.
- 2026-08-20 — the estate checklist sits behind NO access stage, unlike everything else on the executor screen. A task is procedural state about the administration — "obtain certified copies of the death certificate" — not access to anything the decedent kept, so staging it would put an operator's review in front of an executor seeing their own worklist. It follows that on day one, every rung shut, the checklist is the only part of that screen that works, which is why it renders above the inventory: a screen that opens with a locked panel opens with a refusal.
- 2026-08-20 — ticking a checklist item is not step-up gated and the UNTICK is the same route with a boolean. Everything on this surface that MOVES access or money is gated; a tick moves neither. Correcting a mis-click is the protective action here, so it must not be harder than making one — and an executor days after a death should not have to find their authenticator to record that they have found the will.
- 2026-08-20 — settlement's step-up fence walked `OPERATOR_ROUTES` and stopped, leaving every EXECUTOR route on the same controller undeclared: `requestStage` could have lost its guard silently. The added block derives the executor set as "every admin-controller route the console cannot reach", asserts the halves are disjoint, and pairs the three gated writes with `completeTask` as a positive control that must stay UNGATED — a predicate that had started answering true for everything would otherwise satisfy the whole block. A rule applied to one member of a category is a rule half-applied.
- 2026-08-20 — a BFF test asserting "the tick is not step-up gated" was measuring nothing: the BFF evaluates no step-up anywhere, it forwards a bearer and maps a downstream 403. The claim provable at that layer is that the resolvers invent no freshness of their own, and the decorator claim belongs against the real metadata in the service. When a guard exists at two layers, a test must say which layer it proves — and a test that passes for a reason unrelated to its name is worse than no test.
- 2026-08-20 — a due date is a calendar DAY and gets its own formatter (`formatCalendarDate`), not the instant formatter beside it. `settlement_tasks.due_at` is a Postgres `date` that the service widens to UTC midnight on the way out; rendering that in the reader's zone loses a day for everyone west of UTC, and the screen said "September 2" for a date stored as the 3rd. Found by driving the app at `America/Phoenix` — a suite running in UTC cannot see this defect at all, which is why the test forces the zone. `formatDate` was deliberately NOT changed to match: `verifiedAt` and `waitingPeriodEnds` are genuine instants and a reader in Phoenix should see them in Phoenix time.
- 2026-08-20 — `completeTask` emits `settlement.task.completed` on the tick and NOTHING on the untick, and M23 PR3 is the change that makes unticking reachable at all. Left as it is, with the reason stated rather than silently: the reversal is not lost — `settlement_tasks_versions` is trigger-maintained and append-only and records both transitions with the actor, confirmed in the browser drive (two UPDATE rows, `completed_at` set then null). A `settlement.task.reopened` member would be the more legible record, but `AUDIT_ACTIONS` is a closed vocabulary and a new member costs an audit-consumer deployment ahead of its producer — which does not belong in the same slice as a UI. Filed as the first candidate for M23 PR4's audit work.
- 2026-08-20 — `process.env.TZ = …` inside a jest jsdom test is INERT, so a timezone-dependent test must pin the zone in `globalSetup` (which runs before the workers fork) and then ASSERT the zone it got. The web suite's first calendar-date tests "forced" Phoenix in a `beforeAll`, changed nothing, and passed locally only because the machine already sat in Phoenix — CI, in UTC, caught the one assertion where the two zones disagree. Probed rather than assumed: plain Node honours the assignment and jsdom does not. The suite is pinned to a fixed NON-UTC zone deliberately, because in UTC a calendar day and a moment render identically and no such defect is visible at all.
- 2026-08-20 — an executor's read of the estate's contacts is a SEPARATE route (`GET /v1/estates/:ownerUserId/contacts`) rather than a branch inside profile's cross-owner ABAC read, mirroring the split assets made for the estate inventory. Three reasons, and the first is decisive: the authority is a different KIND. `listForOwner` resolves `permission_grants` — things the owner handed out while living, which control 4 freezes at a death report — while an executor holds a `role_assignment` that only becomes real on verification. Widening the grant query to admit them would have thawed every `on_death_verified` grant for profile's only grant reader and needed a row in `ENFORCED_GRANTS` asserting the owner conferred something they never did. The other two: the decision is settlement's, not profile's, and it audits differently (`contact.estate.viewed`).
- 2026-08-20 — the estate's contacts open on the DOCUMENTS rung, not INVENTORY. Who the decedent named is disclosure about LIVING THIRD PARTIES rather than about the estate's holdings, and the people named as beneficiaries are named in the estate's documents; the ladder's own order agrees that inventory is the least dangerous rung and this is not it. The knock-on is deliberate: recording a distribution names a beneficiary contact, so distributions will in practice need DOCUMENTS approved — which matches the checklist template's own ordering, letters testamentary before distributions.
- 2026-08-20 — `estateContacts` is the ONE route on the BFF's profile client whose 403 is not narrowed to the uniform not-found. Every other route there is about the caller's own data, where "not yours" and "no such row" must stay indistinguishable or an id becomes a probe; this one is about an estate the caller was HANDED, reached from a list of the estates they are settling, so collapsing the refusal would tell an executor that a case in front of them does not exist. `STAGE_NOT_APPROVED` is its own code, and a route that declares no `meaning` still gets the collapsed answer — fail closed, so a route added without thinking about this says something uniform rather than something confidently wrong.
- 2026-08-20 — the M23 PR2 exemption reason for profile's two cross-owner contact reads asserted they "resolve through the same settlement staged grant the inventory does... behind the DOCUMENTS rung". That was false when written: `effectiveContactReadGrants` filters `effective_condition = 'immediate'`, so no ladder had any bearing on those routes at all. The reason now says what they are — the §5.5 ABAC demonstrator, grant-resolved, unconsumed — and the executor has a route of his own. A fence's declared REASON is prose beside a mechanism, and prose is where this repo's false claims keep surviving.
- 2026-08-20 — the closed-vocabulary audit rule fired in a real browser drive, which is the first time it has been observed rather than cited. `contact.estate.viewed` was emitted by a rebuilt profile into an audit consumer that predated the action; the consumer logged `audit_event_rejected reason=schema_violation rejectedTotal=1` and dropped the disclosure event silently, so the trail showed nothing while the read succeeded. Rebuilding the consumer first and re-driving produced the full record. The lesson stands as written in `.claude/rules/services-backend.md`: an expected-but-missing event should send you to the consumer's log before the producer's code.
- 2026-08-21 — a distribution amount is READABLE, through a route of its own that spends one audited decrypt per row. It was write-only from M7 PR2 until now, which made `settlement.distribution.approved` — dual-control and step-up gated, and named in docs/03 as a control — an approval of a figure the approver could not see. `assertCaseVisible` gates it rather than the narrower executor test, because it reveals a field of a row `listDistributions` already returns and a narrower gate would leave the approving operator unable to look; the test asserts the two admitted SETS are equal rather than listing the four parties. It is NOT step-up gated: `listDistributions` already says, ungated, that the row exists and carries an amount, and a factor on the last step of a disclosure whose earlier steps are free makes checking harder than approving.
- 2026-08-21 — a crypto-shredded estate answers `content_erased` (410) on the amount route, the third of three distinct facts: `{amount: null}` means no sum was recorded (normal — a distribution may name an asset), a uniform 404 means no such row or not yours, and 410 means the key was destroyed under a legal erasure and the value is gone for everyone, permanently. The arm exists because a FAITHFUL DOUBLE forced it: `FakeFieldCrypto` was extended to refuse a DEK it never minted with the real `DekDestroyedError`, and the service had no handler. It is the spelling `DocumentsService` already uses — one behaviour, one spelling — and on the operator console a 410 had been falling through to UNKNOWN, inviting an operator to retry forever for a key destroyed on purpose.
- 2026-08-21 — GraphQL offers only the three distribution status moves an executor may make. `PLANNED` is a default rather than a move and `APPROVED` is the operator's act under dual control, so neither is in `EstateDistributionStatusChange` and the browser has no way to spell an action the server would refuse — absence over filter, applied to a vocabulary. The fence does not check the three members exist; it accounts for the DDL CHECK constraint's WHOLE vocabulary as executor-reachable or not, so a sixth status added to the database turns it red rather than passing unnoticed.
- 2026-08-21 — the web's distribution transition table is DERIVED from `AdminService.setDistributionStatus`'s own `from` expression and inverted, not hand-written beside it. The failure it prevents is concrete and quiet: a button that answers `DISTRIBUTION_NOT_APPROVED` every time it is pressed. Its first draft went green on a broken parse — the block capture stops before the `;`, so a `\];` anchor matched nothing and produced `[undefined]`, which cleared a non-empty-array floor and then vanished from the inversion. The floor now asserts every parsed predecessor is a real non-empty string, which is the anti-vacuity lesson restated: a length check is not a content check.
- 2026-08-21 — `pathSharedWith` is retired from `route-consumers.spec.ts` with its last instance, on the precedent the mechanism's own test stated in advance. It declared a route with no consumer of its own whose PATH was addressed by a sibling of another method, because the fence matches by path and cannot see a method; M21 PR3b built it, M23 PR2 and PR4b flipped both instances to `consumed`. A declaration form with no instances is a shape the next person reaches for without a reason to, so the form, its two tests and the `EDGE_ROWS` alias only they read were removed together. `EXEMPT_EXECUTOR_CASEWORK` went the same day on the instruction it carried in its own text.
- 2026-08-21 — the end-to-end money fixture is `999999999999999.99`, the widest value `AmountSchema` accepts, because `String(Number(x))` is `'1000000000000000'` — a cent light and rounded to a figure nobody recorded. Asserted at the service, at the BFF client (which additionally REFUSES an amount that arrived as a JSON number rather than carrying it on), and in the browser. An earlier draft used a 17-digit value the route's own regex would have rejected, which is a fixture testing the fixture.
- 2026-08-21 — M25 (crypto-shredding) is sequenced ahead of M24 (dashboard), and M24's written flip-trigger was CHECKED rather than assumed: no demo date, no signed customer, so it has not fired and M24's queue position was a proposal. Argued on the 2026-08-17 selection's own three lenses. Dependency is decisive — M25 is a chokepoint with named dependents (M32, M33 are both routed through it) while M24 has none, and M24 is the first milestone in this queue with NO unconsumed backend routes waiting for it, so the fastest-compounding-debt argument that put settlement in all three judges' top third does not apply to it. Risk gained exactly one real input since the selection: M23 PR4b made the product claim `content_erased` to a user in five places while nothing can produce it. The counter-argument is recorded rather than dropped — M24's shared client read cache is a genuine dependency claim, and it is small and does not need the dashboard to justify it.
- 2026-08-21 — erasure is OWNER-INITIATED ONLY and step-up gated, with no privileged database role in M25. Three consequences are recorded as dispositions in docs/03 §6kk rather than left implicit: no operator-executed erasure, no privileged role, and — the one that is easy to miss — NO PATH ERASES A DECEDENT'S ESTATE, ever, since the requester must be alive and signed in and an account at `settlement` can request nothing. The refusal set follows from the decision (`deceased_pending`, `settlement`, `closed`, any document under `legal_hold`), each refusal naming its own remedy so a living owner wrongly reported dead is told to void the case rather than shown something that reads as an outage. Cancel is one click and ungated: the protective action must never be harder than the permissive one, which here inverts the usual shape because the permissive action is the destructive one.
- 2026-08-21 — docs/02 §conventions and `packages/crypto/src/dek.ts` had both described crypto-shredding as performed by "a privileged retention job (not the app role)" since M1, and there has never been such a role — the repo has no `CREATE ROLE` or `GRANT` outside test files. Both are corrected to report the tree. What replaces the boundary is a declared caller allowlist for `destroyDek` in `packages/contracts/test/erasure-domains.spec.ts`, and the asymmetry is stated in all three places rather than glossed: a role stops a compromised app process from destroying keys at RUNTIME, an allowlist stops a second caller arriving in REVIEW. Claiming the fence closes the residual would have been the more comfortable sentence and the false one.
- 2026-08-21 — the erasure participant set is derived TWICE and compared as sets: `topology.ts` entries with a non-null `kekAlias`, and services whose migrations create a table whose name ends in `deks`. The two are independent because the eight services do not agree on a table name (`deks` in three, `settlement_deks`, `document_deks`, `notification_deks`, `plaid_deks`, `assistant_deks` in the rest) — a fence keyed on the obvious spelling would have found three of eight and gone green. This is what makes docs/04's long-standing "must precede any new encrypted data class" a mechanism instead of a sentence.
- 2026-08-21 — an assertion about an ABSENCE needs a positive control pointed at the same walker, proved rather than argued. The `destroyDek` caller allowlist asserts an empty set; renaming the identifier its AST walker matches left that assertion GREEN and turned only the control red. The control points the walker at the one call site that does exist (`packages/crypto/test/envelope.spec.ts`) and demands it be found, so a typo disarms the fence loudly instead of silently.
- 2026-08-21 — erasure would have immortalised the identifier it erases, which is why M25 PR1 (the version-capture redaction) is sequenced before PR3 (the destroy leg). `users.email_bidx` is an HMAC under a service-wide key rather than a `*_ct` + `dek_id` pair, so the shred does not reach it; `users_capture_version()` is `to_jsonb(OLD) - 'password_hash'`, so every `users` UPDATE writes the full row image into `users_versions` (REVOKE UPDATE, DELETE); and erasure must UPDATE `users` to set `status = 'closed'`. Nulling the live column does not help — the trigger captures `OLD`. Migration 008's comment calls `password_hash` "the ONE column in `users`" the shred does not reach and there are two, but 008 is applied and checksummed and is NOT edited: the correction lives in docs/03 §6kk and in PR1's own migration. Severity stated honestly — an insider with the blind-index key and direct cluster read, so erasure completeness rather than a confidentiality break.
- 2026-08-21 — the blind-index capture defect is a CATEGORY of three, not the one column M25 PR0 recorded. Derived from the schema rather than remembered: every table carrying both a `*_bidx` and a `<table>_versions` shadow — `users` (identity), `contacts` (profile), `plaid_items` (plaid). The member PR0 missed is the sharper one: `contacts.email_bidx` indexes a LIVING THIRD PARTY's address rather than the account holder's, so erasing an account would have left a searchable index of the people that account knew. The severity ordering is the reverse of the discovery ordering, which is the whole argument for deriving a category instead of fixing the instance in front of you.
- 2026-08-21 — the version-capture redaction is DERIVED FROM THE ROW (`jsonb_object_keys(image)`, drop every key ending in `_bidx`) rather than naming a column. A function subtracting `'email_bidx'` literally would pass any test written today and leave the NEXT blind index on that table captured — so the fence asserts the mechanism reads the image's own keys AND refuses a hand-named key, which is what stops the weaker fix landing with the stronger one's face on it.
- 2026-08-21 — the rule is proved in two halves and each names which half it is. The static fence reads the migrations and proves a redaction was WRITTEN; `blindIndexCaptureGaps` asks the running database what function it is executing (`pg_get_functiondef`, corpus derived from `information_schema`) and proves it was APPLIED. Neither is sufficient: a migration written and never applied passes the first, and an empty schema passes the second — which is why the corpus size is asserted before the gap list is believed. The check lives in `@estate/db` with three thin call sites, on the `@estate/config/ci-guard` precedent, because eleven packages once carried a drifted copy of that rule while nothing tested the assertion anywhere.
- 2026-08-21 — `versionsTableSql` is a TEMPLATE with no runtime caller: its output was copied into `.sql` files once, and the migrations are the deployed truth. Updating it fixes nothing already deployed, so the three migrations do the work and both fences read the MIGRATIONS rather than the helper. It is updated anyway so the next table copied from it is born correct. Recorded because "the helper is fixed" is the tempting and wrong summary of this change.
- 2026-08-21 — `document_versions.content_sha256` is KEPT, answering the question M25 PR0 left to PR1. It is a plaintext hash in an append-only table and the shred does not reach it, so someone holding a candidate document can confirm an erased estate held those exact bytes. Kept because it is the disaster-recovery integrity check (decrypt-then-hash, rebuild-and-diff), because the table is append-only so no-hard-deletes forbids removing rows, and because the attack needs the exact plaintext — a far smaller class than an email HMAC, whose candidate space is guessable. What would change the answer is a rebuild check that needs no plaintext hash.
- 2026-08-21 — the retention job has been described in FOUR places, not the two M25 PR0 counted and corrected. The third is `documents.service.ts`'s `softDelete` docstring, corrected in PR1; the fourth is in `documents/migrations/002_document_vault.sql`, which is applied and checksummed and cannot be edited, so it is recorded in docs/03 §6ll instead. A PR that corrects a category and states its size is making a claim about the tree, and PR0's was one short — the same defect it was written about, one PR later.
- 2026-08-21 — M25 PR2 ships the DECISION half of erasure and NOT the driver its plan named. Measuring before building showed the driver has nothing to drive: with no waiting period in scope a request is eligible the moment it is made, and per-domain progress rows only mean something once a fan-out writes them, so an inert driver would be dead code that reads as design. The driver moves to PR3 with the work it exists to run. Recorded rather than silently absorbed, on the M21 PR2 precedent.
- 2026-08-21 — erasure's status allowlist is `['active']` and rides INSIDE the INSERT's own `WHERE EXISTS`, never a read above the write. Deny by default on the most irreversible verb in the product, and the four statuses that cannot reach the code with a live session (`findLiveByAccessHash` admits only `active` and `deceased_pending`) are refused anyway because the reachable set is not a constant. Only `deceased_pending` is a refusal a real person hits, which is why it gets its own token and a remedy — `open_death_report`, meaning sign in and void the case — while everything else gets the generic one. A control firing must not read as an outage.
- 2026-08-21 — the erasure CANCEL is ungated and carries no status allowlist, inverting the usual shape of the protective/permissive rule because here the permissive action is the destructive one. Requesting needs a fresh step-up; withdrawing needs only the session. And an account that became ineligible to REQUEST erasure while a request was live must still be able to withdraw it — refusing the withdrawal because the account is now locked would strand the most dangerous record in the system in its armed state. Fail closed means de-escalate, not refuse everything.
- 2026-08-21 — a mutation that reddens EVERY assertion is evidence about the mutation, not the suite. Deleting the allowlist predicate from the erasure INSERT turned all nine cases red because it was a SQL type error rather than a removed control; the faithful version widens `ERASURE_PERMITTED_STATUSES` and reddens exactly three, leaving six green. Discarding the first result rather than banking it is the step that is easy to skip, and "every mutation went red" is equally consistent with a suite that fails on any edit.
- 2026-08-21 — legal hold is NOT checked when erasure is requested, deliberately. It is a per-document flag in the documents cluster with no account-level equivalent, and a hold placed after the request would make any answer given at request time stale — so the check belongs inside the statement that acts, in PR3. Recorded because "erasure was requested" reads to a reviewer as "erasure was permitted", and those are not the same claim.
- 2026-08-21 — identity's coverage floor is calibrated for the run with NO DATABASE, and M25 PR2 shipped ~200 lines whose only tests were `describeIfPg`. CI's "Test identity with NO database" step failed on exactly that: to that gate, service logic reachable only through a Postgres-gated suite is untested code. The local verification that missed it ran `pnpm test -- --coverage` WITH `PG_TEST_URL` set, which is the one configuration that cannot see the defect — a green run against the wrong environment, which is the "a stale artifact is not evidence" rule pointed at an env var. The fix is the split the repo already implies: what lives in SQL (the allowlist inside the INSERT, the partial index, the capture trigger) stays in the int suite; what lives in TypeScript (which refusal token a status maps to, whether a zero-row insert means refused or already-requested, whether an audit event fires) is unit-tested with a double and runs everywhere. Verify identity BOTH ways before believing a green.
- 2026-08-21 — the erasure guard asymmetry is pinned by an assertion rather than a comment, because it is the kind of thing a reviewer "fixes" by mistake. Adding `StepUpGuard` to the cancel route reads as tightening a control and is the defect: an owner whose session was briefly taken could arm the most irreversible process in the product and then be required to prove themselves before disarming it. The test anchors on `__guards__`, the metadata Nest actually reads when building the pipeline, not on source text — a grep would scan what somebody wrote rather than what the framework will execute.
- 2026-08-21 — M25 PR3 ships the destroy leg for IDENTITY'S DOMAIN ONLY, and the other seven participants get a ledger row rather than a promise. The fan-out is not a matter of writing seven more calls: `estate.auth.events.v1` has a producer and no consumer, and the audit service is the only Kafka consumer in the repo, so reaching profile/assets/plaid/documents/settlement/notifications/ai-assistant means building the first inter-service consumer in each of them. Splitting there rather than shipping a half-built fan-out keeps the irreversible half reviewable on its own. What makes it honest is `erasure_domain_progress`: a row per participant on every request, seven of them sitting at `pending`, so "one domain of eight" is a query and not a sentence in a design doc. `completed` keeps meaning EVERY domain and is simply not reachable in M25 — a terminal state redefined as "as much as this build can erase" would change meaning at the next deploy, which is the worse failure.
- 2026-08-21 — the erasure grace period is SEVEN DAYS, reversing docs/03 §6mm's `[ACCEPTED]` "no waiting period" tag. The tag was recorded when the driver did not exist; building it showed that without a window the driver executes the instant a request is made, which makes PR2's deliberately ungated one-click cancel a button nobody can press in time. That is the protective action being harder than the permissive one, arrived at by omission rather than by decision. The period travels inside the claim statement's own `WHERE`, so it cannot be shortened by a driver that ticks twice, never ticks, or runs in two processes. Recorded as a reversal rather than a new fact because an ACCEPTED tag is supposed to mean nobody owes work, and this one was wrong.
- 2026-08-21 — the destroy leg closes the account and revokes its sessions BEFORE it destroys the DEK, which inverts "the step that cannot be undone runs last" under the exception that rule already carries: here the reversible step is the one that strands state. `FieldCrypto.getOrCreateDek` mints a key for any user with no active one — correct for every other caller and catastrophic for this one. Destroy first, and a session surviving the shred gives the account a brand-new DEK: the row is live again, everything written after it is readable, and the audit trail records a successful erasure. The property is asserted on a CALL LOG in the no-DB suite rather than in the int suite, because a database cannot observe sequence after the fact — reversing the two blocks reddens four named assertions, and every decision test stays green.
- 2026-08-21 — the live `users.email_bidx` is replaced with a real blind index of `<uuid>@erased.invalid`, never with random bytes. `emailBlindIndex` fixes the width, the HMAC key and the purpose label so none can drift from the live ones — a replacement of the wrong shape would make every erased row identifiable by its column alone, a worse leak than the lookup being removed — and `.invalid` is reserved by RFC 2606, so the value can never collide with a real address under `ux_users_email`. This is the live half of what PR1 did for the capture half, and the two are one mechanism: under the pre-PR1 capture body this UPDATE would have copied the old index into `users_versions`, where `REVOKE UPDATE, DELETE` places it beyond every later migration. Erasure would have immortalised the value it was erasing.
- 2026-08-21 — a cancel now answers WHAT IS STILL LIVE instead of always `null`, because PR3 gave the verb a second way to not take effect. Only a `pending` request is cancellable; once the driver has claimed it, keys are being destroyed. "There was nothing to cancel" and "it is too late" need different things from the reader, and the repo's rule is that two failures with different remedies never share an answer — but a second ERROR token would make the protective verb failable, which the same rule set forbids. Returning the remaining live request is both: ungated, non-throwing, and incapable of telling an owner "withdrawn" about an erasure in progress.
- 2026-08-21 — a claim that cannot proceed is RELEASED to `pending`, not failed in place. Eligibility is restated inside the claim because the request may be days old, so an account that moved to `deceased_pending` or `settlement` during the grace period stops the erasure. Wedging it in `executing` would be doubly wrong: `executing` is uncancellable by design, and the live unique index would block a new request — the erasure feature locked shut for that account by a race, with no operator path to open it. On `pending` the owner can still cancel, the next tick retries, and an account that becomes eligible again self-heals. The cost is a request that may sit at `pending` indefinitely, which is tagged ACCEPTED in §6nn: settlement outranks erasure, and inventing a terminal `refused` state would invite somebody to stop retrying something that should resume.
- 2026-08-21 — `'closed'` gets its own login-refusal reason, `account_closed`, and the fix was applied to the whole category rather than the member that prompted it. `closed` had mapped to `account_settled` since M7, so an erased account would be recorded as a settled estate — two situations needing opposite responses, one a person signing in to something they destroyed and the other a possible decedent-credential replay worth investigating. The mapping became a table over every non-signable status and is asserted as a table, so the next status added to the `users` CHECK is a visible decision rather than a silent fall-through. The wire answer is unchanged: every branch throws the same uniform 401, and this decides only what is recorded.
- 2026-08-21 — `ErasureDriver` advances state on a timer and `SettlementWorkflowDriver` deliberately does not, and the difference is written into the class docstring rather than left for a reader to infer from the resemblance. Settlement's driver is powerless by design because a death claim is never fully automated and a human must confirm every step. Erasure's destroys a key, which is permitted because the human review has already happened: the account's OWNER asked, in a session that proved a fresh second factor, no third party can arm it, and no operator role exists that could. Settlement decides something ABOUT a person; erasure executes something FOR one. Losing the driver delays erasures and widens nothing — every safety predicate lives inside the statements it runs.
- 2026-08-21 — a mutation SURVIVED and the survivor was the test, not the code: narrowing the live unique index back to `status = 'pending'` left all fifty erasure tests green. The test named for the widening was reaching its branch by accident — by the time a second request can be made the destroy leg has already closed the account, so the status allowlist refuses the insert and the index is never consulted. The honest reading is the third of the three explanations: the widening is a SCHEMA invariant that no service path can currently violate. So the test was split — one named for the allowlist that actually fires, one that goes to the constraint directly with a positive control — and the mutation now reddens exactly one assertion. A test named for a property must exercise the boundary that property decides; this one passed for a reason that would have survived the property's deletion.
- 2026-08-21 — M25 PR4's erasure panel says what erasure DOES NOT reach, and that sentence is asserted by a test. Erasure covers one DEK domain of eight, so "your account has been erased" is a promise this platform cannot keep; the panel states what is destroyed now, states that documents, assets and other records are queued and not yet erased, and never claims a completion the ledger cannot show. The opposite failure is guarded too: copy that softened the irreversibility would be the kinder sentence and the crueller outcome, because the key is genuinely gone and no support path restores it. Copy is the only place a promise this size lives, and a test is the only thing that would catch its drift — which is why both halves are assertions and not review comments.
- 2026-08-21 — PR3's driver never returned to a request it had already claimed, and PR4 found it by asking what the surface could honestly promise. `claimDue` took only `pending` rows, so a process killed between the claim and the shred left a request in `executing` that nothing ever revisited: uncancellable by design, blocked from being re-requested by the live unique index, and holding an account that was promised destruction and did not get it. The per-step idempotence PR3 documented was real and unreachable — idempotent steps are worth nothing if nothing re-drives them. The claim grew a second arm for stalled requests, deliberately with NO grace period and NO status allowlist on that arm: the account is already `closed` by the leg that ran, so the allowlist would refuse every resume, and the waiting period was served before the first claim. Re-imposing either would strand exactly the request the arm exists to rescue.
- 2026-08-21 — the resume arm keys on the CALLER'S OWN domain, and the first draft that did not was an infinite loop. Asking "does this request have unfinished work" is permanently true while seven of eight domains have no transport, so the sweep re-claimed one row forever. The right question is "is MY work on this request unfinished", which goes false the moment this domain reports. A `worked` set now backs the predicate up and has its own named test, because the failure mode of getting this wrong again is a driver that never returns: no assertion fires, a hung sweep looks like a slow one, and a timeout names nothing. The backstop converts that into a value a test can read.
- 2026-08-21 — the two erasure refusals are kept apart across four hops, and the BFF maps them TOKEN-FIRST because both arrive as 409. A status-keyed rule could not tell `open_death_report` from `erasure_not_permitted`, and merging them at the edge would undo at the last hop exactly the split identity's service went to the trouble of making — one is a control firing with a remedy the owner can take, the other is a plain refusal. A 409 token the edge has NOT learned falls through to the shared mapper rather than being guessed at, so a future refusal surfaces as an unmapped failure instead of wearing the wrong remedy.
- 2026-08-21 — the erasure panel renders NO stop button once a request is executing, and the withdrawable set is an allowlist rather than a check for `executing`. A control that cannot work is worse than its absence: the person presses it and believes they stopped an erasure that is destroying keys. The allowlist matters because identity owns the status vocabulary and it grows — a build meeting `completed` and testing for `executing` would offer to cancel something already finished, whereas an allowlist falls through to "cannot be stopped from here". The same rule shapes the failed-read branch: neither the arm button nor a state is rendered when the panel could not ask, because "we could not ask" must not look like "nothing is scheduled".
- 2026-08-21 — two unfaithful mutations in one PR, both worth naming. Cutting the resume arm out of the SQL by hand produced malformed SQL and reddened every test in the suite, which proves nothing about the control; the faithful version disables it with a single `FALSE AND` and reddens exactly one named assertion. Pointing ONE erasure method at a wrong path left the route↔consumer fence green — that fence matches by PATH, not method, and says so in its own docstring, so the mutation tested a property it never claimed. Pointing all three at a wrong path reddens it and names all three routes. A survivor is only evidence once the mutation is faithful to what the thing under test actually asserts.
- 2026-08-21 — a `cp` restore after a mutation failed silently twice because an earlier `cd` in the same compound command had changed the working directory, leaving a mutated source file in the tree while the command reported nothing wrong. Restores from the scratchpad now use ABSOLUTE paths on both sides and are verified with `cmp -s` before moving on. This is the same class as the `git checkout <file>` lesson from M23 PR4b: the dangerous part is not the mistake, it is that the tree looks fine afterwards.
- 2026-08-21 — the §6p prediction was checked rather than inherited. M17's review filed "once erasure exists, a shredded account cannot reach a ceremony route at all" as a precondition on this milestone rather than a floating residual. It is true, and for TWO separate reasons the guess did not distinguish: session-guarded ceremonies are unreachable because a session resolves only for an 'active' or 'deceased_pending' account, while the UNAUTHENTICATED password-reset redeem holds a code that still names a user id — no session check protects it — and is refused instead by the status allowlist riding inside `updatePasswordHash`'s own UPDATE. When a guard exists at two layers a test must say which it proves, so the assertion names both and carries a positive control on a live account.
- 2026-08-21 — four `[OWNER: M25]` residuals had outlived their sentences and were swept in PR4. Two are closed by work that shipped; three are re-owned to M26 with a note recording who was named and why they could not deliver — `document_search_tokens`, `document_versions.content_sha256`, and the live-blind-index category beyond `users`, all in domains M25 never reaches. A residual whose named owner has shipped without doing it is worse than an untagged one: a tag reads as coverage, and the next reader stops looking.
- 2026-08-21 — the audit-consumer-before-producer rule was observed FAILING on the local stack during M25 PR4, which is the first time it has been seen rather than reasoned about. Identity, the BFF and the web app were rebuilt without the audit service; the first erasure request through the real UI was rejected `schema_violation` at the same millisecond it was made, so the event was produced and the trail stayed silently empty. `AUDIT_ACTIONS` is a closed vocabulary and an older consumer drops every instance it does not recognise — `auth.account.erasure_requested` and `auth.account.erasure_cancelled` are M25 PR2 additions, so any deployment that ships identity ahead of audit loses them. Rebuilding audit first and re-driving the ceremony produced both events with zero rejections. Nothing enforces this ordering, and the failure leaves no mark on the producer side: an expected-but-missing audit event should send you to the consumer's log before the producer's code.
- 2026-08-21 — driving the real app for M25 PR4 found what no test could: /security's own intro paragraph enumerates the protections on the page, and PR4 added a section it did not mention. The page would have described itself incompletely on the one action it holds that cannot be undone. This is the tenth consecutive milestone where driving the app surfaced something the suites could not see, and the class is consistent — copy, links and page-level self-description, none of which any component test asserts because none of them belong to a component.
- 2026-08-21 — M25 PR5's planned deliverable was a `grantStepUp` caller fence; the review that came with it found two live gaps, and both were already argued for somewhere in the tree. The fence is real work — `grantStepUp` is what widens `sessions.stepup_expires_at`, which stands between a taken session and vault open, document generation, erasure and every trustee change, so its caller set is pinned to the two FACTOR PROOFS by set equality, anchored on the injected type rather than a field name, with a separate assertion that the five redemption services (which authenticate somebody without a second factor) never call it. But the question that found the defects was the repo's own: what ELSE is in this category. Asked of "values the crypto-shred does not reach", the answer was two columns in identity's own cluster, not the zero the milestone had assumed.
- 2026-08-21 — `users.password_hash` is nulled by the erasure close, and the argument for it had been sitting in migration 008 since M17. That migration excluded the column from the `users` version shadow with exactly this reasoning: it is a plain Argon2id verifier outside the per-user envelope, so a row image surviving a crypto-shred must not contain it. The reasoning was applied to the shadow and never to the live column four lines away. Everything else in an erased `users` row is either destroyed with the DEK or is an opaque identifier; the verifier is neither — it is derived from a secret its owner may have reused, and a database dump could be worked on offline forever. A rule applied to one member of a category is a rule half-applied, and the two members here were in the same file.
- 2026-08-21 — `email_changes.new_email_bidx` is unlinked by the erasure destroy leg, correcting docs/03 §6ll, which had filed it under "belongs to the domains M25 does not reach". It does not: `email_changes` is identity's own table, in identity's own cluster, reachable by the leg PR3 had already shipped. The residual grouped it with `contacts.email_bidx` and `plaid_items` — genuinely other services' — BY COLUMN NAME, and never asked who owned the table. That is the transferable part: a residual that names the wrong owner reads as covered, and the next reader stops looking. Left alone, an erased account still answered "was this address ever staged here", which is the exact question PR3 went to the trouble of removing from `users`, in a table carrying `REVOKE DELETE` so its rows outlive everything. The replacement is a blind index of `<uuid>@erased.invalid`, same construction as `users.email_bidx` for the same width-and-shape reason. A COMPLETED change keeps `revoked_at` NULL: the table is evidence, and a completed change was not revoked.
- 2026-08-21 — the staged-address unlink runs OUTSIDE the `status !== 'closed'` block, which is the whole of its correctness. Hung off that guard it would be skipped on exactly the path PR4's resume arm exists to serve — a request claimed, closed, then interrupted before its domain was marked done — which is the identical shape of unreachable idempotence PR4 found in PR3's leg one milestone earlier. It also runs AFTER the ineligibility hand-back, so the "nothing has been destroyed, hand the claim back" promise above it stays true. The test proves the close really was skipped before asserting the unlink happened, and the mutation that moves the call inside the block reddens two named tests. Repeating the unlink is safe by construction: each run re-indexes to another address nobody holds, and `revoked_at` is COALESCEd rather than overwritten.
- 2026-08-21 — a step-up fence of my own writing had a surviving mutation, and the survivor was the fence. The window test asserted that `sessions.repo.ts` CONTAINED `STEPUP_WINDOW_MS` — which the surviving `import` line satisfies, so deleting the window from the assignment left it green. Re-anchored on the extracted assignment expression with an anti-vacuity count of how many assignments were found, it kills. A fence whose input is narrower than its claim goes green for the same reason it is wrong, and the fence that stated the rule was not exempt from it.
- 2026-08-21 — `devices.fingerprint_hash` gets no erasure leg because the table has no writer anywhere in identity: it is dormant, so there is nothing to erase. Recorded as a residual rather than a comment because the answer expires the day something writes to it, and no fence derives "tables holding an out-of-envelope digest" from the DDL — so nothing would notice. The out-of-envelope category is now CLOSED for identity's cluster (`users.email_bidx`, `users.password_hash`, `email_changes.new_email_bidx`) and open everywhere else; each domain's erasure leg owns enumerating its own, and M25 PR5's finding 2 is the evidence that building that list from column names is how it goes wrong.
- 2026-08-21 — M24 (dashboard, computable subset) is APPROVED with its flip-trigger unfired, and that is an owner's decision recorded rather than a trigger misread. The queue row's own sentence — "a proposal, not a decision", written because the demo-date/signed-customer trigger was checked 2026-08-21 and had not fired — was re-checked at kickoff and is still true. The owner decided to start anyway. Grounds: M25, the milestone the re-sequence preferred, is complete; M24 is next by queue number and no other row's blocker (E1–E5) cleared in the interim; and the shared client read cache M24 carries — needed by both of its docs/03 §6v residuals and later by M28, M30 and M31 — must ship with a first consumer, which M24 is. The trigger sentence is superseded by a decision, not by silence: the queue reached M24; M24 did not jump the queue.
- 2026-08-21 — M24's scope is the COMPUTABLE subset of docs/00's thirteen dashboard surfaces, measured against the SDL the BFF serves rather than assumed, and pinned by `apps/bff/test/dashboard-computability.spec.ts`. IN: net worth, assets, insurance-as-asset-categories, estate funding %, document completion %, beneficiary health, readiness-as-analyses, security-as-facts, settling estates. OUT, each with a reason the fence probes where the reason is checkable: liabilities (no data model), password health (Zone A is zero-knowledge — the server cannot compute it BY DESIGN), outstanding tasks (`estateTasks` is case-scoped, an executor's worklist; owners have no task system), notifications (M30 owns the feed), timeline (no backing). The fence states which layer it proves: the mapping against the SDL. The thirteen names are a transcription of docs/00 prose it cannot derive, and a red OUT probe means the platform GREW a backing — rescope the item, never delete the probe.
- 2026-08-21 — NO INVENTED SCORES. docs/00 names a "security score" and an "estate readiness score"; no service computes either, and a number invented at the edge is a methodology nobody can defend — it moves when the weighting changes, which reads as a security change that did not happen, and it flattens facts with different remedies into one digit. The dashboard renders the facts the platform can stand behind (MFA level, step-up state, passkey and session inventories, address verification; the four readiness analyses and their statuses). A real scoring methodology, if ever, is product work with an owner, not a frontend PR's private arithmetic.
- 2026-08-21 — insurance appears on the dashboard as what the data model holds: three asset categories (`life_insurance`, `ltc_insurance`, `annuity` — the `assets_view` CHECK owns that vocabulary), a facet of the assets/net-worth tile. A policy-level insurance surface — coverage amounts, premiums, term — has no model anywhere in the platform and would be new scope, not a tile.
- 2026-08-21 — the M26 name collision is FLAGGED, not resolved, by M24 PR0. M25 re-owned the erasure fan-out "to M26" in nine `[OWNER: M26]` residual tags across docs/03 §§6ll–6pp while docs/04's queue row scopes M26 as forensic audit completeness, which holds four earlier `[OWNER: M26]` tags of its own. Two bodies of work share one name; the queue row now says so, and whoever scopes M26 must split them FIRST. Flagged rather than resolved because assigning the fan-out a milestone is a queue decision that is not M24's to take — but inheriting the ambiguity silently would be the stale-prose defect this log keeps recording.
- 2026-08-21 — the shared client read cache (M24 PR1) enrolls reads by ALLOWLIST against a written bar: no audited decrypts, no decrypted PII, no variables. The first two are the audited-decrypt UI constraint applied to a new mechanism — every content/PII read spends a logged KMS decrypt on the owner's trail, and a cache serving one from memory makes the repeat read invisible to that trail, so `Contact`, `DocumentContent`, `EstateDistributionAmount` and their kind can never enroll. The third means keying is by operation name alone: the serializer that could be wrong is the serializer never written. The enrolled set is pinned with `toEqual` (today: `['EmailVerification']`), so growth is a reviewed argument against the bar rather than a drive-by, and a compile-time check refuses a parameterized enrollment outright.
- 2026-08-21 — cache invalidation is STRUCTURAL: the transport announces every successful mutation from the one place all mutations pass through, and the mutation→read map lives as data in the cache (`VerifyEmail` and `CompleteEmailChange` → `EmailVerification`; `CancelEmailChange` and `ResendEmailVerification` deliberately absent — neither changes the answer). The rejected alternative — each ceremony call site remembering an invalidate call — is the forgot-to-tell-the-reader class the §6v banner residual WAS, rebuilt one ceremony later. A refused mutation announces nothing, `SESSION_RENEWED` included, because nothing was performed; queries never announce, or the cache would feed itself. The emitter lives in its own module so the client never imports the cache.
- 2026-08-21 — two kinds of stale are kept apart in the read cache, and each renders differently. An INVALIDATED answer is known wrong (a mutation just changed the fact) and is discarded before the re-read — the banner drops for the round trip rather than render a lie. A REVALIDATED answer (the banner's pathname key) is merely old and keeps rendering until the fresh one lands, which is exactly the pre-cache behavior. The cache changes who SHARES an answer and when it is DISCARDED, never how often the server is asked — the banner's one-query-per-navigation contract survives verbatim.
- 2026-08-21 — `EmailVerificationPanel` deliberately does NOT read through the cache. The §6v remount wrapper does two jobs in one key bump — it re-reads the fact AND resets ceremony form state (a half-typed code) that a completed address change makes stale — and the cache can only do the first. Migrating the panel would have split one mechanism into two for no behavior change. Its direct `gqlRequest('EmailVerification'` call also keeps the operation-consumers fence anchored while the fence learns `useSharedRead('X'` as a second counted spelling, with a positive control so the new half of the matcher cannot go quietly blind.
- 2026-08-21 — `auth.email.viewed` is a NEW audit action, and the vocabulary grew rather than a neighbour being stretched. No existing member honestly names an owner reading their own sign-in address: the auth.email.* family records the change ceremony, and every disclosure member (`contact.link.estates_read`, `document.content.viewed`, `settlement.distribution.amount_viewed`) belongs to another service. Reusing one would have made an investigator join the read to the wrong ceremony. The cost is the consumer-before-producers rule — observed failing for real in M25 PR4, so it is priced, not theoretical — and with nothing deployed it costs one sentence in §6rr's residuals; the record-first emit above FieldCrypto's automatic decrypt event means every reveal lands TWO events, the route-level one naming the SESSION that authorised it.
- 2026-08-21 — the address on file is REVEALED ON DEMAND, never read on mount, and three independent lines all forced the same answer: frontend.md's no-prefetch rule (a page visit is not consent to a recorded disclosure), the read-cache enrollment bar (decrypted PII can never be served invisibly from memory), and the M18 `users` decrypt bound, whose reviewed note calibrated 60/window against "email decrypts on the change/notice paths only — rare by design". A mount-time read would have falsified that calibration on every /security visit; reveal-on-demand keeps every decrypt a user-initiated click. The bound's NUMBER is kept and its NOTE is rewritten in the same change, because a false sentence beside a live detector is the kind of drift this log exists to prevent.
- 2026-08-21 — identity answers its first 410 (`content_erased`, on a decrypt racing a crypto-shred — `distributionAmount`'s arm), and the BFF's shared identity mapper gained its first 410 arm in the same change. Without the arm, the erasure control firing would mask into `identity responded with status 410` → a generic outage, inviting retries against a key destroyed on purpose. Status-keyed rather than token-keyed, on the mapper's own 429 reasoning: 410 means one thing on every identity route. The web side needed nothing — CONTENT_ERASED and its copy have existed since the documents edge — which is itself the one-spelling rule paying out.
- 2026-08-21 — a completed address change DISCARDS a revealed address instead of auto-re-reading it. The revealed value is known wrong the moment the switch commits, so it must not keep rendering (the read cache's invalidation rule, applied by hand to a read the cache is barred from holding) — but the fresh disclosure costs an audited decrypt, and spending one the owner did not ask for would put an event on their trail for a look they never took. The reveal control returns instead; the mutation that auto-re-reads reddens exactly the discard test.
- 2026-08-21 — a discard without a supersede token is a resurrection path, and the PR2 review DEMONSTRATED it rather than argued it: an AccountEmail reveal held in flight across a completed address change landed after the discard and rendered the known-wrong pre-change address directly beside the "has been changed" notice — the thrice-fixed one-page-contradicting-itself class (M19 trust card, M20 session card, M20 PR5 devices list), here about decrypted PII on the security page. The read cache already solves exactly this with supersede tokens, and the reveal cannot use the cache (the PII enrollment bar), so the component carries its own generation token, bumped by the discard and by every new reveal. The transferable rule: the moment a state can be DISCARDED as known wrong, every in-flight read of it needs a token — a discard that only clears settled state protects nothing from the request already in the air.
- 2026-08-21 — the falsified-sentence fix itself shipped half the falsehood forward, and two independent review lenses converged on it. The M18 note said "email decrypts on the change/notice paths only"; PR2's rewrite removed the falsified "only" and kept "change/notice paths" — but the notice leg has NEVER decrypted under the users prefix, at M18 or since (the old-address notice resolves through the notifications recipient store, whose decrypt has its own bound row). The phantom survived the original review, the M24 kickoff read, and the rewrite performed specifically to purge falsehood from that sentence, because every editor inherited the producer list instead of re-deriving it. A note that inventories producers is only ever safe to edit by re-enumerating the producers.
- 2026-08-22 — the dashboard's fetch topology is PAGE-LEVEL READS PLUS PRESENTATIONAL TILES, and docs/04's "through the cache" scope sentence is amended in the same change rather than obeyed. The design fan-out proved the sentence collided with the cache's own written bar: `Assets` is parameterized (the compile-time check refuses it), `NetWorth` and `Readiness` are decrypt-backed (a cache re-serving them makes repeat reads invisible to the owner's trail — the bar's first clause, verbatim), and `Session`'s `stepUpFresh` decays by clock, which no mutation announcement can see. Between a scope sentence and a security bar, the bar wins — never weaken a boundary to simplify a feature — so the page fetches each operation once and passes props (one `NetWorth` read feeds three figures), the enrolled set stays exactly `['EmailVerification']`, and the security tile becomes that entry's designed-for second subscriber beside the app-shell banner. A milestone that invalidates a sentence owns that sentence.
- 2026-08-22 — the readiness analyses run ON DEMAND from the dashboard, never on mount. One `Query.readiness` runs four analysers: four asset-list reads (per-row audited est_value decrypts), two full profile PII fans (ssn_last4 included), a family read, up to 100 sequential beneficiary sub-calls, and an audit event per analyser — and the dashboard is the page every sign-in lands on, so a mount-time read would institutionalise that fan per landing against the written no-prefetch rule. The M24 PR2 reveal set the pattern: the protective default costs one click. The card renders all four statuses once run (no invented "not run yet" server state exists — the client's idle arm is its own knowledge that it has not asked), reuses the shared SECTIONS names so two surfaces cannot name one analysis differently, and writes its own DISABLED sentence because `STATUS_COPY.DISABLED` says "turn it on below" — copy coupled to a page whose consent controls are actually below.
- 2026-08-22 — the shared read cache gained an AUTH BOUNDARY (`Login`, `Register`, `Logout` → `CACHED_OPERATIONS` itself, by reference), because the PR3 fan-out's completeness critic found it SURVIVES A USER SWITCH: sign-out and sign-in are client-side navigations, entries are keyed by operation name and outlive their subscribers, and the only reset was harness-only — user B's first mount rendered user A's cached answer with zero fetches. `Logout` had even been the suite's own "invalidates nothing" negative control (moved to `CancelEmailChange`, the map's other documented deliberate absence). The value is the derived list, not a hand-copy, so every future enrollment inherits the boundary without anyone remembering it; `CompletePasswordReset` stays off the boundary because it mints no session — the sign-in that follows it is a `Login`. Found by asking a question no lens had asked: what clears the cache at the auth boundary.
- 2026-08-22 — the insurance facet is COUNTS-ONLY, and `CATEGORY_LABELS` moved to `lib/categories.ts`, made TOTAL over the DDL's 21-member vocabulary, and fenced against the migration itself in both directions. Per-category value sums were rejected because they would re-implement the service's `ownedShareCents` (estValue × ownershipPct in BigInt) in the browser — a second copy of a money guard — and surfacing the service's already-computed `byCategory` would grow the SDL against PR0's existing-fields-only scope. Two of the three insurance categories (`ltc_insurance`, `annuity`) had no display label anywhere in the web app; with a partial map the facet would have printed raw tokens, and the lib/documents rule (maps total over their unions, or a compile error) now applies to categories too.
- 2026-08-22 — the trust-funding percentage is computed client-side by a new `lib/funding.ts` pure function FAITHFUL to the service's own formula (assets.service.ts `inTrustPct`: BigInt cents, `(inTrust*1000n + total/2n)/total` then /10, null on zero total), because the platform computes this exact number and throws it away one hop before the client — the BFF's schema strips `inTrustPct` and the SDL omits it. Faithfulness is the point: if a later milestone un-strips the field, the two surfaces already agree, boundary included (the half-up test pins 0.75% → 0.8). The parser REFUSES malformed money strings with null rather than guessing (the @estate/money posture at render), and null renders "—" — a percentage of a zero-value estate is not 0%, it is no answer.
- 2026-08-22 — the dashboard's document-completion figure uses the /documents page's OWN denominator — every document on file, revoked and superseded included — via `inForceCount`/`documentsSummaryLine` hoisted into lib/documents.ts, so the tile and the page compute the figure in one place and cannot disagree about the same estate. The readiness analyser's opposite convention (revoked/superseded are "not present at all") is not wrong; it answers a different question — which instruments the estate still needs — and applies only inside its own findings, which the estate-checks card quotes as the analyser's answers, never re-derives.
- 2026-08-22 — `SessionCard` retires with the dashboard: the page's own session gate subsumes both its jobs (the signed-in facts move to the security tile, the signed-out arm becomes the hero plus get-started card), and the page carries the literal `gqlRequest('Session'` spelling so the operation-consumers floor (`callersOf('Session') >= 3`) holds through the swap — the fan-out flagged that floor as EXACTLY met, with SessionCard one of the three. The rail keeps its 'Overview' label (AppShell's test pins the string, and a cosmetic rename is not worth reddening a fence), and `Session` stays double-read on `/` (rail + gate) at exact parity with the page it replaces — §6ss records why enrolling it to dedupe would be worse.
- 2026-08-22 — a DISABLED control strands keyboard focus exactly like an unmounted one, and the M24 PR2 lesson has two halves for that reason. PR3's checks button kept itself mounted through a run ("Run the checks" → "Running…" → "Run again") and its docstring claimed focus therefore never strands — but a natively `disabled` attribute landing on the focused element blurs it to `<body>` in every major browser, which the PR3 review demonstrated on the live app, not argued. Keeping the control mounted is only the first half; the second is moving focus onto the landing outcome (SecurityPanel's PR2 fix had both). The button now uses `aria-disabled` plus a handler guard — the control never leaves the accessibility tree OR the tab order — and the run's landing focuses the always-mounted outcome region, on the running→outcome transition only so an unpressed button never has focus stolen. Deleting the focus move reddens exactly its named test.
- 2026-08-22 — a session that ends mid-dashboard collapses the PAGE to the signed-out arm; no tile words its own session-end sentence. The first draft applied the "two failures, two remedies" rule per-tile, which meant applying it to SOME tiles: three said "sign in" while the security rows and the checks card rendered outage copy inviting a retry that could only fail — the rule half-applied, which the review caught. A surviving UNAUTHENTICATED (the client's silent refresh already retried and failed) is a fact about the SESSION, not about any backing, so the honest rendering is the page's signed-out arm — one mechanism, one spelling, and the escalation's named test is exactly the mutation kill for it.
- 2026-08-22 — a fence whose input is a CHECKSUMMED migration file is frozen with it: categories.test.ts originally derived the vocabulary from 001_financial_schema.sql alone, and since migrations are append-only, any future vocabulary change arrives as a NEW file — the fence would have parsed the frozen snapshot forever, its ledger-side trigger structurally unable to fire, green for the same reason it was wrong. It now scans the whole migrations corpus and asserts exactly ONE category CHECK definition exists, so the migration that redefines the constraint reddens the fence and forces the derivation onto the latest definition deliberately. The transferable rule joins the M18 window-fence lesson: state the corpus, and ask whether the input CAN change when the claim says "when this changes, I fire".
- 2026-08-22 — `mfaLevel` is the SESSION's factor level, not the account's enrollment, and the dashboard drive proved the difference the first time it could: an account that had just enrolled TOTP signed in with password only and the security tile said "MFA not enrolled" — false about the account, and inherited verbatim from SessionCard, where it had been unfalsifiable for twenty milestones because no drive had ever created that state. The chips now say what the value measures: "Password-only session" / "Second factor verified". The eleventh consecutive milestone in which driving the real product surfaced a defect no suite could see — this one a sentence that was WRONG only for a state no fixture had constructed.
- 2026-08-22 — the security review's method is a QUESTION, not a checklist, and the question is *what else is in this category?* Applied to M24's own controls it produced five gaps in one pass, every one of them a rule this repo had already written down and applied to a single member of its category: an audited-volume rule stated in one BFF resolver's comment and enforced in neither it nor its sibling; a session-death escalation that reached the page and not the chrome; `mfaLevel` wording corrected on the dashboard and left on /security; a revealed-amount drop wired to one of two exits from the operator's case screen; and a focus lesson applied to a new card but not carried back to the control it was learned from. The transferable form: when a review fixes something, the next question is not "is it fixed" but "what is the OTHER member" — and when a comment states a rule, check whether the code beneath it obeys the rule, because a comment that states a rule it does not enforce reads as a control to every future reader.
- 2026-08-22 — `executorCases` and `reportableEstates` now read settlement FIRST and skip the profile fan when the list is empty, because the profile read is an audited CROSS-USER disclosure — one `contact.link.estates_read` plus one `crypto.field.decrypted` of another living person's `legal_name`, on THEIR trail, naming the caller as actor — and both resolvers were spending it before knowing whether any row would use it. Since PR3 put the self-hiding executor panel on the home page, that meant every contact-linked account paid one disclosure per linked estate per landing to decorate a list that was empty. The cost of the fix is one serial hop where `reportableEstates` had a `Promise.all`; the audited-volume-is-a-UI-constraint rule outranks a round trip on a screen the owner deliberately opened. Note what the PR3 review could NOT have caught: the resolver is not in PR3's diff — only the panel's new mounting point is — which is why a security review scoped to a milestone's SURFACE rather than its diff is worth running.
- 2026-08-22 — session death is announced by the TRANSPORT, from the one place a refused refresh distinguishes a dead session from an unreachable one, and every surface subscribes rather than each noticing its own 401. PR3's per-page escalation was correct and insufficient: the app shell's rail read `Session` once at mount and had no way to learn, so it kept asserting "Signed in" beside a page that had collapsed, and its Sign-out control then claimed a revoked credential was still live. The channel is the mutation-announcement channel's argument reused verbatim — a convention asking every future reader to tell the chrome is the forgot-to-tell class one component later. The direction matters as much as the mechanism: an UNAVAILABLE refresh must announce nothing, or an outage signs people out of live 30-day credentials, and that guard has its own named test (mutating the announcement to fire on any UNAUTHENTICATED reddens exactly it).
- 2026-08-22 — a `role="alert"` that says "you are still signed in" about a dead session is worse than saying nothing, so Logout treats UNAUTHENTICATED as the outcome it wanted. The button's failure arm was written for a refusal where the credential SURVIVES — that case keeps it — but an already-revoked or expired session answering UNAUTHENTICATED made the app state, in its loudest possible voice, the opposite of the truth, to a user who may have pressed Sign out precisely because they believed the session was compromised. Fail closed means DE-ESCALATE: there is nothing left to revoke, so the honest destination is the sign-in page.
- 2026-08-22 — where the app cannot know a fact, it says the thing that is true either way and lets the SERVER'S REFUSAL carry the rest. /security keyed its authenticator button's label on `session.mfaLevel`, so a TOTP-holding owner on a password-only session was offered a first enrolment and then refused by `SecondFactorGate` — an action the server would refuse, offered by a page guessing at a fact no SDL field exposes. One label now ("Add an authenticator app"), and the STEPUP_REQUIRED arm says what the generic copy cannot: that this account already has a factor, and where to verify. Both callers of that refusal share one sentence, because two spellings of one refusal is one of them going stale.
- 2026-08-22 — a wording fence over a component tree needs NO PARSER, and the first draft's parser proved it. The scan exempted matches inside comments so the comments explaining the defect could quote it, and the exemption was wrong about block-comment continuation lines, so the fence reported its own documentation as a violation. Prefer an ABSENCE to a filter: the comments were reworded to describe the bad sentences instead of quoting them, the corpus (read from disk, never hand-listed) now contains those strings nowhere at all, and the check is a plain substring search that cannot be misconfigured. The quoting lives in docs and in the test file, neither of which can reach a screen.
- 2026-08-22 — M27 is SCOPED to exactly the two halves of its name, and its seven inherited `[OWNER: M27]` tags — five distinct residuals, since the SRP bullet is cited three times — were all re-owned in PR0 rather than carried. The deny token and `emergency.reminder` go to M30 because M30 is the milestone that can close them without reversing a control: an EMAIL one-tap deny is a per-recipient URL, which `apps/services/notifications/test/templates.spec.ts` refuses outright and which docs/03 §6c cites as a CLOSED half of the M6 leakage item, while an SMS one waits on E4 — an in-app deny needs neither, and `ScheduleModule|@Cron(` matches zero files repo-wide so nothing in the tree can fire a reminder either way. The SRP abuse bound and rollback detection go to a new M39 row. Token-gated reset became `[ACCEPTED]`: every compensating control it names has shipped, and a lost password cannot be proved, so nothing is owed. The rule this applies: a milestone's headline scope must not live only in a queue row's prose while its tagged residuals describe other work entirely.
- 2026-08-22 — release becomes RE-COLLECTABLE, and the argument is that nothing was ever destroyed. `markReleased` is `UPDATE ... SET status='released', released_at=$2` and nothing else — `platform_part`, `wrapped_master_key_recovery` and `key_share_ct` all survive it untouched — so "one-shot" was a status check, never a cryptographic one-way door, and what it bought was that closing a tab spends an escrow for nothing in the one scenario the feature exists for. What makes re-collection safe is an asymmetry already in the code: `revoke` is the ONLY transition with no status guard, so the owner can revoke a released policy today, and the protective action stays available and one click while the permissive one becomes re-runnable. Every collection re-notifies, re-emits `vault.emergency.released` and re-runs the settlement gate, so a second collection is more visible than the first. The three other `already_released` refusals stay: `deny` and `rearm` on a released policy are genuine conflicts.
- 2026-08-22 — the grantee read does NOT change a vault credential. `vault-session.guard.ts` binds `row.user_id !== caller.userId → vault_locked` and `vault_sessions` has no notion of acting for another user; widening it with a subject/on-behalf-of column to serve one route is the "weaken a zone boundary to simplify a feature" move docs/01 forbids, and it would touch the guard every item route depends on. Instead the grantee presents their OWN session — they must have one, since their recovery private key is wrapped under their own master key — and the authority to read another user's ciphertext comes from an `emergency_access_policies` row in `status='released'`, re-checked against the settlement gate inside the transaction. Subject and object differ in one handler's authorization, not in a credential. `authz.service.ts` had already written down where this belongs: "there are no beneficiary or grantee reads in Zone A ... That changes when emergency access lands, and it changes here, not by loosening a policy elsewhere" — so the Cedar change is a grantee attribute in the vault's own domain and `owner.cedar` is untouched.
- 2026-08-22 — a soft-deleted `vault_items` row now records WHY, because `reset` and `deleteItem` write a byte-identical `deleted_at` and mean opposite things by it: `reset` replaces the keyset in the same transaction, so every row it retires is cryptographically dead. A restore list built as `WHERE deleted_at IS NOT NULL` would have offered those rows and failed as a silent AEAD error on click — a control firing wearing an outage's face. The column beats `app.change_reason` for three reasons in order of weight: the GUC lands in the SHADOW table while a restore list queries LIVE rows; the shadow row for a soft delete is captured BEFORE the UPDATE, so its image still has `deleted_at` NULL; and no service in this repo sets that GUC, in any cluster, after twenty-five milestones, so `vault_items_versions.reason` has been NULL for every row ever captured and nothing went red. Backfill posture: `unknown_pre_m27` rather than `NOT VALID` grandfathering or a false attribution — backfilling 'user_delete' would offer dead blobs, which is the exact defect the column exists to prevent, and a third value that says "we do not know" is honest, excluded from the restorable corpus, and lets the constraint ship VALID.
- 2026-08-22 — the residuals fence now declares the bullets it does NOT watch, per lead-in, because a stated bound is not a mechanism. docs/03 §6j:1623 recorded "there is no restore surface", assigned it to TB7, carried no disposition tag, and stayed invisible for five milestones while TB7 shipped as M21 without it: §6j organises by PR, `**Added by PR3a (origin matching).**` is in neither `REGION_MARKERS` nor `NON_REGION_LABELS`, and the classification assertion skipped any lead-in whose label did not itself say "residual". The fence's own docstring had ALREADY stated that bound in prose, which is precisely the point — it told a reader the gap existed and told the next author nothing when they wrote into it. The bullets the corpus does NOT reach are now declared as data, per lead-in, with a count and a kind each, so a bullet added where that one hid changes a count and reddens a named assertion (proved: adding an untagged prose residual under `Added by PR3a` reports "declared 6, found 7"). The stated bound of the replacement, in its turn: it counts per lead-in, so a one-for-one swap beneath one heading is invisible, and closing that would mean hand-listing 132 prose sentences beside a document that grows.
- 2026-08-22 — two parser defects surfaced while building that census, both of the mis-attribution class this file has already paid for twice. A bold lead-in whose run WRAPS across lines returned null from `boldLabel`, so both callers fell back to half a sentence ending mid-clause — and one such lead-in in §6pp, which does mention a residual, had therefore gone unclassified since M25 and was found on the fixed scan's first run. And `## 7. Validation program` sits BETWEEN §6hh and §6ii, because deltas append after the numbered sections and §7 was overtaken; the parser only ended §6 on another `## 6` heading, so §7's four bullets were attributed to §6hh. Neither was visible before, for the same reason: none of the affected bullets was tagged or used residual language, so none reached the corpus and nothing went red. The census is the first assertion in that file to look at the bullets the corpus REJECTS, which is why it found them.
- 2026-08-22 — the strongest mutation against the restorable-corpus fence SURVIVED its first draft, and the fix was to change what the assertion is anchored on. Re-labelling `reset`'s rows 'user_delete' — the one change that would hand an owner cryptographically dead blobs — left the fence green, because the draft scanned the whole of `src` for each literal and `'vault_reset'` is ALSO a session revoke reason in `vault.service.ts` and a third vocabulary in `events.service.ts`. Three alphabets share the token; a scan for a spelling cannot tell which one it found. The assertion now discovers the writer METHOD NAMES from their SQL — so it is not keyed on `softDelete`, which could be renamed into invisibility — and reads only the string literals in those methods' call-site argument lists. A surviving mutation means the test is weak, the mutation is unfaithful, or the change is not load-bearing; this one was the first, and the mutation was never weakened to make it red.
- 2026-08-22 — the release delta is TWO guards, not one, and the first draft of §6uu said one. `release` throws `already_released` at emergency.service.ts:579 and then falls straight into `status !== 'waiting' || !releases_at` → `not_requested` at :581, so removing only the first refusal leaves the escrow exactly as uncollectable while reporting it under a token that means something else — the two-failures-one-token defect this document forbids, reproduced against a real database by the M27 PR0 review rather than reasoned from the guard order. `request` cannot rescue it either: `blockReason` returns `already_released` and `markRequested` is the only writer of `status='waiting'` anywhere. The release path must therefore admit `status IN ('waiting','released')` with `releases_at` set and elapsed.
- 2026-08-22 — `deny` is admitted on a RELEASED policy, and the first draft of the re-collection argument got the protective half backwards. That draft justified re-collection by pointing at `revoke`, which carries no status guard — true, and not the point: `revoke` is `@UseGuards(StepUpGuard)`, while the ONE-TAP action is `deny`, deliberately `CallerGuard` only because a step-up prompt between the owner and 'stop this' is a control that argues with itself. `deny` refuses on a released policy. Making release repeatable behind a bare account session while leaving the only ungated stop unavailable would put the permissive action one call away and the protective one behind fresh MFA — the docs/03 rule inverted, in the paragraph that cited it. `deny` keeps its meaning (sticky, no cooldown, no further collection until the owner re-arms); it cannot un-release what the grantee already holds, but it ends the arrangement's ability to hand over more, with one tap.
- 2026-08-22 — the restorable fence is anchored on a route's KEYSET BEHAVIOUR, not on which writer it calls, and the first fix for the surviving mutation was still insufficient. That fix discovered writer method names from their SQL and read only their call-site arguments — but it unioned the reasons across writers, so `reset` and `deleteItem` swapping labels still satisfied it. The assertion now splits each service file into method bodies, marks a body `destroysKeyset` by the presence of a `keysets.replace(` call, and requires `REASON_DISPOSITION[reason]` to be `unrestorable` exactly where the keyset dies. `RESTORABLE_REASONS` stopped being a hand-list at the same time: `REASON_DISPOSITION` is a total `Record<DeletedReason, …>` so the DDL growing a fourth value is a COMPILE error naming the unclassified member, and the restorable set is filtered from it. The bound that remains, stated in the docstring and tagged `[ACCEPTED]` in §6uu: nothing here bounds the SCOPE of a retirement, so a `deleteItem` that retired every item a user owns would be labelled correctly and stay green — an unfaithful mutation, not a weak test, and PR1's undelete is where scope actually lands.
- 2026-08-22 — PR0 found the COMPLEMENTARY hole to the one it was scoped to fix, and pinned it rather than closing it. The census closes the case of an UNTAGGED residual outside the fence's corpus; this is the case of a TAGGED one whose owner has since shipped. `OWNERS` is derived from every row of docs/04's table INCLUDING the closed ones, so a tag naming a finished milestone passes the vocabulary check exactly as a live one does — thirteen residuals in the corpus name M22 or M23. The count is pinned with the owner SET beside it (mis-attribution between two closed milestones preserves a count), so the number cannot GROW in silence, and M40 owns the sweep; driving it to zero means deciding one at a time whether a later slice of the same programme still owes each item. It is larger than thirteen and the fence cannot say how much: M21's eighteen and M24's are invisible to any derivation from that table, because those rows say APPROVED and never gained a completion status.
- 2026-08-22 — that same assertion read the ROW where it meant to read the STATUS, and the residual it demanded is what caught it. The first draft matched `/^\| (M\d{2}) \|.*COMPLETE/`, so the new M40 row entered the completed set the moment it explained that thirteen residuals name milestones whose docs/04 rows read COMPLETE — a row DESCRIBING closed milestones classified as one, on its first run. A status is a bold run in the status cell; prose about other rows is neither bold nor a status, and the check now extracts the third cell and scans only its `**…**` runs, with an anti-vacuity floor on the ROWS as well as on the statuses so a moved table cannot empty the corpus quietly. The general form is this repo's oldest fence lesson arriving in a new costume: anchor on what the claim is ABOUT, because a scan wide enough to be convenient is wide enough to be wrong.
- 2026-08-22 — I asserted twice during the PR0 review, once with a running proof, that the design delta had an AAD/version defect, and both refuters were right that it does not. `itemContentAad` binds `blobVersion`, so a blob sealed at N opens only at N — but a full row image out of `vault_items_versions` carries `blob_ct` and `blob_version` as a MATCHED PAIR, `vault_items` constrains `blob_version` only to `>= 1`, and the client builds the AAD from the version the SERVER served. My proof exercised a shape the design never calls for (the historical blob at a BUMPED version), and the fix I proposed — client re-seal at N+1 — would have been strictly worse against the rollback residual, since it makes a restore indistinguishable from an ordinary edit. The same review pass wrongly CONFIRMED a category finding about `emergency_access_policies` retirement: `upsertConfig` overwrites `platform_part` and `wrapped_master_key_recovery` in place, so both callers of `softDeleteAllForOwner` retire policies whose wrapping is already dead — same meaning, so no discriminator is owed there and the `[ACCEPTED]` residual was right as written. Recorded because a refutation is worth as much as a confirmation, and because an empirical proof of the WRONG SHAPE reads exactly like an empirical proof.
- 2026-08-22 — a `className` is a string, so an app that names classes nothing defines type-checks, lints, tests and RENDERS — it just renders unstyled, and only a browser can see it. `VaultLaunch` and `OperatorLaunch` were written against `panel`/`panel-title`/`panel-note`/`button-primary`, a vocabulary no stylesheet in this repo has ever carried, so `/vault` — the page that explains the Zone A trust boundary before the step-up prompt that opens the vault — served its heading as body text (16px/400) and its primary button as a bare word (transparent, 0px padding, 0px border). Found by driving the real app during M27 PR1a, which does not touch `apps/web`: the twelfth consecutive milestone in which a browser drive surfaced a defect no suite could see. The fix maps both files onto the real vocabulary (`card p-6`, `btn btn-primary`, Tailwind utilities) rather than adding `.panel*` definitions, because a second spelling of `.card` is a copy that drifts.
- 2026-08-22 — the class fence's oracle is the STYLESHEET THE BROWSER RECEIVES, which dissolves the question it was going to have to answer. Checking that every class an app names is defined seems to require separating Tailwind utilities from app-defined ones, and the only way to do that directly is to hand-list Tailwind's grammar — a hand-maintained list beside a thing that grows, this repo's most repeated defect. So `class-vocabulary.test.ts` compiles `src/app/globals.css` through the app's OWN PostCSS pipeline (the `@tailwindcss/postcss` plugin `postcss.config.mjs` names, over the same source tree) and reads the class selectors out of the result: `.card` is there because `@layer components` declares it, `.mt-2` and `.max-w-[12rem]` because Tailwind generated them for these sources, and `.panel-title` is absent because nothing can make it exist. One derived set, no split needed — though both halves are still asserted separately, because a fence that stopped seeing one of them would otherwise go quietly green. The class names USED are read from the TypeScript AST rather than by grep, since a grep for `className="…"` cannot see `className={STAT_LABEL}` and would state a claim about all of them while checking two thirds. Stated bound: a class assembled at runtime from pieces is not reachable, and the partial token either side of a `${}` is DROPPED rather than checked, so the corpus under-reports at those seams instead of inventing failures.
- 2026-08-22 — widening that fence from the two files that prompted it found four more components on its first run, which is the "a rule applied to one member of a category is a rule half-applied" rule collecting on itself. `EstateDistributions`, `ReportDeathFlow`, `SettlementCases` and `WaitingPeriodPanel` all wrote `className="label"`/`className="input"` where the vocabulary is `field-label`/`field-input` — unstyled labels and unstyled form controls on the report-a-death flow and the waiting-period control, both protective surfaces; `UnverifiedAddressBanner` wrote `bg-surface-sunken` where the theme key is `--color-sunken` and so the utility is `bg-sunken`, leaving the app-wide unverified-address strip with no background; and `Dashboard` wrote `notice`, which never existed anywhere, on the paragraph that tells a user their session has just died. Six components, one defect, and the two that were reported were the two somebody happened to open. Proved by reverting: each revert reddens `names no class the stylesheet has no rule for` and leaves the other five assertions green, and a faithful non-mutation (reordering tokens inside a real `className`) stays green throughout — so the fence is not merely failing on any edit.
- 2026-08-22 — `blob_version` was doing TWO jobs and M27's restore is what forced them apart, so `vault_items` gains `revision` (migration 005) and `If-Match` moves to it. The AEAD binding must travel with its ciphertext — `itemContentAad` seals against the version, so a restore puts the captured pair back and the live version can go DOWN. The concurrency token must never repeat — `updateItem` compared `If-Match` by strict equality, which detects change only while a value occurs once per row's life, an invariant that lived in a single `+ 1` at one call site rather than in the schema. Held together they are contradictory: restore a row from 5 to 3, edit twice back to 5, and a client still holding the first 5 passes `If-Match` and silently overwrites the restore and both edits, with the audit trail recording a routine update. Found by the PR1 design fan-out's refute-by-default verifier, which could not refute it. The trigger ASSIGNS `OLD.revision + 1` rather than validating a supplied value, so the invariant is the table's rather than a caller's, and a downward `blob_version` becomes legible as a restore — material M39's rollback detection has nothing else to look at today.
- 2026-08-22 — a change that cannot be falsified until the NEXT PR is a change nobody checked, which is why M27 PR1a's integration test forces its own divergence with SQL. On a row that has only ever been created and updated, `revision` and `blob_version` advance together and are always equal, so every assertion driven through those states passes identically whichever column the service compares — the three `If-Match` tests that predate the change stay GREEN under a mutation that reverts it, which is the same fact from the other side. The test moves `blob_version` away from `revision` directly, asserts they differ before driving anything, then exercises the arm where they DISAGREE in both directions: the blob version is refused as a token, the revision is accepted. Every client double was given a revision that deliberately differs from its blob version for the same reason — doubles that agree with each other cannot catch a call site that picks the wrong field.
- 2026-08-22 — the vault answered `403 forbidden` for another user's item and `404 not_found` for a missing one, and both now answer 404 because ownership is FUSED into the statement rather than checked after the row is in hand. The pair was an existence oracle for any item UUID that leaked, against docs/03's uniform-404 rule, and the read itself had already answered a question about someone else's data before any gate ran. Cedar still decides whether this principal may take this action — the layer M27 PR3's grantee read needs — but it no longer decides ownership, because the row never arrives. The test compares the two responses as WHOLE VALUES: a status match with a differing body is still a discriminator.
- 2026-08-22 — the same question asked of the rest of the category found `emergency_access_policies` split the same way IN THE SAME SERVICE, with the right answer twelve lines below the wrong one: `requireGranteePolicy` already answered a uniform 404 and explained why in its own comment, while `requireOwnerPolicy` let Cedar answer 403. Both arms are fused now and the two unfused lookups DELETED rather than left available — `EmergencyRepo.findLiveById` turned out to have had zero callers before this change and was already dead code. `ai-assistant` was checked and is correct (its authz throws `NotFoundException`) and is the precedent this follows; `plaid`'s `sync`/`revoke` carry the identical defect and got a queue row (M41) rather than a fix, because reaching from a vault PR into another service is scope creep to propose rather than perform.
- 2026-08-22 — M27 PR1a's adversarial fan-out found a defect the PR itself shipped, and the interesting part is why five gates missed it. PR1a added a required `revision` to the extension's `update` message and changed the hop that READS it (`vault-host`), but not the module that BUILDS it (`vault-client`) or the popup that calls it (`vault-screens`). `pnpm typecheck`, `pnpm lint` and 488 tests were green, and every popup edit would have gone out with NO `If-Match` header at all and been refused 400. Two boundaries were unsound in opposite directions: `ask(message: unknown)` type-checked nothing leaving the popup, so the `VaultRequest` union was documentation on the send side; and `isVaultRequest` asserted `value is VaultRequest` after inspecting `target` and `kind` alone, so the router read `message.revision` as a `number` that was `undefined`. Fixed at both ends — `ask` now takes `VaultRequest` (proven load-bearing: reverting the producer reddens `vault-client.ts(160,50)` with the exact error that would have stopped this) and the predicate validates every field the union declares. A type predicate that inspects less than it asserts is a cast wearing a check's clothing.
- 2026-08-22 — the test that "proved" the extension sent `revision` built its input by hand, so it proved only that `VaultHost.updateItem` forwards what it is handed — the producer was never exercised. Replaced by a screens-level test that drives the real chain (a listed item, the edit form, `vault-client`, the message on the wire) and asserts against a fixture whose `revision` (41) deliberately disagrees with its `blobVersion` (4). It fails with `Received: undefined` against the code as PR1a shipped it. The rule this repo already had — a fixture where two facts AGREE cannot catch a call site that picks the wrong one — was applied to the doubles in PR1a and not to the fixture that mattered.
- 2026-08-22 — widening `isVaultRequest` to validate payloads immediately caught a stale test double nobody had noticed: `wiring.spec.ts`'s `UPDATE` fixture sent `content` where the union declares `changes`, and the router has always read `message.changes`. It passed for years because the old predicate vouched for fields it never looked at. A gate that checks less than it claims does not merely fail to catch new defects; it preserves old ones.
- 2026-08-22 — migration 005's comment justified touching one table by asserting a fact about the tree — that item content is the only AAD binding a mutable per-write counter — and then hand-listed six builders to support it. The CLAIM was true; the list was not: `fieldAad` and `aliasAad` do not exist, and every inline-built Zone B AAD was missing. The same wrong list had been copied into docs/03 §6vv under the heading "DERIVED RATHER THAN ASSUMED". Replaced by `packages/vault-crypto/test/aad-bindings.spec.ts`, which holds every AAD as data keyed on the LITERAL the runtime feeds to AEAD (not the function name, which can be renamed into invisibility), derives the live set from both zones, compares in both directions, and asserts mechanically that exactly one binds a counter. Migration and doc now cite the fence instead of restating it. CLAUDE.md's rule — a comment justifying an omission by asserting a fact about the tree is a test nobody runs — was written for exactly this and I wrote the comment anyway.
- 2026-08-22 — M27 PR1a fused ownership into the lookup on BOTH arms of the emergency service, and only the grantee arm had a test; every existing non-owner probe hit `/request`. `deny`, `rearm` and `revoke` were unfalsifiable. Closed with a probe whose corpus is READ OUT OF THE CONTROLLER — every route naming a `:policyId` joins it by existing — carrying step-up headers where the route demands them so the probe reaches the lookup instead of stopping at a 403 that says nothing about ownership. Proven by unfusing the owner lookup: exactly the three owner-arm routes redden, both grantee-arm routes stay green, and the positive control (the policy is real and every route reachable) stays green. An arm-specific failure is what distinguishes a probe that measures ownership from one that measures liveness.
- 2026-08-22 — `POST /v1/vault/items` is an existence oracle across users and is NOT fixed: `vault_items.id` is a global `PRIMARY KEY` supplied by the client, so a colliding id answers 409 `item_exists` where a fresh one answers 201. Three review lenses raised it independently and no refuter could refute it. Closing it means per-user uniqueness — a primary-key change on a table with a version-capture trigger and its own history — which is a schema decision to propose rather than perform inside a PR about a concurrency token. Recorded as a tagged residual owned by M41 and pinned by a characterization test, because the uniform-404 rule is categorical and this is a genuine exception to it; the practical reach is bounded by ids being unguessable UUIDs, which is a mitigation and not a reason to leave it unwritten. The §6vv sentence claiming "every item route" was corrected to the id-bearing routes it actually covers.
- 2026-08-23 — M27 PR1b's design fan-out refuted five of its own seven lenses, and the refutations were run against a real database rather than argued. The three that changed the build: the versions reader pages on `revision` and never `version_seq` (a platform-wide BIGINT identity, and this service's cursors are base64url of PLAINTEXT, so paging on it would publish a decodable write counter and put a sequential id on the wire); the reader is a `LEFT JOIN LATERAL` and not a plain `LEFT JOIN`, because with the cursor in a join condition the LIMIT applies after the join and every page materialises the row's whole history — measured 77 shared buffers against 5 on a row with 5,000 images, re-measured independently before adopting; and the "only predicate is `row_id`" absolute is fail-OPEN, since item ids are client-supplied, so the image's own `user_id` is projected and asserted against the live row. That assertion RAISES rather than filtering precisely because it cannot fire today: a silent filter would turn the day the no-hard-delete guarantee breaks into a surface quietly showing fewer versions.
- 2026-08-23 — PR0's `deleted_reason` discriminator had a hole, found by two lenses independently and confirmed in the source. Migration 004 exists so a restore never offers a blob the keyset outlived, but `softDeleteAllForUser` carries `WHERE deleted_at IS NULL`, so an item the owner deleted BEFORE a reset was never touched by that reset and kept saying `user_delete` — restorable — while the keyset that opened it was replaced in the same transaction. PR1b's list would have offered it and the failure would have arrived as a silent AEAD error on click: a control firing wearing the face of an outage, which is the exact shape 004 was added to prevent. `reset` now relabels rows already retired and leaves their `deleted_at` alone, because WHEN the owner retired a row stays true and only its decryptability changed. `reset` has exactly two callers of `keysets.replace` and only one destroys a master key — the rotation re-wraps the same one — so no timestamp on `vault_keysets` can distinguish them and a targeted relabel at the destroying call site is the honest fix. Proved by reverting it: `Expected "vault_reset", Received "user_delete"`, one assertion red and forty-nine green.
- 2026-08-23 — the shadow table's per-item handle is a GENERATED column rather than one the capture trigger fills. `vault_items_versions.revision` is `GENERATED ALWAYS AS ((row_data->>'revision')::int) STORED`, so it cannot disagree with the image it names, it needs no edit to the SHARED `versionsTableSql` capture function in packages/db (a local edit to one copy of a generated body is the drift this repo keeps closing), and it is correct for rows captured before it existed — where a trigger column would have needed a backfill, since `CREATE OR REPLACE FUNCTION` only affects future captures. Images predating migration 005 have no `revision` key and are NULL: unnameable, therefore unrestorable, which is the fail-closed direction.
- 2026-08-23 — a restore writes THREE columns and everything else is an absence rather than a filter. `blob_ct`, `blob_version` and `item_type` are named; `id`/`user_id` (identity), `created_at`/`updated_at`/`revision` (trigger-owned) and `deleted_at`/`deleted_reason` cannot travel because they are not in the SET list. The last pair is the one that matters: an image captured at an UNDELETE holds the row as it was WHILE RETIRED, so writing it forward would be a restore that deletes the item. The reader excludes such images at the source rather than checking at the call site, so the arm cannot be reached instead of being refused when it is — which is what keeps `vault.item.restored` truthful by construction rather than by a guard somebody could forget.
- 2026-08-23 — none of PR1b's four routes carries `StepUpGuard`, and the asymmetry with `deleteItem` is deliberate. `updateItem` destroys the previous content of an item, carries `VaultSessionGuard` alone, and is admitted to the `extension` audience; putting a fresh factor in front of the UNDO of that write while the write itself needs none would make the protective action harder than the permissive one, in the milestone that exists to honour that rule. All four are refused to the `extension` audience — the audience that can overwrite is not the audience that can roll back — and the count in `session-audience.spec.ts` moved by a deliberate edit, which is what that fence is for.
- 2026-08-23 — two fences were found weaker than their own docstrings while PR1b used them. M27 PR1a's uniform-404 probe matched `'vault/items/:itemId'` with the closing quote immediately after the parameter, so it captured only routes whose path ENDS at the id and was structurally blind to any route beneath it — it would have stayed green while probing none of PR1b's three. Widened, and proved by reverting: `Expected >= 6, Received 4`. Investigating that number then found the same scan was reading PROSE: a doc comment quoting a decorator counted as a route, which inflates a floor and, worse, would keep a fence green over a route deleted while a comment still mentioned it. Both scans are now anchored at line start.
- 2026-08-23 — the residuals fence had a floor only on its total (95 against 212 actual bullets), so it could not see a single residual being REMOVED — and the one it was least able to see was the §6j bullet recording that no restore surface existed, the most load-bearing sentence about M27's own scope and the one this milestone had every reason to want gone. It now carries a floor PER SECTION that ratchets up and never down, exactly like a coverage threshold, plus an assertion that every section holding residuals HAS a floor, so the same gap cannot reopen one level up. CLAUDE.md already said an anti-vacuity floor belongs at every LEVEL of a scan; the total was the only level this fence had. Proved by deleting the §6j bullet: `"§6j: 10 < 11"`, one assertion red and fourteen green.
- 2026-08-23 — `EventsService.emit` took a hand-written union of twenty `vault.*` action literals while `AUDIT_ACTIONS` already held TWENTY-FOUR, so four members of the closed vocabulary were unreachable from the one service that owns them — not by decision, but because nobody re-typed them. A producer cannot emit an action its own parameter type has never heard of, and nothing was red. Replaced by `Extract<AuditAction, \`vault.${string}\`>`, which makes the vocabulary the single source: an action added there is emittable the moment it exists, and one removed stops compiling at the call sites rather than at a list. Found because adding `vault.item.restored` — in the vocabulary since PR0 with zero producers — would not compile.
- 2026-08-23 — M27 PR1a's REVIEW edited migration 005's comment (the hand-listed AAD builders), which changes its checksum, and any environment that already applied 005 now raises `MigrationDriftError` on its next migration. Measured on the local stack: stored `a25b14eb…`, file `7975c171…`. Defensible because 005 had never left this machine — unpushed, nothing deployed, and scratch-schema int suites rebuild from zero every run, so the only environment affected is a developer's own stack, whose remedy is `pnpm stack:reset`. Recorded rather than left to be discovered: the choice was between a permanent factually-wrong comment in an append-only file and a one-time local reset, and the comment was wrong about the tree in a way the next reader would have believed. The rule stands unchanged for anything that has shipped; what this entry adds is that "applied" means applied SOMEWHERE THAT MATTERS, and a local dev stack is the one place that can be told to start over.
- 2026-08-23 — migration 006 was verified against a POPULATED table, not only scratch schemas: applied inside a transaction on the running stack's vault database (5 items, 7 captured images) and rolled back. `ADD COLUMN ... GENERATED ALWAYS AS ... STORED` rewrites the table, which a fresh scratch schema cannot exercise. The generated handle derived revision 1..7 strictly increasing against blob_versions 1,2,7,2,2,2,3 — the exact history M27 PR1a's browser drive created, and the clearest possible statement of why the two numbers had to be split: the handle is monotonic per row precisely where the version is not.
- 2026-08-23 — M27 PR1b's adversarial fan-out (52 agents, 8 lenses) returned 20 surviving findings, and the two that four and three independent lenses raised were both about a claim being wider than its evidence. FIRST: `listRestorableItems` returned every retired item's whole ciphertext and emitted no audit event, while both of its siblings on the same disclosure emit — and it could not have, because the controller never passed a session id, so the omission was structural rather than a decision. Fixed by threading `accountSessionId` and emitting `vault.items.listed` with a required `scope` of `'live' | 'restorable'`; a required argument rather than a default, so a third list route cannot inherit "live" by saying nothing. Deliberately NOT a new `AUDIT_ACTIONS` member: a new action id is a closed-vocabulary change an older consumer drops silently, and this is a property OF the event rather than a different event.
- 2026-08-23 — the index fence shipped in PR1b EXPLAINed a query written out by hand in the test — the right shape, the right index, and a string appearing nowhere in the product. A verifier proved it worthless by rewriting `listRestorable` to the flat `deleted_reason = ANY($2)` form: all six assertions stayed green while the plan abandoned the index entirely. The same review found it asserting `not.toContain('Sort')`, which is FALSE of the real query — it merges one ordered scan per reason and must sort the union — an assertion that could only hold because it was measuring something else. The reader is now invoked through a recording `Queryable`, so the text under EXPLAIN is the text Postgres receives. The load-bearing assertion is the count of `Limit` nodes rather than the absence of a Seq Scan: whether a flat query gets a sequential or a bitmap scan is a function of the DATA, but only the LATERAL can put a `Limit` per reason, so that property does not depend on the fixture. The mutation that left the old fence 6/6 green now reddens four named assertions.
- 2026-08-23 — PR1b introduced a race into `reset` and closed it in the same milestone. The retirement was two statements — soft-delete the live rows (`WHERE deleted_at IS NULL`), then relabel the retired ones (`WHERE deleted_at IS NOT NULL`) — predicates that partition the table only while nothing moves between them. `undeleteItem`, which PR1b also introduced, is exactly the verb that moves a row from the second set to the first, so a row undeleted in the gap was matched by NEITHER and survived the reset LIVE with a blob the keyset replaced in the same transaction had just made undecryptable. Now one `UPDATE ... FROM` over a `FOR UPDATE` CTE: a concurrent writer is waited for and its row re-checked against the version it committed. Proved by reverting to the two-statement shape and running two real transactions that commit the undelete at the fatal moment — the race test and the one-statement test go red while both non-concurrent tests stay GREEN, which is the positive control that matters here: the old shape is correct except under interleaving, so a suite that reddened everything would have been failing on the edit rather than on the defect.
- 2026-08-23 — three fixtures in PR1b were found unable to fail, all for the same reason and all proved by mutation. The versions-reader fixture built its history from create-then-update only, so `revision === blob_version` in every image and `toVersionDto` could return one where the other belonged with every decrypt still passing; a delete/undelete cycle now separates them, since both bump the trigger's counter and neither touches the AEAD binding. Every fixture used one `item_type`, so a restore that dropped `item_type` from its SET list was invisible; the type now changes mid-history and is derived from the schema enum rather than spelled. And the `kind: 'version'` audit emit had no assertion at all — deleting it outright left the whole suite green. This is M27 PR1a's own lesson arriving one PR later: a fixture where two facts AGREE cannot catch a call site that picks the wrong one.
- 2026-08-23 — the vault README's two route tables were hand-maintained beside a route set that grows, and had drifted twice: PR1b added four item routes and documented none, and `GET /v1/vault/recovery-key` had been missing since M14. Both rows added, and the class closed with a fence that derives the live set from `PATH_METADATA`/`METHOD_METADATA` on the controller prototypes — what the router itself consults — and compares SETS in both directions, since an undocumented route and a documented route that no longer exists are different defects. Parameter names are normalised (`:policyId` and `:itemId` both become `:id`) because the table documents a route's shape, not a rename the router does not care about. `VaultAction` got the same treatment: a bare union grew by three actions that no test exercised in either direction, so it is now derived from a `VAULT_ACTIONS` const the suite can enumerate, and a ninth action arrives with its permit and deny cases already written.
- 2026-08-23 — `route-consumers.spec.ts` was structurally blind to the isolated vault origin, and M27 PR2 is the change that could see it. The fence matched a URL template against the name of the file DECLARING the route, so a caller whose paths are `/api/vault/…` — rewritten to `/v1/vault/…` at the edge — matched nothing no matter how many routes it consumed, and `EXEMPT_RESTORE_SURFACE` had been quietly holding four routes open on a deadline the fence itself could never enforce. Measured with the screen written: `fileMatchesPath` names all four; `templateMatchesPath` stays 13/13 green and names none. The exemption is DELETED rather than emptied, with a gravestone comment saying what stood there — an exemption that outlives its deadline is a permanent hole wearing a temporary name, and an empty list is a mechanism waiting to be refilled by the next person in a hurry.
- 2026-08-23 — the same fence caught my own bug inside the same PR, which is the only reason the interpolation rule is written down. `itemVersions` first built its URL as a single template with the query inside it; the scan collapses every `${…}` to `:p`, so the final segment read `versions:p`, matched no route, and the sweep named three of the four restore routes while silently missing this one. The query is now APPENDED to a template that ENDS at the route. A fence anchored on what the runtime reads would not have cared; this one is anchored on source text, so the rule it needs — keep the path a single literal ending at the route — belongs beside it rather than in someone's memory.
- 2026-08-23 — three restore refusals reached the reader as one sentence, and one of the three can never succeed on retry. The client mapped 403/404/409 to `FORBIDDEN`/`NOT_FOUND`/`CONFLICT`, so a stale concurrency token, a superseded version and an item whose keyset a vault reset destroyed all said "reload and try again". `ITEM_UNRESTORABLE` now says the contents were destroyed by the reset and deliberately does NOT say try again — sending someone back to press the same button forever is the failure the M9 rule exists to prevent. `VERSION_NOT_FOUND` does not borrow `NOT_FOUND`'s copy either: the ITEM is visibly on screen while its version is gone, so "that item is no longer there" would be a false sentence about something the reader can see. 412 keeps its unconditional `CONFLICT` — it is a precondition failure with no token to branch on. The mapping is a 16-row table with four positive controls, so a new token cannot inherit a neighbour's sentence by being added next to it.
- 2026-08-23 — the design called for a "Current" row in the version list and there is no such thing, so the label and its filter were deleted rather than shipped. The capture trigger reads `OLD`, so every image carries the revision the row had BEFORE the write that captured it: the live revision is never among the images and the version the reader is already at has no image at all. The screen therefore offers every row the server returns, and the server's no-op arm is unreachable from here by ABSENCE rather than by a predicate someone has to keep true. The spec asserts the absence — `max(image revisions) < live revision` — instead of testing a branch that cannot run. Verified against migration 005's trigger and PR1b's own integration test before removing anything, because a critique that says a case cannot happen is a claim to check, not to accept.
- 2026-08-23 — History is reached from the vault LIST and never from the edit form, and both restore screens return to the list rather than to the item. This is a correctness boundary wearing a navigation decision's clothes. Entering history from inside a populated `<form>` would drop a half-typed secret with nothing said, because this origin has no dirty check, no cleanup hook and no confirm dialog by design. The return path is the sharper half: handing a reader back a PRE-restore `OpenedItem` leaves the next save sealing under a `blobVersion` the row no longer has, and the row lands permanently unopenable with nothing in the response saying so — the exact failure `.claude/rules/zone-a-vault.md` names. The post-restore re-render adopts the FRESH item the server returned for the same reason.
- 2026-08-23 — a version this client cannot open is listed and carries no restore button, while an unreadable DELETED item keeps its "Bring it back". The asymmetry is the whole safety argument: restoring an unopenable image writes ciphertext nobody can read over live content, converting a readable item into a dead one through a supported route, whereas undelete writes no ciphertext at all and cannot make anything worse. The row still shows in both cases — the owner is entitled to know their past exists — so what is withheld is the action and not the fact.
- 2026-08-23 — driving the real app found M27 PR2's own defect, in the function I had just written a residual to excuse. `captureTime` trimmed the server's ISO string and therefore rendered UTC, and I justified it in docs/03 §6xx by `apps/vault-web`'s dependency fence of exactly `['zod']`. The premise was false: `Date` is the RUNTIME, not a dependency, and nothing about the fence forced UTC. The cost was not hours of skew but a wrong DATE — the drive edited an item at 17:00 on a Sunday in Phoenix and the history said Monday, which is wrong every afternoon for every owner west of UTC, on the one screen that exists to say when something changed. The parts are now read off the `Date`. The TEST is the load-bearing part: any assertion naming a literal like `2026-08-23 17:00` states a fact about the RUNNER's zone, and CI runs in UTC, where the trim and the fix are indistinguishable — so the spec asserts an EQUIVALENCE instead, that the same instant spelled `…T00:00:31Z` and `…T17:00:31-07:00` renders identically. Proved by reverting: two named assertions redden and sixteen stay green under BOTH `TZ=America/Phoenix` and `TZ=UTC`. This is the thirteenth consecutive milestone in which a browser drive found something no suite could see, and the first in which what it found was a residual I had written to explain the defect away.
- 2026-08-23 — `app.ts`'s own docstring said "the vault has five states" and listed them, beside a file that had grown screens in M15, M18, M23 and now M27. The count and the list are both gone rather than corrected to seven; a hand-maintained list beside a thing that grows is this repo's most repeated defect, and the fifth correction of the same sentence is evidence the sentence is the problem. What replaces it says why there is no router and stops.
- 2026-08-23 — the same drive found the LAST gated action on the vault origin that could not ask for a factor, and it was the screen everything else sits behind. `POST /v1/vault/srp/start` and `/srp/verify` both carry `StepUpGuard`, whose 403 `stepup_required` exists — its own comment says so — so a well-behaved client elevates; `renderUnlock` called `session.unlock` bare. Once the vault-open step-up aged past five minutes, an owner returning to the tab was told "that action needs a fresh identity check, and it was not completed" on a page carrying nothing that could complete one, and the only real remedy was re-opening from Estate — which the copy deliberately refuses to suggest, because a fresh handoff is exactly what the step-up change exists to devalue. The copy was already right; being right about a remedy the page does not provide is the whole defect. Unchanged since M15 PR2, fixed inside M27 PR2 because a history nobody can reach is not a restore surface. This is the M15 review's enrollment finding — "the fix is only complete if every gated action can ask" — arriving a third time in the same category, which is what not sweeping a category costs. Proved by reverting to the bare call: the two new assertions redden and the three step-up tests that predate them stay green.
- 2026-08-23 — M27 PR2's adversarial fan-out (36 agents, 6 file-scoped lenses, every finding refuted twice by default) raised 15 and 12 survived, and the two HIGHs were both defects the PR itself introduced hours earlier. FIRST, the one three lenses found independently: `renderUnlock` hosted its step-up prompt INSIDE its own `<form>`. `renderSetup` carries a load-bearing comment twenty lines above its own `note` saying exactly why that node must be OUTSIDE — the prompt is itself a `<form>`, and a form nested in a form is invalid HTML — and the unlock fix did not read its sibling. In a hand-built DOM the inner form is not dropped by a parser, it EXISTS, so its submit event BUBBLES to the unlock handler and confirming the code started a second unlock beside the retry `withStepUp` was already making. Measured at three `srp/start` POSTs where the suite asserts two. The M15 review found this same shape on setup; this is its third instance in the same file, and the reason a category sweep is not optional.
- 2026-08-23 — the harness could not see it, which is why it shipped. `submitStepUp` dispatched `new Event('submit', { cancelable: true })` — `bubbles` defaults to FALSE — so a synthetic submit reached the prompt's own form and stopped there. A correctly-hosted prompt and a nested one are indistinguishable under a non-bubbling event, and every count assertion in that suite was therefore inert against this defect while reading as rigorous. The harness now bubbles, which is what a real click does. Proved by reverting the node placement: the count assertion and a new un-nested assertion redden while both SETUP tests stay green — the positive control that says the suite is failing on the defect and not on the edit.
- 2026-08-23 — `version_conflict` is NOT restore-only and PR2's copy pretended it was. `updateItem` and `restoreItemVersion` throw it for the identical `locked.revision !== ifMatch` test, so the sentence "This item changed while you were looking at its history. Reload the history and try again." reached the EDIT FORM — a false statement naming a screen the reader is not on, pointing at a control that is not there. That is precisely the wrong-remedy class the refusal split exists to fix, half-applied, committed in the same change that wrote the list explaining it. `apps/web/src/lib/copy.ts` had already settled this in M19 for the same token — "surface-neutral on purpose: the same stale-If-Match refusal now covers documents AND assets, and the remedy is identical" — so the fix is that precedent, not a new idea. The code stays a distinct token from `CONFLICT` (which covers `item_exists`, `keyset_exists` and 412, genuinely different conditions); only the copy stops naming a surface.
- 2026-08-23 — the capture-time test I wrote to be zone-independent FAILED ON CI, and the reason is the half of the proof I skipped. `expect(asZulu).not.toContain('2026-08-24 00:00')` is itself an assertion about the RUNNER's zone: under TZ=UTC that string IS the correct local rendering. I had run the MUTATION red under TZ=UTC and concluded the test was portable — but red-under-mutation is not green-under-fix, and I never ran the positive control in the zone that mattered. CLAUDE.md asks for both halves and my own memory file records this exact failure mode. The equivalence assertion alone is the discriminator and needs no help: the trim renders the two spellings of one instant differently in EVERY zone. Now verified both ways — green under UTC, Phoenix, Kolkata (a :30 offset) and Auckland; red under all four with the trim restored.
- 2026-08-23 — the same review swept the timestamp category the captureTime fix had not, and found the Emergency Kit's `Issued:` line still stamping `new Date().toISOString().slice(0, 10)` — the UTC date, on a document whose entire purpose is being read months later by someone reconstructing when things happened. Both now share one `localDate` helper. `releasesAt` was checked and is correct: it renders as a remaining DURATION, which no zone can shift.
- 2026-08-23 — two numbers in this milestone's own prose were measured facts attached to the wrong mechanism. docs/03 §6xx and docs/04 both said the consumer fence "named three of the four routes and missed the fourth" when the sweep was rewrite-blind; the measured answer for THAT defect is ZERO of four, because the stale-exemption sweep compared RAW templates while the main check already applied the rewrites. "Three of four" is a real measurement of a different defect — my `${query}` interpolation collapsing the last segment to `versions:p`. Re-measured directly rather than reasoned about: old sweep 0/4, new sweep 4/4. A number that travels away from the thing it measured is the second-copy defect wearing a citation's clothes.
- 2026-08-23 — §6ww's forward pointer answered PR2's screen question wrongly, and the true answer is stronger. It said a reset-killed row appears on Deleted items with `ITEM_UNRESTORABLE` as the sentence the owner reads. It does not appear anywhere: `REASON_DISPOSITION` classes `vault_reset` as unrestorable, so `RESTORABLE_REASONS` is `['user_delete']` and the row is on NO surface — an absence, not a message. `ITEM_UNRESTORABLE` stays reachable only through a race (listed while it still said `user_delete`, undeleted after a reset relabelled it), which is the arm the screens spec actually forces and now says it forces.
- 2026-08-23 — the route↔consumer fence applied EVERY edge rewrite as a prefix, including the rows the edges enumerate precisely so they are not prefixes. `deriveEdgeRewrites` captured `tree:\s*(true|false)` into `m[3]` from the day it was written and never stored it — `EdgeRewrite` had no such field — so `fileMatchesPath` ran one unconditional `template.startsWith(rewrite.from)` over the lot. The hazard is the one vault-web's own comment names and docs/04 restates: `/api/auth/logout` is a prefix of identity's unauthenticated `/v1/auth/logout/refresh`, so a consumer template one segment past an exact row would have certified a route that edge answers 404 — the fence doing exactly what the edge refuses to do. `tree` is stored now, read from each source rather than assumed (the operator rows declare none and its matcher is segment-wise exact), and a `tree: false` row matches only a template that IS its path. Inert when found: raised by M27 PR2's adversarial review, REFUTED there on both grounds — no row on this tree produces a false match, and the code was byte-for-byte pre-existing — and fixed separately rather than as scope creep in a screens PR, which is the same call M23 PR4b made about `pathSharedWith`. Proved by reverting the gate: `an EXACT edge row (tree: false) refuses a template that merely SHARES its prefix` goes red (`Expected: false, Received: true`) while the PREFIX-row positive control and fourteen others stay green; hard-setting the vault rows' flag reddens the second-derivation test too. One mutation SURVIVES and is not weakened: hard-coding the operator rows' flag to `false` changes nothing, because no operator row declares `tree` today — the reader is load-bearing the day one does, which is what the per-edge count assertion exists to catch.
- 2026-08-23 — the two halves of the route↔consumer fence disagreed about what "addresses" means, and only one of them resolved the edge rewrites. The half that CERTIFIES a consumer has resolved them since M19 PR4; the half that RETIRES an exemption compared an `/api/…` template against a `/v1/…` route path with `templateMatchesPath` alone, where the FIRST SEGMENT never agrees — so it was blind to every consumer reaching a route through an edge, which on the vault and operator origins is every consumer there is. `EXEMPT_RESTORE_SURFACE` is the case in flight: PR1b's four routes are exempt pending PR2's screen, and that screen addresses them as `/api/vault/items/…`, so the check whose whole job is to make flipping mandatory would have gone on asserting "no product consumer" about routes the product had started calling. The rule now lives in `staleExemptions(templatesByFile, decls)` and resolves the rewrites. Proved with a PLANTED pair rather than a green suite, because no exemption on this tree is edge-addressed today and a fix whose only evidence is a green suite is a fix with no evidence: both halves are real — a route the repo serves and the `/api/…` path the vault edge maps onto it, built from the derived row — and reverting the matcher reddens `the stale sweep SEES a consumer that reaches an exempt route THROUGH an edge` (`Expected [1 entry], Received []`) while sixteen stay green, including the real sweep. The planted registry is the test's own, so the case survives PR2 flipping the real entries.
- 2026-08-23 — and the adjacent finding was REFUTED rather than fixed, which is the more useful half. The `tree: true` arm ends in `continue`, skipping the segment-wise arm that the pre-fix code gave every row whose `from` held a parameter, and that reads as a dropped capability. It is not: the vault edge matches a tree with `pathname.startsWith(r.prefix)` against a REAL request path, so a `:param` inside a tree row's prefix matches nothing at runtime — restoring the arm would model a call the edge cannot make, the same over-match class the `tree` gate exists to close. What the finding was right about is that the assumption was silent. Every `tree: true` row's `from` is now asserted to carry no parameter, so a row that grows one names itself instead of being modelled wrong; planting `{ prefix: '/api/vault/:itemId/', tree: true }` in the edge source reddens it with `"parameterised": true`.
- 2026-08-23 — the route↔consumer fence's remaining review findings, and the two that were answered by MEASURING rather than by editing. Fixed: the refusal test proved its property against a hand-built one-row table while claiming it about the fence, so it now runs against `EDGE_REWRITES` and through `fileMatchesPath` on a real file (the planted single row stays only for the disagreeing `tree: true` arm, where a table the test controls is the point); the positive control read its consumer file out of `ROUTE_CONSUMERS` instead of spelling the path a second time, and pins three fields of the row rather than all five, because a control whose job is to stay GREEN under a reverted gate must not also redden the day `EdgeRewrite` grows a field; and the three readers of each edge's `server.ts` now share one `EDGE_SOURCE_TEXT` read, since three `readFileSync` passes over one file are three chances to disagree about what was read. The `tree: false` hard-coded onto PASS_THROUGH_ROUTES and the grantee projection was justified by a comment asserting a fact about the tree — a test nobody runs — so the edge's own matcher lines are asserted now (`r.path === pathname`, `pathname === GRANTEE_CANDIDATES_PATH`, and the `r.tree ? startsWith : ===` ternary), with the comment naming WHICH layer that proves: the shape the model rests on, where the behaviour is proved a layer down by `apps/vault-web/test/server.spec.ts`. Turning the edge's pass-through matcher into a prefix reddens it, one assertion red and sixteen green.
- 2026-08-23 — and the efficiency finding against the same fence was WITHDRAWN ON MEASUREMENT, which is worth recording because the number that raised it was noise. The claim was that gating on `tree` made the exact arm split `rewrite.from` for every template pair, costing ~4% (419ms → 437ms). Both numbers were single runs; the same test on unchanged code varies 410–443ms. The fix — skipping the rewrite loop for templates outside the roots derived from the table — was written, measured at ~412ms against a ~420ms baseline, and reverted: the test is dominated by `extractTemplates`' file I/O, not by the matcher, and ten lines of derived-root machinery for 2% inside the noise band is complexity bought with nothing. A performance finding needs a repeated measurement before it is a finding at all.
- 2026-08-23 — the service half of re-collectable release would have shipped §6uu's safety argument as a FALSE statement about the product, and the argument is the reason the change is legal. §6uu justifies re-collection by saying the owner's stop stays available and one click; that is true of `emergency.service.ts` and was false of the only screen an owner has. `policyRow` gated deny on `status === 'waiting'` and revoke on `status !== 'released'`, so a released policy rendered NO controls at all — the one status where a grantee can now collect with one tap was the one status where the owner could do nothing, which is docs/03's rule inverted twice over rather than once. `granteeActions` had the mirror gap and offered collection on `waiting` alone, so the new capability had zero callers on the surface it exists for. The transferable form is the repo's own rule read in the harder direction: "ship a capability in the same change as its caller" is usually about a route nobody calls, and this is the case where the missing caller is a PROTECTIVE one and its absence is what makes the permissive change unsafe. When a design argument names a control, check that the control is RENDERED, not merely reachable.
- 2026-08-23 — hand-written status tables sat under a comment saying they were "pinned to `002_emergency_access.sql`", which is this repo's name for a test nobody runs, and the thing they failed to notice is the defect above. They are read out of the CHECK constraint now and each asserts SET EQUALITY with it before reading a button, so a status added to the constraint reddens each table by name until somebody decides what the screens do with it. The cost is stated rather than hidden: reading another package's migration makes `apps/vault-web` escape its own input set, so it gains the canonical `turbo.json` test inputs and loses fine-grained test caching — `packages/config/test/turbo-test-inputs.spec.ts` detected the escape on the first run and named the package, which is the fence working as designed rather than a surprise. Too broad re-runs a fast jsdom suite; too narrow replays a cached pass on a gate whose only input changed, and only one of those can ship a wrong answer.
- 2026-08-23 — the last things enforcing one-shot release were two TEST DOUBLES, and both suites stayed green against a server that no longer exists. `emergency-crypto.spec.ts` answered 409 `already_released` on every release after the first and `screens-emergency.spec.ts` defaulted to the same token, so a client asserting "a second release is refused" and a client asserting "a refusal does not read as success" were both proving things about a route that had changed underneath them. CLAUDE.md already says a double must be faithful about what it REFUSES and about ABSENCES, not only about values; the direction that costs here is a double refusing what the real thing ALLOWS, because the suite goes green and the capability is invisible. The crypto fake now refuses only an unarmed escrow and the screen fake defaults to `not_requested`, which is what a release with no elapsed `releases_at` actually answers.
- 2026-08-23 — the honest residual of re-collectable release is not the obvious one, and stating the obvious one would have hidden it. Re-collection gives the LEGITIMATE grantee nothing new — they already hold the master key — so the case that changes is an attacker who compromises the grantee AFTER a legitimate collection: one-shot answered `already_released` and gave them nothing, while a repeatable route rebuilds the owner's master key for them. What bounds it is that the protection given up was largely illusory, holding only against an attacker whose victim used THIS client, which wipes the recovered key and retains nothing, and nothing forces a grantee to use it. What replaces it is active rather than passive: the attacker must open the grantee's own vault by SRP, every collection re-notifies and re-audits and re-runs the settlement gate, and either stop is one owner action. A control that is spent protects nobody after it is spent. Recorded as `[ACCEPTED]` in §6yy with the weakest link named separately and owned by M30: a repeat collection carries the same notification kind as the first, so it reads identically to a duplicated delivery for the person deciding whether to press stop.
- 2026-08-23 — a search for a WORDING cannot find a CLAIM said a third way, and the sentence it missed was the most consequential one in the change. After rewriting every spelling of "release is one-shot" I could find in comments and copy, the live drive walked into `renderRelease`'s warning — that continuing spent the arrangement and could be done once — which contains neither string I had swept for. It is the last thing a grantee reads before acting in an emergency, and it told them they had a single attempt: the precise hesitation M27 PR3a exists to remove, printed on the screen where hesitation decides the outcome. The fix is paired with a ban held as DATA (eight spellings, plain substring, `src/**/*.ts` including comments), and the comments explaining the history DESCRIBE those sentences instead of quoting them — the M24 wording-fence lesson arriving at the exact point it would have bitten, because my first draft of the comment quoted all three and would have been reported as the violation. The transferable form: when a fact changes, sweep for the CLAIM by reading the screens, and then make the spellings unrepeatable as data — a sweep is a snapshot, a fence is a ratchet.
- 2026-08-23 — the owner's status vocabulary, read by the grantee, produced a sentence that was FALSE ABOUT ITS READER, and it was the entry immediately after the one I had just fixed. `describeGranteeStatus` fell through to `POLICY_STATUS_WORDS` — written for an owner reading their own arrangement — so a denied policy told the GRANTEE "stopped by you" on the very row explaining why their button had gone, and `configured` said "ready if you cannot", whose second person points at the wrong party and inverts the promise. I had just added a grantee-specific override for `released` and had not asked what else in that map was audience-specific; the answer was one line below. This is "a rule applied to one member of a category is a rule half-applied" catching me inside the change that cites it, one screen later. Only the differing entries are overridden, so there is no second copy of the map to drift, and a fourth DDL-keyed table asserts what the grantee is told on all six statuses plus that neither second-person wording reaches them on any row. General form: a shared vocabulary consumed by two AUDIENCES needs the audiences enumerated, not the entries — the wrong entry is the one nobody thought to ask about.
- 2026-08-23 — driving the escrow end to end needed two real accounts and I would not forge the third credential to get there. The pre-existing owner account's password was not mine, and the shortest path — minting a password-reset code and inserting its hash into `password_resets` — is credential forgery even against a local stack, so the sandbox refused it and the refusal was right. The path taken instead used only legitimate ceremonies: a third account created through signup, TOTP enrolled through /security, its vault built and its recovery key published through the real screens, with the roles flipped so the escrow OWNER was the account whose password I had created. The two shortcuts that remain are state construction rather than authentication — `contacts.linked_user_id` set directly (the link ceremony is a code exchange M13 owns and PR3a does not touch) and `releases_at` moved into the past (the alternative is waiting 48 hours). Worth recording because the distinction is the useful one: constructing STATE a ceremony would have produced is a drive; producing a CREDENTIAL the ceremony would have checked is not, however local the stack.
- 2026-08-23 — making `deny` admit on a released policy meant the OWNER'S STOP ERASED THE RECORD OF WHAT IT WAS STOPPING, and the fix is a timestamp rather than the refusal it would have been tempting to restore. `markDenied` writes `denied_by_owner` over `released` and clears `releases_at`, so `status` — which had been the durable record precisely BECAUSE deny used to refuse — stopped carrying the fact that the master key had been handed over. `released_at` was stored on the row and serialized by no DTO, so the owner's escrow view returned byte-identical policies for "I stopped them before anything left the server" and "I stopped them after they rebuilt my master key", whose remedies differ by an entire vault reset. The client made it concrete: its honest copy branched on `status === 'released'`, so denying and then removing a grantee who holds your master key produced "that person can no longer open this vault" — the sentence that file's own comment calls the most consequential lie this screen could tell, emitted by the control meant to handle it. Reinstating the refusal would have restored the defect the milestone exists to remove; exposing `releasedAt` (a bare timestamp, no material) keeps both the ungated stop and the after-the-fact review. The transferable rule: when a change makes an action overwrite a status, ask what that status was the ONLY record of — a guard being removed can silently delete evidence as well as a restriction.
- 2026-08-23 — anchor UI copy on the FACT, not on a status another action may overwrite, and the discriminating case is the field being ABSENT rather than false. The first draft of the fix keyed on `policy.releasedAt !== null`, which a fixture caught immediately: a service older than this origin sends no such field, and `undefined !== null` is TRUE, so every row it served would have announced a collection that never happened. `!= null` covers both spellings of absent, and the status is then the best remaining signal — and it is the RIGHT one for an old service, because deny refused on a released policy there, so its status genuinely is durable. A DTO field's type is a claim about the server, not a guarantee about the bytes; the M12 rule (never blank a row a newer service serves) has a mirror image (never invent a fact an older one omits).
- 2026-08-23 — a phrase ban is a hand-list that cannot be derived, so it must claim only what it proves, and mine claimed "in any spelling" while missing a live sentence inside its own corpus. `releaseAndRecover`'s capitalised heading — the module's ONLY description of the function that performs the collection — said the call could succeed once per arrangement, and the list banned `spent the escrow` (which matches nothing anywhere) rather than the `spends the escrow` actually written. Two independent lenses found it. The heading is now the narrow true one, the bound is stated (a fifth wording passes; reading the screens is the control for that), both inflections are banned, and the text is de-wrapped before searching — because the review also found a banned phrase quoted across a line break, green only because of where the line happened to end and one re-flow from red. De-wrapping widens what a search sees and can only find more, which is the safe direction for a filter; the M24 lesson's danger was an EXEMPTION, which narrows.
- 2026-08-23 — a vocabulary fence must read every migration, because append-only means the vocabulary can only ever widen somewhere else. The status scan read `002_emergency_access.sql`, the file that created the table — and editing that file raises `MigrationDriftError`, so the ONLY legal way to add a seventh status is a later `DROP CONSTRAINT … ADD CONSTRAINT`, which is exactly what `003_notification_kinds.sql` already does to another constraint 002 declared. The review proved the blindness by adding such a migration: all four tables stayed green. The scan now walks the directory with a floor, anchors on statements naming the table so a constraint belonging to another table cannot be mis-attributed, and takes the last definition. Writing it surfaced a second observer defect worth more than the first: these migrations carry TRAILING comments and one contains a semicolon, so splitting statements on `;` cut the CREATE TABLE in half before the CHECK and the parser returned an empty vocabulary — which without the floor would have read exactly like a table with nothing to check.
- 2026-08-23 — a table that records WHETHER a control exists cannot see the control saying the wrong thing, and the sentence it failed to protect was the one the design document calls the most consequential lie. The owner-stop table accepted either stop label by design, since both are stops; so nothing asserted that a RELEASED row gets the released-specific wording, and two mutations producing the pre-PR3a copy on a post-PR3a status survived the entire suite. The waiting arm has had that assertion since M15 — a rule applied to one member of a category, in the PR that keeps citing it. Two service-side mutations survived the same way: skipping the settlement gate on a re-collection (the window docs/03 §5.1 control 5 exists to close, correct in the shipped code and unasserted because the only gate test walks `waiting → released` and is structurally unable to see the released arm), and `collectable = true`, whose status half is carried entirely by `!releases_at` on every state the API can reach — so its fence has to FORGE the disagreeing state directly, since only the database can make those two facts disagree.
- 2026-08-24 — the residual asked for a name that DOES NOT EXIST ANYWHERE ON THIS PLATFORM, and finding that out changed the fix rather than delaying it. §6yy carried "[OWNER: M27] the grantee's row names the owner by raw account id" on the model of the owner's own screen, which reads a name out of their contact list — so the residual framed it as a cross-user disclosure decision, which name released to whom. `profile` has no display-name column at all: a person's name exists ONLY inside other users' per-user-encrypted contact rows, and the grantee has no contact row for the person who named them. So "serve the owner's name" was never a disclosure question, it was a request for a new Zone B identity field and a cross-service lookup, neither of which is a reading-surface's business. The answer shipped instead is a nullable owner-authored `label` on `emergency_access_configs`: the owner writes exactly the string their grantees read, so there is no name released that they did not choose to release and no second service in the path. Optional, falling back to the id, because a required label would make ARMING emergency access strictly harder than leaving it unarmed — the protective action carrying the extra step, which is the M6 rule that the protective action must never be harder than the permissive one, inverted. (Cited as "docs/03 §5.2's rule" when this entry was written, and that was wrong: §5.2 is three lines of emergency-access-abuse controls and contains no such rule — it lives in the §6g/§6j/§6ii deltas. Caught by an adversarial pass over the PR description, which is a late place to catch a citation nobody had opened.) The transferable form: when a residual names a fix, verify the fix's PREMISE before scheduling it; this one had been true-sounding and unchecked since it was written.
- 2026-08-24 — the Cedar gate on the grantee read CANNOT DENY, a mutation proved it, and the comment claiming otherwise was corrected rather than the mutation weakened. Deleting `assertCan` from the read path left all 62 tests green. That is not a weak test and not an unfaithful mutation — it is CLAUDE.md's third reason: `listReleasedGranteeIds` selects the owner's policies at `status='released' AND deleted_at IS NULL`, and the guard immediately above has already returned THIS row under the same two filters, so the principal is in the set by construction. Two derivations of one row cannot disagree. My first draft of the comment said the layer was "genuinely capable of denying" because the set is read from the table rather than assembled from the caller's id — true about the query and false about the consequence. The call stays for two reasons stated plainly instead of dressed up as a gate: every other read in this service consults the PEP, so the single read that did not would be an exception a reader has to discover; and it is the attachment point, since a later narrowing by item type or settlement stage is a change to `vault.cedar` and to the resource built there and to nothing else. What is now tested is what is true — that the PEP is CONSULTED once per item served, with the owner and the released grantee set on the resource — and the fixture inserts a SECOND owner item so "per item" and "per request" are distinguishable numbers. Recorded as `[ACCEPTED]` in §6zz, because a limit that lives only in a comment is a limit nobody re-reads.
- 2026-08-24 — a dedupe predicate compared a DB-SERVER timestamp against an APP-SERVER one, and the integration suite found it the loud way. `vault.read_by_grantee` fires once per collection, derived from `emergency_access_notifications.created_at >= emergency_access_policies.released_at` — no new column, and it re-arms by itself on each fresh collection where a stored flag would have needed resetting by each of the four writers that move a policy in and out of `released`. But `created_at` defaulted to `now()` while `released_at` comes from the injected clock, so a suite that simulates a 48-hour wait by jumping the clock into 2027 made the predicate read "never told" on every single read. In production the two clocks are both wall time and the bug is invisible until they skew, at which point it is a duplicate notice or a missing one depending on which way they drift. `claimNotification` now writes `created_at` explicitly from the service clock — the only writer on that table that does, and the only kind whose row is ever read back. General form: if two timestamps are ever compared, they must come from ONE clock; a `DEFAULT now()` is a second clock hiding inside a schema convenience.
- 2026-08-24 — the claim row is written INSIDE the transaction that holds the policy lock, which is the opposite of how every other notification on this table is recorded, and the difference is the whole correctness argument. `notify()` sends first and records after, which is right for the seven kinds nothing reads back. `read_by_grantee` IS read back — the row is the dedupe — so a decide-here/record-later split would put the check and the write in different transactions and the `FOR UPDATE` on the policy would protect nothing: two grantee reads racing the first collection would each find no row and each send. So the row is claimed with a null `delivered_at` under the lock and `markNotificationDelivered` fills the outcome in afterwards. The trade taken deliberately and recorded in §6zz: dedupe keys on the ATTEMPT, not the delivery, so a failed send is not retried for that collection — because delivery-keyed dedupe turns a dead channel into one message per item, and a notification storm is not a safety property.
- 2026-08-24 — `['decrypt']` is not what "read-only" means for a vault master key, and the screen that caught it was designed not to say why. The recovered escrow key is imported non-extractable with narrowed usages so the browser itself refuses to seal anything into the owner's Zone A — a stronger guarantee than "no screen offers a save button", which is also true. The first draft passed `decrypt` alone, and every item rendered as unreadable: an item blob carries a WRAPPED per-item key, so `decryptItem` calls `crypto.subtle.unwrapKey` with the master key before it decrypts anything. The diagnosis was slow because `#openBlob` deliberately refuses to distinguish a wrong key from a malformed blob — one message, so the pair is never an oracle — which is the correct design paying its cost exactly once, in development. The narrowing that matters survives: `decrypt` and `unwrapKey`, never `encrypt` or `wrapKey`. Fenced at the SOURCE and labelled as such, because the runtime property lives in a private field and reaching the `CryptoKey` to read `.usages` off it would mean cutting a test-only accessor into the one module whose whole claim is that it holds keys and hands out none.
- 2026-08-24 — PR3b absorbed PR4 because this repo's own rule left no alternative, and the docs sentence it invalidated was rewritten in the same change. docs/04 scoped PR3b as the route and PR4 as "the grantee's reading surface". Landing them apart would have shipped a Zone A cross-user read path that nothing called, into a milestone whose next PR is the security review — the exact shape of "zero-caller surfaces are this repo's largest recurring gap". It was forced from the other end too: §6yy assigns the owner-id residual to "the PR that must already answer whose vault am I reading in order to render a reading surface at all", and an owner-authored label is only checkable on a screen that renders it. PR4 is kept as a numbered row reading FOLDED INTO PR3b rather than renumbered, so every existing reference to PR5 stays true.
- 2026-08-24 — the derived route fence caught its own new members on the first run, and what it wanted was a second credential rather than an exemption. `a stranger gets one answer from EVERY policy route` reads the route table out of the controller precisely so a new `:policyId` route joins by existing, and it already carried step-up freshness "so the probe reaches the lookup instead of stopping at a 403 that says nothing about ownership". The two new routes are `VaultSessionGuard`-gated, so the probe stopped at `403 vault_locked` — not itself a leak, since it is the same answer whether the policy exists or not, but a fence that stops before the thing it tests passes for the wrong reason. It now detects the guard the same way it detects step-up and opens a REAL vault for the stranger, with a floor asserting the guard is still being detected. The set assertion is hand-listed on purpose and grew by two, which is the reviewable half of a derived fence: the derivation finds the routes, the list says which ones a human agreed to.
- 2026-08-24 — a time-travelling fixture invalidates anything time-bound it set up EARLIER, and it cost the same hour twice. The grantee-read block simulates a waiting period by jumping the clock forward three days; the vault session it had opened before the jump was expired on the other side of it, so every read answered `403 vault_locked` and looked exactly like a route that refuses released grantees. Sessions are minted last now, after the clock has stopped moving, and the one test that must move the clock mid-body re-opens explicitly. The same block also stopped inheriting `status='released'` from the `release` describe — it arrived `denied_by_owner`, because a block in between presses the stop — and drives rearm/request/elapse/release itself. Three requests, and a class of failure that reads like a broken route is gone.
- 2026-08-24 — `markRearmed` clears four columns and deliberately does not clear `released_at`, which is what made the read guard's status check load-bearing and what a mutation had to reach to prove it. Widening the guard to admit `waiting` left all 61 tests green, because the only refusal case went through `rearm`, which writes `configured`. That reads like a redundant guard. It is not: a policy that was collected, stopped, re-armed and re-requested sits at `waiting` carrying a `released_at` from the PREVIOUS collection — because M27 PR3a anchors "was this ever collected" on that field precisely because no transition clears it. A guard keyed on "has a released_at" rather than on the status would hand that grantee the owner's items while the owner's waiting period was still running, using a collection the owner had already stopped. The fixture asserts it reaches that state before asserting the refusal, because a test named for a property must exercise the boundary the property decides.
- 2026-08-24 — UNDERSTATING what a button does is the same defect as overstating it, and PR3a's fix became PR3b's bug on the same line. `renderRelease`'s second warning said reading the owner's items was not built — the honest answer while it was not, written by the PR that had just finished correcting the sentence beside it for having gone stale. PR3b builds it, so the sentence became false in the identical way, on the identical screen: the last thing a grantee reads before acting in an emergency, whose whole job is to make somebody weigh the action. The screens spec asserted `not built yet` and now asserts the opposite plus its absence, so the pair cannot rot in either direction. General form: a warning screen's copy is a claim about the code, and every PR that changes what the button does owns every sentence describing it — including the ones the previous PR wrote to be honest.
- 2026-08-24 — a source fence read the DOCUMENTATION of the rule instead of the rule, and only a mutation could tell them apart. `vault.cedar`'s two narrowings — `action == Action::"read_by_grantee"` and `resource is VaultItem` — are asserted as source so that deleting either fails at the policy as well as behaviourally. Deleting `resource is VaultItem` from the permit left the assertion GREEN, because the paragraph above the policy explains the narrowing by name and the regex matched the comment. Comments are stripped before matching now, with an anti-vacuity check that stripping did not empty the corpus. This is the same class as anchoring a fence on an identifier a caller chose rather than on what the runtime reads, arriving in a file format where the comment and the code look alike. The adjacent one, found the same way: the extractable-key fence's regex already consumed the closing bracket, so it searched for `], true` in text beginning after the `]` and could never match — mutating the argument to `true` reddened nothing.
- 2026-08-24 — the grantee's Cedar attribute is NOT called `namedBeneficiaries`, and the reason is that `loadBundledPolicies()` concatenates every `.cedar` into every service's PDP. `beneficiary.cedar` permits plain `read` on ANY resource whose `namedBeneficiaries` contains the principal, so spelling the grantee set that way would have handed every named beneficiary a Zone A vault read — docs/03 §5.5's exact prohibition — and no test of `vault.cedar` would have noticed, because the leak would be in the resource rather than in the policy. `vault.cedar` is narrowed twice for the same shared-bundle reason: by action id, and by `resource is VaultItem` so that no resource in any other service carrying an `emergencyGrantees` attribute can match it. And `vaultItemResource` OMITS the attribute when the grantee set is empty rather than passing `[]`, so an owner-side call site that never learned about grantees builds a resource the grantee policy cannot match at all — the failure direction of a mistake there is refusal.
- 2026-08-24 — the owner-authored vault label was cleared by the control that exists to set it, and the field's own hint asserted the opposite. `configure` replaces an escrow wholesale and writes `label = EXCLUDED.label`, so an ABSENT label clears the stored one — deliberate, because a name surviving into an arrangement the owner rebuilt without one would be the old escrow's name on the new escrow's grantee rows. The client then built the field with `value: ''` unconditionally and never handed `granteePicker` the `escrow.data.label` the same render already held. So an owner re-arming for any other reason — adding a second contact, changing the waiting period — silently discarded the name, every grantee row reverted to `Vault of <uuid>`, and the hint said "leave it blank and they see the id, as before", which is false exactly when there was a label before. That is §6yy's defect re-introduced by the PR closing it. The field is seeded from the current escrow now and the hint says "clear it and they see the id instead", which is true in both directions. Three tests, because the field has three arms and only the middle one was broken: typing SENDS, re-arming untouched KEEPS, blanking CLEARS. It went uncaught because the screen double rebuilt `escrow` with no `label` key at all — a double must be faithful about ABSENCES, not only values, or the write path it stands in for has no observer.
- 2026-08-24 — a lock landing mid-`collectGrant` was silently undone, and docs/03 §6zz had accepted a residual whose premise that failure destroyed. The method read the session before two awaits — a network release and a key import — and wrote the recovered owner key after them unconditionally. A `lock()` arriving in that window (the idle timer, or `pagehide` when a laptop shuts) nulled `#vault`, and the write then re-installed the OWNER's master key on a session whose status is `locked`, where `touch()` returns early and therefore never arms another idle timer. The key outlived the control that exists to drop it, until an explicit lock or page teardown — while the residual read "a grantee who walks away is protected by the idle lock". It re-checks `#vault` by IDENTITY now, which catches a lock and a lock-then-unlock alike, with no await between the check and the write. Aborting costs a collection rather than the arrangement, which is only true because PR3a made release repeatable — the earlier PR is what makes the safe answer available here. General form: `#requireOpen()` before an await proves nothing after it.
- 2026-08-24 — three grantee acts sat behind one settlement gate and produced one indistinguishable audit event. `vault.emergency.release_blocked` carried the same action id, actor, resource and detail whether the gate refused a request, a collection or (PR3b) a READ, so a reading screen refetching was byte-identical on the trail to a grantee hammering the release route — which is the probing signal the event exists to surface. Fixed the way `itemsListed` fixed the identical problem one action over: a REQUIRED `surface` argument threaded into `detail`, not a new action id, because a new id is a closed-vocabulary change an older consumer drops silently and this is a property OF the refusal rather than a different refusal. The vocabulary comment beside the member said "a release (or a request)" and was left two-of-three by this PR; it is the enumeration that rots, so it now describes the category and points at the discriminator.
- 2026-08-24 — `decryptItem` asked for `['encrypt', 'decrypt']` when unwrapping each per-item key while only ever decrypting, and one zone up that turned a stated platform guarantee into a false one. M27 PR3b hands a grantee the owner's master key as `['decrypt', 'unwrapKey']` and both the code comment and §6zz claimed "the browser itself refuses to seal anything into the owner's vault". `unwrapKey` is genuinely required to open an item, and the key that came back out of the unwrap could encrypt — so the granted key did yield keys producing valid owner ciphertext, and the only thing preventing a forged blob was that no route accepts one from a grantee. An absence of callers is not a platform guarantee, and a future grantee-facing write path reviewed against that sentence would have been reviewed against something false. Narrowed to `['decrypt']`, which costs nothing (`encryptItem` GENERATES a fresh item key and never unwraps one, so this is the only item-key unwrap in the tree) and makes the claim hold. The test observes the usages `decryptItem` actually requests rather than re-deriving the envelope format, because a reimplementation is a second copy that can agree with itself while both are wrong.
- 2026-08-24 — `decodeCursor` validated the timestamp half and not the id half, so a crafted cursor answered 500 where every other malformed cursor answered 409. `id.length === 0` was the only check, so `<valid ISO>|zzz` passed, bound against a `uuid` column, and Postgres raised 22P02 — which reaches the caller as `internal_error`: an outage's face on a client error, on a Zone A cross-user route. The existing test asserted one cursor that dies on the FIRST guard, so it said nothing about any arm after it; it is a table over five shapes now, each naming which guard it exercises. The fix is `UuidSchema.safeParse` rather than a local regex, so the cursor's idea of an id and the routes' idea of an id cannot drift apart. Pre-existing on the owner's own list routes, which is why exporting the codec for reuse was right and exporting it unexamined was not.
- 2026-08-24 — three of this PR's own tests could not observe what they were named for, and the browser drive proved the behaviour they failed to measure was correct anyway. `RE-ARMS the notice on the next collection` ran under a frozen clock, so a second collection wrote the same `released_at`, the previous notice still satisfied `created_at >= released_at`, and the test was satisfied identically by a `hasNotifiedSince` that ignored its `since` argument — confirmed by mutating exactly that and watching only this test go red. `tells the owner ONCE per collection` counted ROWS and never asserted a send, so a service that claimed the notice and never delivered it passed a test named for telling the owner. And the forbidden-button list was hand-written with two members that could never match — `'Delete'` is not a label in this app (`'Delete this item'` is, and `toContain` on an array is strict equality) and `'Save changes'` belongs to a screen the test does not inspect — while the grantee's item DETAIL screen, the one that could plausibly drift into reusing the owner's edit form, was never checked at all. Derived from `app.ts`'s own button labels now, minus an allowed set, so a new owner-side button is forbidden by existing.
- 2026-08-24 — a comment justified stopping at a source check by asserting the runtime property was unreachable, and it was wrong about the tree. `fences.spec.ts` read the `importAesKey` call for the granted key and explained that no test could reach the `CryptoKey` to read `.usages` without cutting a test-only accessor into the module. `collectGrant` RETURNS the `GrantedVault`, `masterKey` included, so the real key from the real ceremony is reachable by wrapping the prototype method in a test — no production change and no accessor. Both layers exist now and each says which it proves: the fence fails on the CALL (a refactor that imports the bytes somewhere else breaks it before any fixture notices), the runtime test fails on the KEY. This is the rule that a comment justifying an omission by asserting a fact about the tree is a test nobody runs, caught in the file whose job is to run those tests.
- 2026-08-24 — the vault origin's step-up prompt retries ONCE, and the two other origins facing the same delay both poll. `HttpSessionVerifier` positive-caches a session for 30s, so for up to one TTL after a successful step-up a peer service still answers from a cached un-elevated session; `apps/web/src/lib/step-up.ts` documents this at length and exports `STEP_UP_PROPAGATION_BUDGET_MS`, and `apps/operator-web` carries the same, each with a test asserting parity against `verifier.ts`. `apps/vault-web` has neither — and it is the origin with the most step-up-gated actions (vault open, keyset create, add item, arm emergency access, reset). Measured against the running stack rather than argued: first gated call 403, `POST /v1/auth/stepup` 200, identity immediately reporting `mfaLevel: stepup`, the single retry 403, fifteen consecutive 403s, and the elevation visible at T+30.1s — matching `DEFAULT_CACHE_TTL_MS` to the poll interval. The harness's own first draft is the positive control: it waited for an unspent TOTP window BETWEEN the probe and the step-up, put 30s inside the measured interval, and the identical retry succeeded. So the faster the user types, the likelier the failure, and the message they get — "That action needs a fresh identity check, and it was not completed" — is false. Found by driving the real app; not fixed here, because it is M13-era and the fix belongs with the other two origins' shape rather than inside a Zone A read.
- 2026-08-24 — a COVERAGE FLOOR found dead code that two reviews and a browser drive had read straight past, and removing it deleted a client failure code too. `readAsGrantee` and `release` each carried `if (policy.status === 'revoked') throw ForbiddenException({ error: 'policy_revoked' })`. Neither line is reachable: `markRevoked` is the only writer of `status='revoked'` and it sets `deleted_at` in the same statement, while `lockLiveByIdForGrantee` — the read every grantee route goes through — filters `deleted_at IS NULL`. So a revoked policy answers the uniform 404, which is the right answer anyway, being indistinguishable from "not yours". The gate that surfaced it was the vault package sitting at 93.93% against its 94% floor with exactly those two lines uncovered, and the honest reading of an uncoverable line is not a test to write. A dead branch that LOOKS like a control is worse than no branch: a reader believes revocation has its own refusal when it does not, and the next person to add a grantee route copies the pattern. Removal is behaviour-preserving by construction rather than by test — the row never arrives — and the absence is commented at the release path so it is not re-added. It cascaded one zone up: `ACCESS_STOPPED`'s sibling `ACCESS_REVOKED` in `api.ts` had been added in the same review for a token the service can no longer emit, so it went with it rather than becoming a zero-caller surface — this repo's largest recurring gap, arriving in miniature inside the PR that names it.
- 2026-08-24 — VERIFYING THE PR DESCRIPTION FOUND FIVE DEFECTS IN THE CODE, which is not what a description review is supposed to be for and is the reason this entry exists. A refute-by-default fan-out was pointed at the PR body's factual claims — seven lenses, each a named file list and a named claim set, every non-confirmation sent to a second adjudicator whose default was that the first verifier had misread. 61 claims, 49 confirmed, 12 not, and both adjudicators upheld rather than overturned. Only ONE of the six was prose-only. The transferable form: a PR description is the one artefact that states, in one place and in the author's own words, what the change is supposed to have done — so checking it against the tree checks the author's MODEL of the change, and the places the model is wrong are exactly the places the code was written from a wrong model. Every one of the five was invisible to `format`, `build`, `lint`, `typecheck` and a 76-test integration suite, because each was a true-sounding sentence beside code that did something slightly different. Cheaper than a browser drive and it found a different class.
- 2026-08-24 — `policy_revoked` was removed from TWO of FIVE dead arms, and the two were exactly the two a LINE coverage floor can see. `markRevoked` sets `deleted_at` in the same statement that writes `status='revoked'`, and both `lockLiveByIdForGrantee` and `lockLiveByIdForOwner` filter `deleted_at IS NULL`, so no route in this service can observe that status. Five guards tested for it anyway. The first pass removed the two whose `throw` was the whole statement — an uncovered LINE — and left three sitting on an `if` that executes on every call and merely never takes its branch, which a line floor cannot see and the 78% branch floor did not force. That is "a rule applied to one member of a category is a rule half-applied", committed inside the change that cites the rule, and it survived because the thing that found the first two was a coverage number rather than the argument behind it. All five are gone now, and the ABSENCE is fenced instead of commented: `revoked-is-unobservable.spec.ts` asserts the two facts that make the arms unnecessary — one writer that soft-deletes in the same statement, and every `lockLive*` lookup filtering `deleted_at IS NULL` — with the lookup set asserted as a SET so a third lookup arriving is a decision somebody records rather than a silent gap. Pinning "no `policy_revoked` appears" would have gone green for a tree that added an unfiltered lookup, which is the tree where the refusals are genuinely needed again.
- 2026-08-24 — a count of writers was wrong in FOUR places at once because it was a true statement about one column generalised to another. `releases_at` is cleared by `markDenied`, `markRearmed` and `markRevoked` — three writers, correct, and already recorded. The dedupe predicate's justification restated that as "the four writers that move a policy in and out of `released`", which is a different set: `markRearmed` never sees a released policy, because `rearm` refuses one with `already_released` before reaching the repo. The real answer is three — `markReleased` in, `markDenied` and `markRevoked` out. It was wrong in `emergency.repo.ts`, in `readAsGrantee`'s guard list, in migration `008`'s header comment and in the integration suite's own commentary, and the four disagreed with each other about whether the fourth writer moved a policy IN or OUT, which is the tell nobody read. The load-bearing fact — that `rearm` refuses a collected policy — had NO test, which is precisely why the count could be wrong everywhere and green everywhere. It has one now, placed where the fixture genuinely reaches `released` and asserting the row did not move; the guard's token is mutated to prove exactly one assertion sees it. Migration `008` was edited despite being append-only and checksummed, which is legitimate only because it is unmerged and applied on one dev box: the checksum covers comments, so this is the last moment such an edit is possible, and the whole set was re-applied to a fresh database with a clean second run to prove it.
- 2026-08-24 — the button fence's INPUT was narrower than its CLAIM, and the button it could not see is the one most likely to drift. It read `buttonEl`/`quietButton` call sites and then claimed "a new owner-side button joins this set the moment it is written". This client makes buttons three other ways: `policyRow`'s `act()` helper forwards a label to `quietButton` as a VARIABLE, and nine labels are built with a raw `el('button', …, ['…'])`, including the ternary `existing ? 'Save changes' : 'Add to vault'`. So `'Save changes'` — the owner's edit-form submit, the single most plausible thing to reappear on a grantee's item detail screen — was outside the corpus, which is also why the hand-written list this derivation replaced could NAME it and never match it. The corpus is all four forms now (37 labels, 26 forbidden after the allowed set) with an anti-vacuity pin PER FORM, because a floor on the total cannot see one matcher of three stop while the other two hold the count up. Proven both ways, which is the half that matters: rendering `'Save changes'` on the grantee detail screen passes 65/65 against the old corpus and reddens a named assertion against the new one.
- 2026-08-24 — two comments asserted facts about the tree that were false, in the review that exists to catch exactly that. One said the malformed-cursor table "names WHICH guard each row exercises": `decodeCursor` has three refusal predicates and the table has five rows, so two pairs necessarily double up, and `'not-a-cursor'` does not reach an encoding check at all because `Buffer.from(…, 'base64url')` is LENIENT — it returns mojibake with no `|` in it and dies on the separator guard. The other attributed "the protective action must never be harder than the permissive one" to docs/03 §5.2, which is three lines of emergency-access controls and states no such rule; it is the M6 rule, living in the §6g/§6j/§6ii deltas. §5.2 was cited correctly two lines away for the one-tap-deny control it DOES list, which is how the wrong one read as right. Both corrected, and the cursor mapping was verified by running the five inputs through the predicates rather than by reasoning about them — the same discipline the comment was violating.
- 2026-08-24 — `jest -t` IS A BROKEN OBSERVER on `emergency.int.spec.ts`, and it nearly bought a false mutation proof. Trying to isolate one test from a cascading mutation, `-t "…"` showed the target red and the positive control ALSO red — which reads as "this mutation breaks everything", the exact signal CLAUDE.md says to distrust. Running the same filter on the UNMUTATED tree showed both red as well: the describes here share state built by a `beforeAll` plus the tests that precede them, and `-t` skips the state-builders while still running the block. So the filter cannot be used to isolate anything in this file, and the honest proof is a full-file run with a mutation chosen NOT to cascade — changing the refusal's token rather than removing the guard, so no state moves and exactly one assertion can see it. General form: a test selector is part of the harness, and a harness that changes what the fixture built is a measurement device that changes the measurement.
- 2026-08-24 — the waiting-period field was the label's defect ONE LINE DOWN, and driving the app is what found it. `granteePicker` builds two adjacent fields; the review seeded the first from the current escrow and left the second at the `value: '48'` it has carried since M15 PR3. So every re-arm rewrote the owner's chosen waiting period to 48, and the drive caught it the loud way: armed at 24, re-armed only to re-confirm a key, and the new row read 48. THE OBSERVED DIRECTION IS THE HARMLESS ONE and that is the trap — 24→48 lengthens the window and reads as a non-event. `WAITING_PERIOD_HOURS` admits 24..8760 and nothing preserves a stored value, so an owner who deliberately chose 72 hours, or a year, has the docs/03 §5.2 protective window SHORTENED to 48 by a re-arm about something else entirely. A protective control weakened as a side effect of an unrelated control is precisely what this form exists to prevent. Seeded from `policies[0].waitingPeriodHours` — one value per escrow because `configure` sends one — with 48 kept as the never-armed default. Three tests, driven in the LOSING direction (72, not 24) because a fixture that only ever lengthens the window would pass against a field that silently clamps upward. This is "a rule applied to one member of a category is a rule half-applied" for the second time in one milestone, both times committed by the change that cites the rule: the first was `policy_revoked` on two of five guards, this is one of two fields. The category that keeps escaping is "the sibling in the same construct", and the thing that caught both was looking at something OTHER than the diff — a coverage floor, then a browser.
- 2026-08-24 — the §6 section bound went to THREE letters because §6zz is the last two-letter section there is, and the extension had to reach six copies of one number. M27 PR5 writes the next delta, which is §6aaa, and would have landed on the identical wall §6aa hit in M21 PR2. The bound is spelled out six times in `threat-model-residuals.spec.ts` — the parser, the lead-in classification scan, the partition total, the outside-§6 scan, and twice as a `CLOSED: §` citation — and the copies are separate ON PURPOSE, because an independently re-derived scan turns a break in the parser into a DISAGREEMENT rather than a shared blind spot. Independence in the traversal is worth having; independence in the letter bound is how the one-letter era lasted, since every copy was wrong together and nothing compared them. So a new assertion derives the bound from this file's own source and requires the copies to agree, with a floor at that level too: five could be renamed out of existence and a set of one distinct value would still be a set of one. The second half is the one with teeth, and it was not the reason the test was written. `the parser sees every §6 delta the FILE contains` compares the parser against an "independently permissive read" — but nothing MADE that read permissive except an author's care, and widening the parser to `[a-z]*` turns that comparison into a tautology that passes on every possible file while reading exactly like the fence that caught the one-letter regex. Mutating the parser to `[a-z]*` was proved to leave that guard GREEN and redden only the new assertion. Three arms in all: one copy left behind at two reddens only the new test (nothing else in the file can see a half-applied extension), the vacuity mutation reddens only the new test, and extending all seven spellings COHERENTLY to four keeps all sixteen green — the positive control, without which "every mutation went red" is equally consistent with a fence that fails on any edit. The extension itself was proved against a probe §6aaa section carrying an untagged residual and a bogus `[OWNER: M99]`: at bound two both stayed GREEN and the heading guard alone fired, at bound three the tag, owner and per-section-floor assertions fire and the heading guard passes — the section moved from invisible to judged. The probe also reproduced the mis-attribution class on the old bound, where the untagged bullet reddened `a residual written OUTSIDE §6` and named the wrong place to look. Two queue rows were added in the same change, since the owner vocabulary derives from docs/04 in both directions and refuses a tag naming a row that does not exist: M42 for cross-user free text, M43 for one derivation per closed vocabulary.
- 2026-08-24 — a hand-kept list's per-entry comments SLID one row when an entry was appended, and the comment is the entire reason the list is hand-kept. `OWNERS` carries a reason per member so that a wrong owner is reviewable in a diff — the membership itself is derived from docs/04's tables in both directions, so the comments are the only part a human writes. M41 was appended to the list carrying `// residual ownership re-sweep (added M27 PR0)`, which is M40's title AND M40's provenance; M40 was left with no comment at all. Nothing could catch it: the derivation compares ids, and a comment is invisible to it, so both entries read as correct forever. Realigned against docs/04's own row titles. The general form is that a fence which derives the DATA and leaves the ANNOTATION hand-written has moved the rot rather than removed it, and the annotation is where the reader's judgement actually happens.
- 2026-08-24 — the §5.2 waiting period reached the grantee as "Something went wrong. Try again.", and it is reachable with no race at all. M27 PR3b named four refusals on the grantee's READING screen and left the RELEASE screen's two, one route over — the milestone's own recurring shape. `waiting_period_active` fell off the end of `failureFor`'s 403 block to UNKNOWN; `not_requested` fell to CONFLICT's "This item changed since you opened it. Reload and try again", an ITEM-shaped sentence on a screen holding an arrangement, naming a remedy this SPA does not have. The reachability is the part that matters and it was found by driving: `collectable()` offers the "Open the vault" button when the BROWSER's clock says the period elapsed, while the service refuses on the SERVER's, so ordinary forward skew offers the button early and the advice "try again" is the one instruction that cannot work until the clock moves. Three more fell out of the same sweep: `denied_by_owner` is one stop the service spells at TWO statuses (403 from `release`/`readAsGrantee`, 409 from `request` via `blockReason`) and was matched only inside the 403 block, so the sticky refusal rendered as "reload"; `POST /api/auth/stepup` answers 429 and `failureFor` had no 429 branch, though apps/bff added a status-keyed one for this exact defect in M19; and `NOTIFICATIONS_UNAVAILABLE`'s copy named "arming emergency access" while `request` throws it at a grantee who is arming nothing. The NOT_FOUND override PR3b wrote for one screen became ONE helper used by four rather than four pasted ternaries.
- 2026-08-24 — the fence that would have caught all of it was written, and it caught two defects in ITSELF first. `copy.spec.ts` derives the `ApiFailure` union from its own declaration and the service's error tokens from the service's own source, and requires every token to be either named in `failureFor` or recorded with a reason — no third state. Its first draft parsed the union from RAW source and `indexOf(';')` stopped at a semicolon inside the union's own documentation, deriving 12 of 21 members; a floor caught that. Its second scanned three NAMED service files and missed `http-error.filter.ts`, `vault-session.guard.ts` and `schemas.ts`, where `internal_error` and `invalid_request` actually live — so the fence written to catch a corpus narrower than its claim shipped with a corpus narrower than its claim, and what caught it was its own stale-exemption assertion, the direction most fences do not have. The pattern was LIFTED rather than invented: `apps/operator-web/test/copy.spec.ts` already did this, on the other client of the same routes, which is the same rule half-applied across origins.
- 2026-08-24 — a Cedar call that could never deny anybody was DELETED rather than repointed, and the guard its sibling already had was added. `granteePublicKey` ran `assertCan(actor, 'read', vaultResource(actor))` and then read a DIFFERENT user's key. That is not merely aimed at the wrong entity: `owner.cedar` permits any action when `resource.owner == principal`, there is no `forbid` in the bundle, and `authz.spec.ts` pins exactly that for all nine actions — so the call was a tautology, and a dead branch that LOOKS like a control is worse than no branch because the next cross-user read copies it. `GET /v1/vault/recovery-key/:granteeUserId` carried no `VaultSessionGuard` while `GET /v1/vault/recovery-key`, earlier in the same controller, carried one and explained in its docstring why. What was exposed is not the key — it is published so strangers can seal to it, and its private half is wrapped under the target's master key — but the EXISTENCE answer beside it, which `app.ts` tells the owner is not something they should be able to probe. Not repointed because no Cedar action expresses "may read another user's published key", and inventing one to make a gate look present is the same defect wearing a policy file. Deleting the call orphaned its parameter and would have failed `noUnusedParameters` — a typecheck the jest run would not have run — so the service and controller signatures moved with it. The README's Gates column said "caller" for BOTH recovery-key rows; the first was already wrong before this PR, because `readme-routes.spec.ts` derives the ROUTE column and not the gates.
- 2026-08-24 — `revoked-is-unobservable.spec.ts` claimed EVERY live-policy lookup and inspected the two whose METHOD NAME began `lockLive`. `emergency_access_policies` has five readers; three escaped, and the two that project `status` straight onto the wire were among the escapees while the two inside only fed internal guards. All five filter the tombstone today, so the claim was true and only its input was narrow — which is exactly why it was invisible. Demonstrated rather than argued: adding an unfiltered sixth reader left the old fence green at 6/6. The corpus is now the SQL the runtime sends, over the whole `src` directory rather than one named file, keyed on the PREDICATE rather than the name — so a rename moves nothing and a sixth reader is a decision somebody records in `EXPECTED_POLICY_READERS`. It is alias-aware, and that is not decoration: `listByGrantee` joins `emergency_access_configs`, so a bare /deleted_at IS NULL/ would be satisfied by a tombstone belonging to the JOINED table — it passes for the right reason today, which is precisely why a bare test would go on passing after that changed. A named CONTROL proves the alias handling on fixtures rather than trusting it. Two comments in `emergency.service.ts` asserted "the two reads every policy route goes through", which is false: `describe` reads through `listByOwner` and `grantedToMe` through `listByGrantee`.
- 2026-08-24 — the version history showed two rows identical to the character, holding different secrets, and one restore is enough to cause it. `captureTime` rendered `YYYY-MM-DD HH:MM`. The drive produced the collision on the first attempt without trying to: edit an item, put the previous version back, and the restore captures the image it replaced — 26 seconds later, one label, both offering "Put this version back". docs/03's residual rates this recoverable "because the restore is itself captured" and describes the screen as listing "when each image was captured"; both halves assume the WHEN discriminates, and at minute resolution it does not, so the recovery path is ambiguous for the same reason. The gap in the tests was structural rather than an oversight: the existing assertion pinned that one instant renders the same however it is SPELLED, and an equivalence is only half a specification — `x renders like y` is satisfied by a function that renders EVERYTHING alike, and the anti-vacuity regex beside it was satisfied by that function too. A discrimination assertion is what closes it, and it needed its own mutation to prove: reverting to minutes is caught by the shape regex, so a second mutation keeping the format and pinning seconds to '00' was required to show the `not.toBe` does any work at all.
- 2026-08-24 — the residual-ownership sweep found the machinery mis-describing itself in five places, none of them a tag. Two documents asserted "M21's eighteen and M24's residuals" are invisible to any derivation; M24 owns ZERO `[OWNER: M24]` tags, so a row scoping a residual-ownership sweep was miscounting one of the two milestones it named. `'6ww': 3` sat below its actual four, and a floor-per-section exists precisely because a total cannot see a deletion — one that lags cannot either. The M26 row counted nine tags "across §§6ll–6pp" when the nine span §§6kk–6pp; the count was right and the range excluded one. Two prose residuals had been falsified by M27 itself and left standing: one said this client "cannot yet" read a released owner's items, in the same `app.ts` that had shipped `renderGrantedItems`, and one was "tagged and owned by M27" when M27 PR2 had closed it. Both are prose rather than tagged bullets, which is why the fence could not see them — the exact class §6uu says it cannot see. And `OWNERS`' per-entry comments had SLID one row when M41 was appended, so M41 carried M40's title and M40's provenance while M40 carried none; the membership is derived from docs/04 in both directions, so the comments are the only part a human writes and the only part nothing checks.
- 2026-08-24 — a conditional was COPIED between two clients without the mapping it depended on, and that is the shape worth remembering rather than the copy. `apps/vault-extension`'s step-up screen answered a rejected authenticator code with PAIRING copy — "create a new one in Estate under Security", about a number that is read off an authenticator and cannot be created — while telling a device whose pairing had genuinely been revoked to "try the current one". The branch was correct where it came from: `apps/vault-web/src/client/stepup.ts` keys the TOTP sentence on `UNAUTHENTICATED`, and on that origin `failureFor` maps EVERY 401 to it. The extension's `failureFor` splits 401 into `invalid_code`, `srp_failed` and the rest, so the identical expression names a different failure in the file it was pasted into. It type-checks perfectly and means something else — the discriminant rule this repo already states for narrowed union types, arriving on an error surface. The fix is one token in the condition; the finding is that a copied conditional inherits a dependency on the mapping around it, and nothing in the language or the test suite was watching that dependency.
- 2026-08-24 — identity ANTICIPATED this defect in a comment and the client committed it anyway, in the opposite direction. `refuseStepUpForRate` throws 429 `too_many_attempts` rather than `invalid_code` and says why: "never `invalid_code`, which already means 'that code was wrong' and would send a user to re-read an authenticator when the remedy is to wait (the M12 lesson about one token changing meaning with the surface)". The extension had no 429 branch, so the cap fell to `UNKNOWN` — "Something went wrong. Please try again in a moment." — putting an outage's face on a control and inviting exactly the retry the cap was refusing. A producer stating a distinction in prose does not make any consumer honour it. `apps/bff` took the member at M19 PR4, `apps/operator-web` at M21 PR3a and `apps/vault-web` at M27 PR5 — one PR before this one; the extension was the fourth client on this ceremony and the one nobody came back to. Four PRs spread across nine milestones to finish applying one rule to one category. The first draft of this entry dated the first two to M17, because bff's comment says "once M17's per-session or per-account bound" — but when the BOUND was added is a different fact from when a client learned to word it, and `git log -S` is what tells them apart. An adversarial lens caught it; the census was the load-bearing claim of the entry and it was wrong.
- 2026-08-24 — the replaced test was green on a FAITHFUL fixture that exercised the wrong boundary, which is a failure mode distinct from the invented-enum one. `names the CODE when a step-up is refused, never a password` answered `401 unauthorized` — a real token, thrown by identity's `SessionGuard` — but `unauthorized` is what a DEAD PAIRING looks like, not a rejected code. So the test named for one property asserted on the branch of another and passed while the screen was wrong about both. The first read of this called the fixture invented; checking identity's source rather than trusting that read is what turned a wrong diagnosis into the right one. Pinning a fixture to the service's vocabulary is necessary and not sufficient — it must also be the member of that vocabulary the property under test is about.
- 2026-08-24 — the discrimination fence is blind to a SWAP, and it says so rather than implying otherwise. Driving every refusal the step-up route can answer and asserting the sentences are pairwise distinct catches a COLLAPSE — two refusals sharing one sentence — and was measured to catch exactly that and nothing else. It stays GREEN when the discriminator is reverted, because a permutation preserves distinctness: three wrong sentences are still three different ones. Two per-refusal assertions carry that half. Recorded because "every refusal renders its own sentence" reads like a complete specification and is not one, and the next person to write this fence elsewhere will reach for the pairwise check first.
- 2026-08-24 — the fence written to catch a corpus narrower than its claim shipped with a corpus narrower than its claim, twice, and an adversarial lens found both. Its "every refusal is classified" check inspected only status 400, so a new refusal falling through to "Something went wrong" would have been DISTINCT from the other three and passed the pairwise comparison while saying nothing. And its walk over identity's source had two blind spots: a `this.X(` call resolving to no method body is indistinguishable from a method with no throws — the walk steps over a subtree and reports a shorter, plausible answer — while the parser reads only `throw new X({ error: … })` and the very file it walks spells seven refusals as `throw invalidCredentials()`. Neither was reachable from `stepUp` today, which is exactly why neither would have been noticed. Both now turn the fence RED and name the offending method, proved by mutation. THE GENERAL RULE, third time in this repo: the fence that enforces a rule is the code least likely to be read as subject to it.
- 2026-08-24 — a review lens asked whether the new fall-through's PRECONDITION holds and found a defect older and wider than the change. `messages.UNAUTHENTICATED` says the device has been disconnected; on the step-up route that code also arrives when an access token has merely aged past its fifteen minutes, because `vault-screens.ts` takes a raw bearer and never routes through `withSession` — at all eight of its call sites, not just the new one. So the vault half of the popup tells a live pairing it is gone. It PREDATES this change, which only made the eighth surface consistent with the other seven; the previous copy there was wrong differently. Recorded as a residual rather than fixed, because routing eight sites through `withSession` means threading a refreshed session back into popup state and is its own change. Worth keeping as a pattern: "is the precondition of the copy you fell through to actually true for this caller" found more than "is this branch correct" did.
- 2026-08-24 — the PR-description check found a false claim that was already COMMITTED, and it was a claim about the defect's own provenance. §6aaa opened by saying the defect "existed nowhere in this tree, only in a pull-request body". `gh pr view 164` says otherwise: #164's body never mentions the browser extension — its one occurrence of the word is "half-applied extension", about widening a regex bound. The finding came out of PR5's review fan-out and was written down NOWHERE, which is worse than the claim and is the fact worth recording. Fourth consecutive milestone where drafting the description and then checking each sentence against the tree found something; the new part is that the sentence had already survived a commit, so the check is not redundant with the pre-commit review — it asks a different question. THE PATTERN THIS EXPOSES: provenance claims feel like memory rather than assertions, so they get written without a source. "It exists only in X" is a claim about X, and `gh pr view` or `git log -S` settles it in one command. See the M27 PR5 entry above for the technique itself.
- 2026-08-25 — a browser drive is only evidence if the defect can reach the assertion, and two versions of this one could not. Driving M44 PR2 in real Chrome first seeded the popup with an ALREADY-expired bearer: worthless, because `refreshStatus` refreshes and stores at mount before the vault half is mounted, so the snapshot under test was fresh and the reverted defect passed 13/13. The expiry has to happen UNDER an open popup, since `vaultMounted` guards the remount and that is what strands the captured value. The second version probed with the `Lock` button, which renders "Vault locked" without reading the call's outcome — a probe whose screen says the same thing either way. The probe is now the unlock, whose result is rendered. Also discounted: a mutation that did not COMPILE, so the harness ran the previous package and reported a failure that was a stale artifact rather than the mutation. Three ways to get a green (or a red) that means nothing, in one afternoon, on the one check that exists to prove the fix.
- 2026-08-25 — when a stale VALUE is the defect, replace it with a CAPABILITY rather than refreshing it at each use. The extension's vault half took `bearer: string` at mount and spent it for the life of the popup; past the access token's fifteen minutes every screen told the user their device was disconnected, about a pairing that was alive, and named re-pairing — a step-up on the app origin — as the remedy. Rejected: wrapping each of the eight call sites in a refresh, which is eight places to get one thing wrong and leaves a ninth site free to reintroduce it. Chosen: `VaultScreensDeps` declares `call`, the module holds no credential, and there is nothing left there to hold a stale one. The general rule this instance teaches: when every consumer of a value is correct and the result is still wrong, the defect is the type, not the call sites.
- 2026-08-25 — refreshing belongs to whoever owns the session, not to whoever makes the call. `withSession` takes a `PairedSession` and a refresh ROTATES it (`toSession` requires a `refreshToken` in the response), so calling it inside `vault-screens` would have refreshed correctly and left `popup.ts` holding the previous session — right in storage, wrong in the tab, and spending a rotated-away refresh token on the next call. The capability is therefore injected by the popup. Applying that reading to the rest of the file found the SAME snapshot on the disconnect button, which closed over the session its draw was handed; `disconnect` reads a refused refresh as "already gone" and forgets locally, so the stale capture would have produced a browser that looks signed out over a live server session — the outcome its own comment calls the worst there is. A rule applied to one member of a category is a rule half-applied.
- 2026-08-25 — a disjunct that cannot fire is deleted, and the claim that it cannot fire becomes a fence rather than a sentence. `withSession` treated a refused refresh as revocation on `UNAUTHENTICATED` or `INVALID_CODE`; `POST /v1/auth/refresh` throws only `invalid_token`. docs/03 §6aaa had recorded it as harmless and named the real risk precisely — "the failure mode of a later edit is somebody making it true" — which is an argument for a test, not for leaving it. The fence walks the route out of identity's controller and service, then drives each refusal through the REAL `refresh()` over a transport double and asserts every disjunct is a code that measurement produced. Measured, not mirrored: `failureFor` is module-private and a re-implementation of it here would be a second copy that drifts.
- 2026-08-25 — a mutation SURVIVED and was answered with a test, which is the branch of that rule this repo takes least often. Deleting `rotateSession(current)` left all eleven popup cases green; the honest reading was a weak test, not a harmless line, because the rotation is what keeps the disconnect path from spending a dead token. The new case drives the rotation through the VAULT half on purpose — `refreshStatus` re-renders through `show()` and would carry the new session anyway, so a test written around it passes with the line deleted, which is precisely how the gap survived. Also worth recording: the structural fence and the behavioural cases catch DIFFERENT mutations. A capture-and-reuse-bare mutation is caught by the fence and missed by the behaviour, because the captured value happens to be fresh at that instant.
- 2026-08-25 — the same defect in two clients needed OPPOSITE fixes, and the reason is REACH rather than taste. `apps/vault-extension` and `apps/vault-web` both answered a refused step-up code with the wrong sentence. M27 PR6 fixed the extension by re-keying the prompt's special case onto `INVALID_CODE`; M44 PR1 fixed the vault origin by DELETING the special case and letting `messageFor` own the sentence. Neither fix works in the other file. The extension's `invalid_code` serves TWO ceremonies — a refused pairing code and a refused authenticator code — so one map entry cannot serve both and the call site, which is the only thing that knows which ceremony is running, has to choose. This origin's serves ONE, because `POST /api/auth/stepup` is the only route it calls that identity answers `invalid_code` on, so the map can own it and a discriminator that could be inherited wrongly stops existing. THE RULE: before copying a fix between two clients of one service, ask how far the token REACHES in each. Prefer the ABSENCE to the branch wherever reach allows it.
- 2026-08-25 — a test was green for TWO independent wrong reasons at once, which is worth naming because either alone is a known shape and the pair is what made it invisible. `says what THIS form holds when a code is refused` drove the vault origin's step-up prompt with `401 invalid_credentials` — the LOGIN token, which that route never sends — and asserted `/codes last about 30 seconds/i` against `document.body.textContent`. The prompt's own INPUT HINT reads "From your authenticator app. Codes last about 30 seconds." and is in the DOM from first render, so the assertion matched the hint rather than the refusal. Measured both ways: it stays green when the fixture is pointed at a 503 rendering an unrelated sentence, and it stayed green when the branch it existed to protect was deleted outright. A fixture aimed at the wrong boundary is one defect; an assertion whose corpus is wider than its claim is another; together they produce a test that cannot fail about a thing it does not test. The refusal assertions now read `p.status-error`, and a named CONTROL asserts the hint is present before any refusal so the reason for that scoping cannot rot.
- 2026-08-25 — the copy fence claimed "every service refusal is classified" while reading ONE of the two services this origin relays. `apps/vault-web` proxies three identity routes, so `invalid_code`, `unauthorized` and `too_many_attempts` are refusals it can put in front of a user, and none of them was in a corpus scoped to `apps/services/vault/src`. The upstream set is now derived from the runtime's own `PROXY_ROUTES` table so a fourth upstream turns it red. Two smaller things fell out. Tokens answered by STATUS rather than by name needed a declared category of their own: `mappedTokens()` reads `token === '…'` tests, so a refusal handled correctly by a 429 branch was indistinguishable from one nobody classified, and excusing it as unreachable would have been false. And the premise under most exemptions — "that ceremony is not proxied here" — is now itself asserted against the route table, because an unchecked reason is prose.
- 2026-08-25 — a fence that names DISTINCTNESS must compare the things that differ, not the labels beside them. `copy.spec.ts`'s "every failure code has a sentence of its own" derived `case` labels and threw the sentences away, so nothing in it could see two codes returning identical English — the precise defect the file was written about. Proved by restoring it: the pre-PR behaviour passed all 325 tests the package held at that point in the branch — including a case named "the two that share are the two that mean one thing", whose comment named the sharing pair in prose because nothing in the file could compute it. The parser now keys code to sentence and resolves fall-through groups, so a deliberate pair (NETWORK/UNAVAILABLE) has to be DECLARED with its reason and a stale declaration reddens too. The general shape: a set-membership assertion over names is satisfied by any mapping, including the constant one, and reads exactly like a distinctness check.
- 2026-08-25 — when a fence cannot READ something, count it rather than widening the pattern. `copy.spec.ts` scans `error: '<literal>'` and so was blind to `throw new ConflictException({ error: outcome.blocked })`; `already_waiting` is returned by `blockReason` and spelled by no `error:` literal anywhere, so it sat outside a corpus the file claims is complete. Rejected: a regex that follows values through variables, which is a parser wearing a regex's clothes and fails silently at the next indirection. Chosen: derive the SITES whose token is not a literal, declare what each can produce, assert both directions. A new computed throw reddens as an undeclared site; a site that stops computing reddens as a stale key. Classifying the four newly visible tokens also caught a stale exemption, which is the usual second yield of making something visible.
- 2026-08-25 — a DERIVATION and an OBSERVATION are different evidence, and the repo had only the first about the route two clients both got wrong. M27 PR6's fence walks identity's source to learn what `POST /v1/auth/stepup` can answer, and `copy.spec.ts` classifies that vocabulary — but no test ASSERTED the token identity actually sends, and M27 PR6 shipped saying something stronger and false. The repo did hold the observation, twice, and neither instance carried the discriminating detail: `apps/bff/test/identity-client.spec.ts` records in PROSE that "five wrong codes at `POST /v1/auth/stepup` answer 401 `invalid_code`, the sixth answers 429 `too_many_attempts`" — a manual M19 drive nobody re-runs, beside a test whose 429 is stubbed; and `identity.int.spec.ts`'s "rejects step-up with a wrong code" really does drive the running service, then asserts `res.status` and never looks at `res.body.error`. So the wire had been seen and the TOKEN had not been checked, which is exactly the bit both clients got wrong. Recorded because the false version was the more flattering one — "nobody had ever looked" excuses a defect that "two records looked and neither wrote down the load-bearing half" does not. `apps/e2e/test/vault.e2e.spec.ts` now drives the real ceremony (register, enrol TOTP, verify) and records the wire: `401 unauthorized` with no session, `401 invalid_code` for a wrong one, `429 too_many_attempts` at the cap, with every answer before the cap asserted to still be `invalid_code` and a floor proving the cap did not fire immediately. It confirms the derivation rather than contradicting it — which is the outcome that makes the technique worth keeping, since the whole defect in both clients was being confidently wrong about what this route says.
- 2026-08-25 — a column that is ALLOWED to say nothing cannot be repaired by a better parser, so the answer was made mandatory instead. `threat-model-residuals.spec.ts` refuses a residual owned by a milestone that has shipped, and derived "has shipped" from the Status cell of docs/04's queue table — free prose. Four of the twenty-four rows could not be read as a lifecycle, and THREE OF THE FOUR had finished: M24 said `**APPROVED 2026-08-21, section below.**` with PR0–PR4 shipped, M27 said `**SCOPED 2026-08-22, section below.**` with every PR shipped, and M44 said "… and the row is now complete." inside a bold run in lower case, which the case-sensitive scan did not match. Only M21's silence was honest. The first two are the ones that settle the design: no amount of regex quality finds a completion word in `APPROVED` or `SCOPED`, because the word is not there — the failure was that a row was PERMITTED to answer with something else, and a permission is not a parsing problem. Every row now opens its Status cell with exactly one token from a closed vocabulary — `**PLANNED.**`, `**IN PROGRESS.**`, `**COMPLETE.**` — as a bold run that is the token and nothing else, narrative following in the same cell, and a row carrying no token reddens by name. Each declared token must also be carried by at least one row, so a parser that quietly stopped seeing two of the three cannot pass on the strength of the third. Applying the rule moved the derived completed set from three to six. None of the three newly visible milestones owns a residual, so the stale count is unchanged at thirteen — the hole was load-bearing for the NEXT tag, not for any tag that existed. Worth noting what the old anti-vacuity floor was doing: `MIN_COMPLETED_MILESTONES` sat at exactly 3 against a derived set of exactly 3, so the guard against a broken derivation was pinned to the broken derivation's own output and had no headroom to notice.
- 2026-08-25 — the fence asserts which TABLE its column index belongs to, because docs/04 has two tables and the other one's third cell answers a different question. The queue table is `| # | Milestone | Status |`; the escalations table immediately above it is `| | Item | Blocker |`. (An earlier draft of this entry said "six lines above" in three places, and the gap was eighteen — widened by this very PR, which expanded the paragraph between them. A line count between two things in a growing document is the second-copy-that-rots defect in miniature, so the number is gone rather than corrected.) Reading "cell 3" without checking where you landed is how a fence comes to assert that a blocker is a lifecycle, which is the narrower-input-than-claim shape this repo keeps arriving at. Both headers are now asserted, each to be unique, each table's third column NAMED out of the document (`Status` for the queue, `Blocker` for the escalations), and every milestone row asserted to sit below the queue header — the fence stating its own reach as a mechanism rather than as a sentence in a docstring. The escalations half is left deliberately undone and recorded as a residual: `[OWNER: E1]` carries twenty tags and `[OWNER: E4]` one, but an escalation that clears makes its residuals RE-OWNABLE rather than closed, so copying the milestone vocabulary across would be the fix that looks total and is not.
- 2026-08-25 — two cross-checks that would have made the declared status TRUE rather than merely present were built against the tree and REJECTED on measurement. "A COMPLETE row's PR-split bullets must all say SHIPPED": seventeen of docs/04's forty `- **PR…` bullets carry no shipped marker of any kind, and M27's PR0, PR1a, PR3b, PR5 and PR6 are five of the seventeen — all five shipped. "A PLANNED row must have no per-PR record section": only M21, M24 and M25 use `#### M<nn> PR…` headings, three of the seven milestones that have shipped a PR, and M22 and M23 carry no `####` heading whatsoever. Either would have been a fence green for exactly the reason it is wrong. Both rejections and their numbers live in the fence, not only here, so the next author does not pay to re-derive them — and the bound they leave is stated as an `[ACCEPTED]` residual rather than left implied: a token says which stage the work is at, not that the stage is true. THE GENERAL SHAPE: rejecting a mechanism is a result worth writing down at the same weight as adopting one, because the alternative is the next person building it and discovering the same seventeen.
- 2026-08-25 — the M40 row has now been wrong about a milestone it names TWICE, and the second error was the correction to the first. M27 PR5 struck M24 from "M21's eighteen and M24's" because M24 owns zero residual tags. The replacement sentence said M21's eighteen were invisible "because that row says APPROVED and never gained a completion status — which is the same stale-prose defect the M23 row already records about itself". M21 has not shipped: docs/04 records its PR5 — documents evidence content plus the legal-hold lift ceremony — as NOT YET SHIPPED, and no commit names it. So M21's silence was honest and the defect is the OPPOSITE of M23's: M23's row described a FINISHED milestone as in progress, and M21's described a LIVE one by saying nothing. The eighteen are live debt on a live owner and correctly sit outside the stale count — they remain a defect for a different reason, since M21's one outstanding PR does none of them, which is a later PR's adjudication. Recorded as a pattern rather than a slip: both errors are in a row whose SUBJECT is ownership accuracy, both survived review, and both were the kind of claim that reads as recall rather than as an assertion needing a source. The row's own residual at docs/03 §6uu carried the same sentence and has been corrected with it.
- 2026-08-25 — the same re-sequencing rot the M21 section congratulates itself for avoiding had happened one document over, and was found by asking whether M21 was finished. docs/03 §4 said the legal-hold lift ceremony was owed by "(**M21 PR4**)". docs/04 records the 2026-08-19 re-sequence explicitly — PR4 became the security review, the lift ceremony moved to PR5 — under the banner "Recorded rather than silently absorbed — a plan that quietly re-aims is a plan nobody can audit". The plan was recorded; the threat model was not. The same paragraph in docs/03 already carries one correction of exactly this class, about **M21 PR3b**, so this is the second instance in one paragraph. No fence can see it: the residual tag regex is line-initial and its owner capture cannot even express a PR-level owner like "M21 PR4", and the sentence is inline §4 prose outside §6. Recorded because the lesson is not "update both documents" but that a milestone re-aiming its PR numbers invalidates every sentence elsewhere that cites one, and nothing in this repo derives those citations.
- 2026-08-25 — a catch-all owner tag is not an argument, and this one disagreed with its own bullet. docs/03 §6e recorded the documents 404-vs-403 existence oracle under `[OWNER: M21]`; §6vv recorded the identical shape on `plaid` under `[OWNER: M41]`, whose queue row is named for the sweep. One category, two records, nothing connecting them, so whoever opened either saw half the work. The M21 tag never had a case behind it: M21's scope statement excludes other services' guard surfaces, and the §6e bullet's own last clause said the real fix belongs in the service. It is the shape §6r names — an item assigned to "whichever milestone next touches them" — which is exactly how an item goes uncosted, and it is why M40 forbids bulk retagging: a bulk answer is what produced the problem. Both halves are now `[OWNER: M41]` and cross-cite each other, so ONE GREP RETURNS THE CATEGORY. That is the mechanism, not the prose: the tag is the membership list, and the failure mode being closed is a reader finding one record and believing it complete.
- 2026-08-25 — "what else is in this category" was answered by measurement rather than by the row's own list, and the row's list was right about the answer and wrong about the reason. The read-before-authz category is exactly EIGHT routes in TWO services and nowhere else in the repo — `documents` six, `plaid` two — derived from the handlers, each member confirmed by a refute-by-default pass, with all ten services under `apps/services/`, the BFF, both proxy origins, `apps/web` and `packages/` swept. The exclusions are each a NAMED mechanism and they are not the same mechanism: `assets` reaches `assertCanOrNotFound` on nine of its ten id-bearing routes, three directly and six through a shared write path, and holds the tenth out by gating before it reads; `profile` gates before it reads, and its by-id reads (`findById(id)` in `contacts.repo.ts` and `roles.repo.ts`, which select on the id alone) compare ownership AFTER the read and answer the same uniform 404 — two mechanisms, not the one this list first credited it with; `vault` fuses the owner into the WHERE so "not yours" returns zero rows; `ai-assistant`'s authz throws `NotFoundException`; `audit` has no HTTP surface at all; `identity` puts the owner predicate in the UPDATE; `notifications` takes no per-row decision. The M41 row had credited `settlement` to `assertCanOrNotFound`, and that helper is called at exactly THREE sites in a service with thirty-one routes — what keeps the rest out is that `SettlementAdminService` takes no Cedar PEP whatsoever and reaches a uniform 404 through three other helpers, plus routes excluded by ORDER because `OperatorGate.assertIn` throws before any lookup. An audit checking helper usage alone would have called most of settlement unprotected and been wrong about all of it. THE GENERAL SHAPE: a category list that names one mechanism per member is more useful than one that names a single mechanism for everybody, because the second kind is a summary that reads like a derivation.
- 2026-08-25 — the protection a residual claims can belong to the CLIENTS rather than to the boundary, and the difference decides whether a future member is a leak. Both members of the category are unreachable from browser traffic today, and the M41 row recorded that as though the edge narrowed the category. It does not. `apps/bff` collapses a BARE 403 to `NOT_FOUND` in exactly two of its six downstream clients — `documents-client.ts` and `profile-client.ts`; three of the other four — assets, identity and settlement — have only token-qualified 403 branches, and the token is `stepup_required` alone, so they let an unexpected bare 403 fall through; while `assistant-client.ts:211` reads a bare 403 as `DISABLED` — an outage-shaped answer, not a leak, and not the absence of a branch. The per-route `meaning.forbidden` belongs to `profile-client.ts`, one of the TWO collapsers; an earlier draft of this entry listed it among the other four, which is the same mistake in miniature — a mechanism attributed to the group it is the exception to. THE SAME DRAFT THEN DID IT AGAIN IN THE PRECEDING SENTENCE, and the prose pass caught that too: `assistant_disabled` was listed as a token of the three, and it occurs at exactly one site in the BFF — `assistant-client.ts:404`, the FOURTH client, the one the sentence exists to contrast them against. Twice in one paragraph, the second time inside the correction of the first, is the measurement: this error is not a lapse of care, it is what a contrast sentence does when the item list is recalled instead of re-derived. Both proxy origins collapse nothing — they return the upstream status verbatim — and `apps/operator-web`'s client maps 403 to `FORBIDDEN` and 404 to `NOT_FOUND` as two codes with two sentences. So the sweep M41 owns has a second question inside it: not only which services hold the shape, but which of them sit behind a surface that would render it. Worth recording how this was nearly missed in the other direction: a first pass grepped `status === 403` per client, got a non-zero count for all six, and read that as "every client collapses it" — the grep counted token-qualified branches that do the opposite of collapsing. Reading the six branches settled it; counting them did not.
- 2026-08-25 — a characterization test reads exactly like a specification until somebody checks which behaviour it was written to hold still. FOUR assertions in TWO files pin the plaid oracle's leaky answer — `plaid.int.spec.ts:214` and `:218` for a stranger on sync and on revoke, `plaid.service.spec.ts:89-90` for the same pair at the service — so the fix that collapses them to the uniform 404 turns those four RED, and they are part of that change rather than evidence against it. The 403s at `plaid.e2e.spec.ts:174` and `plaid.int.spec.ts:272` are the OWNER's step-up refusal — the e2e one asserts the `stepup_required` body, the int one only the status — and must stay GREEN: the same route owes 403 to an owner who has not stepped up and 404 to a stranger who has, which is the discriminant the fix has to preserve. An earlier draft of this entry credited the e2e file with a stranger assertion it does not contain. No test drives a plaid ROUTE with an item id that never existed and none asserts an HTTP 404 on any item route, so the comparison the property is about — stranger-on-a-real-id against anyone-on-an-unknown-id, at the wire — is never made. The miss branch is not untested: `plaid.service.spec.ts:103` asserts `NotFoundException` for an item the owner has just revoked, and a soft-deleted id reaches the same `requireItem` miss an unknown id would. The BRANCH has a test; the PROPERTY does not — and the first draft of this entry said "nothing anywhere requests an unknown item id", which the service-layer assertion falsifies. Recorded on the M27 PR5 precedent (docs/03 §6xx; docs/06, 2026-08-24), which is where this lesson actually lives — an earlier draft attributed it to M25 PR3, whose record is the destroy leg and says nothing about equivalences anywhere in it. The real lesson is sharper than the one I first wrote: an equivalence is only HALF a specification, satisfied by a function that renders everything alike, and what closes it is an assertion that DISCRIMINATES. A fabricated citation is the most expensive kind of wrong sentence here, because provenance is exactly what a later reader will not re-derive.
- 2026-08-25 — a count's DECLARED oracle counted prose mentions of the thing it counts, and so agreed with the superseded figure for the wrong reason. Two documents told a reader how to derive M21's residual count, both as `grep -c '\[OWNER: M21\]'` — unanchored, matching the tag wherever it appears, including inside a sentence ABOUT the tag. M40 PR1 re-tagged one bullet to M41 and then wrote §6eee, whose prose names `[OWNER: M21]` once; the bullet-anchored count fell to seventeen and the declared one stayed at eighteen, so the published oracle kept answering the OLD number and would have read to the next person as confirmation. Both copies are now anchored on the bullet (`^- \*\*\[OWNER: M21\]\*\*`), which is what the fence itself reads. THE GENERAL SHAPE: an oracle published beside a count is only as good as its corpus, and a corpus wide enough to include the document's own commentary about the data will eventually track the commentary instead of the data. This is the fence rule — anchor on what the runtime reads, and state the corpus — applied to a grep a HUMAN is told to run; the rule was already in force one layer up, and the prose copy was exempt only because nothing executes it.
- 2026-08-25 — the adversarial review of PR1 found five false sentences about OTHER FILES in my own draft, all of one kind: a mechanism or a count attributed to a file I had not re-read while writing the sentence. `assets` was credited with `assertCanOrNotFound` on "all twelve of its id-bearing routes" — thirteen routes, ten id-bearing, nine reaching the helper (three directly, six through the shared `runCommand`), and the tenth held out by gating before it reads. Four BFF clients were said to carry "no bare-403 branch at all"; `assistant-client.ts:211` reads a bare 403 as `DISABLED`, an outage-shaped answer rather than no answer. A per-route `meaning.forbidden` was listed among those four when it lives in `profile-client.ts`, one of the two COLLAPSERS it was being contrasted with. The plaid characterization tests were counted as three across two files, one of them an e2e assertion that is the OWNER's `stepup_required` — the true shape is four assertions, in `plaid.int.spec.ts` and `plaid.service.spec.ts`, and the owner's two must stay green because they are the discriminant. And a precedent was cited to "M25 PR3", whose record is the destroy leg and contains no equivalence discussion at all: the lesson is M27 PR5's, and it says the OPPOSITE of what I credited it with — an equivalence is only half a specification. The assets sentence is the exact error §6eee convicts the M41 row of further down the same section, committed in the act of writing the conviction. THE RULE THIS BUYS: a sentence about another file is an ASSERTION, not a recollection, and the moment to re-derive it is while writing it. The claims that FEEL remembered are the class that survives review, because they read as knowledge rather than as measurement.
- 2026-08-25 — a line number into ANOTHER document is a citation that rots on the next insertion above it, and this PR both broke one and found two that had been wrong for a long time. M40 PR1 inserts 13 lines into the first half of docs/03, which silently moved the target of docs/04's `docs/03:5165` — a correct citation before this PR and a wrong one after, with nothing to notice. Sweeping every `docs/0N:NNNN` citation in tracked source and docs and resolving each against both `fbf4eff` and the working tree found four: one broken by this patch, one this patch had already repaired for the same reason (`docs/04:5639` in the fence's own docstring, moved by seven), and TWO that were already wrong at `fbf4eff` — `packages/contracts/test/threat-model-residuals.spec.ts` says twice that `## 7. Validation program` sits at `docs/03:4523`, and it sat at 4551, off by twenty-eight, in comments that explain why the parser needed a fix. All four are now anchored on the TEXT they mean. THE GENERAL SHAPE: a cross-file line number is a fact about the tree asserted in a comment, which the repo already calls a test nobody runs; the difference from other stale prose is that this kind is invalidated by edits that have nothing to do with it, so it decays without anyone being wrong about anything. The cheap fence is the sweep itself — extract every citation, resolve it at HEAD and at the merge base, and diff the resolved lines. AND THE SWEEP HAS THE SAME DEFECT IT FINDS, which is the third instance of one shape in this PR: run it after the repair and it still reports four broken, because its corpus now includes THIS SENTENCE and the fence comment that name `docs/03:5165`, `docs/03:4523` and `docs/04:5639` as the examples. A citation and a mention of a citation are indistinguishable to it, exactly as a residual tag and a sentence about a residual tag were indistinguishable to the unanchored `grep -c` this same PR replaced, and as `assistant_disabled` was indistinguishable from a token of the group it is the exception to. Three times in one change: a corpus wide enough to include the document commenting on the data will report the commentary. Any real fence here must exclude prose that quotes a citation as an example, and stating that is cheaper than shipping a fence that goes green for the reason it is wrong.
- 2026-08-26 — an item can be deferred WITH AN OWNER and still be owed by nobody, and this one was for nine days and six shipped milestones — M22, M23, M24, M25, M27, M44, the `**COMPLETE.**` rows of docs/04's queue table, which is the oracle the residuals fence itself reads (introduced by M21 PR0, `9885362`, #109, 2026-08-17). docs/03 §4's deferred-TB7 list gives every item it defers a bolded owner — JIT elevation (E1), session recording (E1), KMS grant suspension (E1), the M18 alarm surface (M23), the legal-hold ceremony (M21 PR5) — and two of them name an owner called "**M21 follow-on**": peer approval of an operator action, which carries the fuller "not the minimum slice", and operator-assisted account recovery, which does not. That is not a milestone. No queue row, no `OWNERS` entry, nothing to schedule, and no way for any derivation to notice, because §4 is PROSE and the residuals fence's corpus is §6 BULLETS — the blind spot §6uu already records, found this time from the other side. The fix is a row: M46, the TB7 follow-on, which is that owner made real, and the two §4 sentences now name it. THE GENERAL SHAPE: a deferral is only as good as the owner's ability to be SCHEDULED, so an owner that is not in the vocabulary the plan derives from is a deferral to nobody wearing the costume of a decision. The reviewable version of "defer it with an owner" is "defer it to a row".
- 2026-08-26 — two scope statements disagreed, and the one that was wrong held the pointer to the one that was right. docs/04's M21 scope SAID the milestone does NOT ship "JIT elevation, peer approval, session recording, KMS grant suspension (all E1 — they need real IAM)" — quoted as it stood at `71b8af7`, because this change repairs it — and its very next sentence deferred authority: "Each of those is named with its owner in the rewritten `docs/03` §4 TB7 block". Open that block and peer approval is not E1 — it is the "M21 follow-on". So the summary swept four items into one owner while pointing at the list that disagreed with it, which is the §6eee shape ONE PR later in the same milestone, between two SCOPE statements rather than two residuals. The E1 sweep was also wrong on the merits for that member: M-of-N over the stage chain is a second-approver column and a CHECK in settlement's own cluster — the shape that service has already shipped FOUR times (docs/03 §6b enumerates them; an earlier draft of that bullet said twice, a hand count) — and needs no IAM, so leaving it in E1 would have put an item in the escalation queue that was never blocked on Money. THE RULE: when a summary and the list it defers to disagree, the summary is the copy, and a sentence that both summarises AND cites its authority is the most convincing way to be wrong.
- 2026-08-26 — one tag cannot dispose of three items, and a bullet that argues about owners inside itself has already stopped being the record. docs/03 §6aa's allowlist-row residual named three properties — no expiry, no case scoping, no session binding — and handed TWO of them to owners its tag did not name, IN ITS OWN PROSE: "session binding is PR3's audience; expiry and just-in-time elevation are **E1**" — leaving the third, case scoping, with no owner at all, which is the worse half, because an item with no owner inside a bullet whose tag is wrong is invisible from both ends. It carried a single `[OWNER: M21]` tag matching none of them. Whichever owner moved would have taken the other two with it, and one of the three had already SHIPPED. Split here into three bullets, and ALL THREE end at M46 — case scoping, session binding, and expiry. Session binding was drafted `[CLOSED: §6bb]` on the strength of PR3a's audience and was false (see the entry below); expiry was drafted E1 on the strength of the bullet's own prose and was also false, because `settlement_operators` is a table in settlement's own cluster whose membership predicate is `revoked_at IS NULL` — expiry is one column and one conjunct, needing no IAM, which is the same merits test that moved peer approval out of E1 in the entry above. So the bullet's self-assigned owners were wrong for TWO of the three, which is what a residual arguing about owners inside itself buys you. §6b's interim-allowlist bullet had the same shape with two controls and is split the same way; there the E1 half (JIT elevation) survives the test. THE DETECTION RULE, cheap and worth running: a residual whose body contains a second owner name is a residual whose tag is incomplete — grep the corpus for bullets naming an owner in their prose that differs from the one in their tag.
- 2026-08-26 — I read a decorator's PRESENCE as a decorator's BEHAVIOUR, and nearly closed two live residuals on it. M40 PR2 drafted `[CLOSED: §6bb]` on docs/03 §6z's "an operator is still an ordinary account session" and §6aa's "an allowlist row has no session binding", reasoning that M21 PR3a shipped the `operator` audience and settlement decorates routes `@AllowSessionAudiences('operator')`. Both were FALSE. `AllowSessionAudiences` returns `[DEFAULT_SESSION_AUDIENCE, ...audiences]` — its parameter type `Exclude<SessionAudience,'account'>` forbids naming `account` precisely because it is always included — and `CallerGuard.audiencesFor` UNIONS the route list with the service-wide grant, its docstring saying it "could never accidentally narrow". Settlement binds no `ALLOWED_SESSION_AUDIENCES`, so a decorated route admits `account` OR `operator`, and settlement consults `caller.audience` in no control anywhere. The decorator WIDENS. Worse, docs/04's own M21 PR3 bullet states the trap verbatim — "decorating a settlement route ALSO admits every ordinary account session. `OperatorGate` stays the control" — in the file this PR was already editing, forty-seven lines below this change's own nearest edit to that file. TWO RULES, and the second is the one that generalises. (1) **CLOSED is the only disposition that stops anyone looking again**, so it needs evidence of BEHAVIOUR, demonstrated, never the presence of the mechanism the bullet names; every other tag can be wrong and survive contact with the next sweep. (2) Re-reading a RESIDUAL is not the same act as re-reading the MECHANISM the residual names, and a sweep whose unit of work is the bullet will do the first while feeling like it did the second. Caught by a refute-by-default reviewer that probed the built `dist` rather than reading the call site — which is the level the claim actually lived at.
- 2026-08-26 — naming a PR as a remedy is how a record survives the remedy not happening, and the TB7 documentation did it three times. §6z's allowlist reader says "the reader is PR3"; PR3 shipped, and `operator.controller.ts` exposes queue, administrable, report-provider, review, verify and evidence-read with no route listing allowlist membership — the CLI is still the only reader. docs/03 §4 already logged two instances of exactly this before anyone looked for a third: the M18 alarm surface that "said **M21 PR3b**, which shipped the console without it", and the legal-hold ceremony that said M21 PR4 before docs/04 re-sequenced that slot. Three records in one milestone's documentation naming a PR that shipped past them is not three mistakes; it is what a remedy written as a PR NUMBER does, because the number is discharged by the PR merging rather than by the condition holding. THE RULE: write the remedy as the CONDITION that must become true, and let the PR be evidence for it — "a route lists allowlist membership" survives PR3 shipping without one, and "the reader is PR3" does not.
- 2026-08-26 — a residual justified its owner by pointing at a sibling's owner, and the pointer was wrong in both halves. docs/03 §6b's intake rate-limiting bullet read "Filed against the same owner as §6z's unbounded operator actions because both are the same absent machinery in the same service, and splitting them across milestones is how one of them gets built and the other keeps its exemption." §6z has no rate-limiting residual — the sibling is *Operator actions remain unbounded* in **§6bb** — and that sibling is owned by **M23**, and "the same owner" was NOT false when the sentence was written, which is worse than if it had been: at `009bad7` (2026-08-17) that sibling did not exist anywhere in docs/03, so the sentence forward-referenced a residual nobody had written yet; M21 PR3a created it the next day (`388d22b`) under `[OWNER: M21]` — the same owner — making the clause briefly TRUE; and M21 PR3b re-owned it to M23 two days later. The claim was never re-checked at any of the three moments its truth changed, and the pair the bullet asked not to split is split today. M40 PR2 accepts the intake bullet on its own bounds (the linked-contact gate and the one-open-case index) and repairs the citation, recording that the pair stays split DELIBERATELY because they are not the same surface: one bounds reporter traffic at intake, the other bounds authenticated operator actions. THE SHAPE: an owner justified by another item's owner inherits that item's drift as well as its argument, and the two rot independently.
- 2026-08-26 — a fence whose NAME was right and whose BODY was not, found by mutating the thing it was named for. `threat-model-residuals.spec.ts` carried a test called "a CLOSED residual cites the delta that closed it", and M40 PR2's mutation harness rewrote a `[CLOSED: §6bb]` tag to `[CLOSED: §6zzz]` — a section that has never existed — and the suite stayed GREEN. The assertion checked the SHAPE `§6[a-z]{0,3}` and never that the id named a real delta, so it enforced the FORMAT of a citation while permitting a citation to nowhere, which is the same defect the test's own comment says it exists to prevent: it "offers them nowhere to look". Closed by checking the cited id against the section set the parser already derives — the same derive-don't-describe rule the owner vocabulary follows one test above — with an anti-vacuity floor so the assertion cannot pass because nothing is closed. THE GENERAL SHAPE, and it is the reason to mutate the assertion you are relying on rather than the code around it: a test name is a CLAIM about coverage and the body is the coverage, they drift independently, and the name is what every later reader trusts. PR2 only found this because it was the first change in a while to DRAFT a CLOSED tag and therefore the first to ask whether that tag was checked — and the tags themselves did not survive its review, so the repair outlived the thing that motivated it.
- 2026-08-26 — a deferral's precondition can be spent by a LATER PR, and nothing watches. docs/03 §6dd deferred an audit emit with a stated condition rather than a judgement: "left as found because the route has no consumer anywhere — no BFF client, and the operator edge's exact-match allowlist does not carry it". M23 PR4b (`4e125e1`, #141) then built exactly that consumer — a BFF client method, the `setEstateDistributionStatus` mutation, a three-member SDL enum and an executor button reading "Raise a dispute" on a paid-out row — and did not add the emit. A latent defect became a LIVE one on a step-up-gated write verb. §6dd is M21 PR4's delta section, so M21 wrote the condition and M23 PR4b spent it — a DIFFERENT milestone, which makes THIS instance worse rather than better, because the PR that discharges a precondition has no reason to have read the section stating it. Two drafts of this entry were wrong in opposite directions: the first said the spender was inside the deferring milestone, which is false here; the second generalised the repair to "every instance crosses a milestone boundary", which is false of the next sentence — §6bb's interstitial was deferred by M21 PR3a and met by M21 PR3b, the SAME milestone. Two of the three cross and one does not, so the milestone is not part of the shape. The same shape appeared a second time in the same sweep with a smaller consequence: §6bb's operator interstitial deferred until, in its words, "there is something behind it", the console shipped, and nobody re-decided. THE RULE: a deferral that names its own trigger is only as good as whatever watches the trigger. A residual is re-read when somebody sweeps residuals; the event that discharges it happens somewhere else entirely, in a PR that has no reason to look. Write the condition as DATA a fence can watch, or expect to find it spent by accident — M40 PR3 recorded that as its own residual rather than pretending the sweep is a mechanism.
- 2026-08-26 — a residual owned by a COMPLETE milestone is not merely unowned, it is UNREAD, and that is a different defect with a different cost. M40 PR3 adjudicated the thirteen residuals tagged M22 or M23, both milestones long shipped. FIVE were still TRUE — every assertion in them re-verified — and EIGHT carried at least one false clause; all thirteen were REWRITTEN, so "held exactly as written" measures the editing rather than the decay — one had become a live control gap, one had had its justification refuted the day after it was written, two carried three items each under a single tag, and several stated counts that a later PR had moved. The stale-owner fence could see that these tags named a finished milestone; nothing could see that their SENTENCES had rotted, because no mechanism reads prose for truth. THE MEASUREMENT WORTH KEEPING: an unowned residual decayed at roughly 60% (eight of thirteen) over FIVE TO SIX DAYS in this repo — M22 and M23 shipped 2026-08-20 and 2026-08-21 and this sweep ran 2026-08-26, so the interval is days, not the months an earlier draft of this entry claimed, and it decays in a direction — toward understating risk, because the surfaces that would have made it live are exactly the ones built after it was written.
- 2026-08-26 — an assertion cannot survive its own success, and a zero that cannot fail is not a measurement. `threat-model-residuals.spec.ts` pinned the stale-owned set with an owner-SET comparison against `STALE_OWNERS` — built inline as `expect([...new Set(stale.map(...))].sort()).toEqual([...STALE_OWNERS].sort())`, there being no such variable as `staleOwners`, which an earlier draft of this entry invented — and with `expect(stale.length).toBe(STALE_OWNED)`, at thirteen across M22 and M23. Pinned to a NON-EMPTY set those were real: a broken parser, an unread status column or an empty corpus all produce a number that is not thirteen. M40 PR3 took the set to zero and both assertions became `[] vs []` and `0 === 0` — which is what a correct run produces AND what every one of those failures produces. THE FIX IS A POSITIVE CONTROL, not a floor, because there is no number above zero to floor: a synthetic bullet tagged with a milestone taken from the DERIVED completed set (never hard-coded, so it cannot outlive the milestone it names) is fed through the SAME predicate the real corpus just used, and the fence asserts it FIRES — paired with a negative twin using a live milestone, because "the predicate fires" is also what a predicate firing on everything does. AND THE FIRST ACCOUNT OF THE CONTROL'S COVERAGE WAS WRONG, caught by the harness rather than by review: the draft said it caught a broken status column or an empty completed set, and it does not — both redden one assertion earlier at `MIN_COMPLETED_MILESTONES`. What it uniquely catches is the PREDICATE failing while its inputs stay healthy, shown as a counterfactual PAIR (same mutation: control present = 1 failed, control removed = 18 passed) rather than argued. THE GENERAL SHAPE: when a fence's subject is successfully driven to empty, the assertion that guarded it silently changes meaning from "nothing is wrong" to "nothing was looked at" — ask it of every ratchet that reaches its floor. And state a control's coverage from the counterfactual, never from what you meant it to do; a mutation that reddens proves the FILE catches something, not that YOUR addition does.
- 2026-08-26 — a justification can die while its conclusion survives, and only the conclusion gets re-read. docs/03 §6bb said operator actions were unbounded "because a bound whose only caller is a surface that does not exist is a control nobody exercises". M22 PR1 (`a7eb15b`) shipped one the NEXT DAY — an append-only operator action ledger with a derived coverage fence, a twelve-distinct-case ceiling over an hour, and proof by execution that `settlement.operator.breadth_exceeded` fires — which refutes the reasoning rather than merely dating it, since the console it said did not exist now proxies fourteen settlement routes. The conclusion "unbounded" stayed literally true of REFUSAL (the bound warns and never refuses, deliberately), so a reader checking only the headline would have found it correct and moved on. What actually survived was a different item entirely: operator READS, which the ledger records nothing about and whose two worklist reads its own fence DECLARES exempt. THE RULE: re-check a residual's REASON, not only its claim — a claim that stays true for a new reason is a claim whose owner, size and remedy have all changed underneath it.
- 2026-08-26 — a bullet that names one edge of a two-edge template describes half its own population. docs/03 §6dd recorded "three operator-edge items with no reach path": a malformed-`Host` 500, a `config.secureCookies` nothing reads, and `APP_ORIGIN` validated with `z.string().url()` rather than as an origin. Every one of the three exists identically on `apps/vault-web` — `z.string().url().optional()` is the same line in both `config.ts` files — so three shapes across two origins is SIX instances and the bullet named three. The two isolated origins were built from one template and have drifted with nothing comparing them, which is why M40 PR3 opened M47 for the PAIR rather than tagging the operator half: tagging one would have recreated the one-category-two-owners defect M40 PR1 closed, from the other direction. THE DETECTION RULE: when a residual names a component that was created by copying another, grep the sibling before writing the count — and if the pair has no derivation, that absence is itself the residual worth recording.
- 2026-08-26 — reading a decorator's optionality as a behaviour, again, one PR after recording the rule. docs/03 §6dd carried `CallerGuard`'s `@Optional()` reflector as fence debt: the shared source fence "cannot see that the reflector is `@Optional()` and falls back to the service-wide list when it does not resolve — a perfectly decorated route on such a container is silently inert". Re-reading the CODE rather than the residual: the fallback UNIONS and never narrows, so an unresolved reflector cannot widen admission, which is this repo's own "fail closed means de-escalate" rule being honoured rather than broken; it is reproduced with a positive control in two suites; and profile's real container is proven to resolve it inside a blocking gate by the stack e2e. The bullet feared a property the code does not have. This is the SAME error M40 PR2 recorded at §6fff one PR earlier — presence of a decorator read as behaviour of a decorator — found this time by re-reading rather than by review, which is the only reason it did not ship a third time.
- 2026-08-27 — a deferral justified by an ABSENCE is only as good as whatever watches the absence, so the absence becomes DATA. M40 PR3 caught, by hand, one bullet left as found "because the route has no consumer anywhere" that a later PR of a DIFFERENT milestone then gave a consumer, without re-deciding anything. The general shape is not findable by re-reading prose, because the PR that spends a precondition has no reason to have read the section stating it. `PRECONDITIONS` in `threat-model-residuals.spec.ts` therefore holds each such deferral as a record — the absence, the corpus that would falsify it, what "filled" looks like, and where it is deliberately NOT counted — and the probe reads the TREE rather than other prose. Its sweep found no second instance, which is a claim only because the sweep's positive control is PR3's own case fed through the same probe and returning SPENT: a clean run and a broken probe are otherwise the same output.
- 2026-08-27 — the escalation vocabulary is deliberately NOT the milestone vocabulary, because the two mean opposite things about a residual that names them. `**COMPLETE.**` on an owner makes its residual STALE — the work is done and the tag is now a lie. `**CLEARED.**` on an escalation blocker makes its residual SCHEDULABLE — the work can start and nobody has started it. Reusing one word for both would collapse a discharged obligation into an unstarted one. docs/04's escalations table gains a State column (five rows, all `**BLOCKED.**`) with two assertions: every escalation row declares a state, and no residual names an escalation whose blocker has CLEARED. The State cell reports the BLOCKER, not the work.
- 2026-08-27 — a section does not review itself, stated as a corpus bound rather than left implicit. M40 PR4 re-adjudicated the FOURTEEN `[ACCEPTED]` bullets M40 wrote or amended in other milestones' sections — thirteen newly marked plus one whose tag predates the milestone — and excluded the six residuals M40 wrote about its OWN work in §6ddd, §6fff and §6ggg. Those six are the milestone's account of its own bounds, written by the same judgement doing the reviewing, so folding them into the corpus would have measured self-agreement and reported it as coverage. The exclusion is written into the section rather than inferred from the count, because two of the fourteen and six of the twenty are indistinguishable to anyone recounting later.
- 2026-08-27 — an acceptance whose argument does not reach the surface it defends is reopened SPLIT, not re-tagged whole. docs/03 §6g accepted that unlinking does not revoke what the link enabled, naming two survivors and one gate; there are seven behind two gates, and `assertCaseVisible` takes no Cedar decision at all, admitting on the frozen `kase.reported_by === actor` snapshot while re-deriving the EXECUTOR's linkage live two lines below. All five reads it guards sit POST-verification, the window that bullet's own last sentence says the owner's void does not reach. The bullet held two different things and gets two dispositions: the money route — `distributionAmount`, which decrypts under the DECEDENT's DEK — is a defect and goes to M48 with the four silent reads' missing trail; the case-exists reads stay `[ACCEPTED]`, because a reporter is meant to know the case exists. Re-tagging the whole bullet would have made evidence into debt and buried the one route that returns plaintext money.
- 2026-08-27 — a milestone's LAST PR flips its own lifecycle token, in the same change. Leaving M40's row `**IN PROGRESS.**` after the security review that finishes it would be exactly the M23 defect the milestone exists to sweep — a finished milestone described as live — committed by the sweep itself. The rule is the same one PR0 asserted for every other row: the Status cell answers the completion question, and the PR that makes the answer change is the PR that owns changing it.
- 2026-08-27 — an anti-vacuity floor belongs ON the true derived value, and the earlier cautionary tale is about a floor derived from a BROKEN measurement, not about a floor that sat too high. `MIN_COMPLETED_MILESTONES` was 3 when six milestones had shipped, so it ratified the parser bug it existed to catch; the fix was totality in the Status column, not a bigger number. With M40's own row flipped the derived set is seven, and the floor ratchets 6 -> 7 — proved load-bearing by reverting the row to `**IN PROGRESS.**` and watching `no residual is owned by a milestone that has already SHIPPED` go red at `>= 7` against `6`, while the other twenty-one assertions stayed green. At 6 that same mutation passed twenty-two of twenty-two.
- 2026-08-27 — a prose count can be falsified by the sentence that asserts it, and this one was. A draft of §6hhh said of §6o's dead cross-reference that "the string occurs exactly once in all of `docs/`, here" — true until the sentence quoting it existed, and false the moment it did, wrap-insensitively twice. The replacement asserts a property of the TARGET instead: §6m has no numbered list and does not contain the word "question". The residual-language corpus rule bit the same paragraph a third time, USE versus MENTION: two of the four evidence bullets quoted "owed by" and "its own milestone" from the residuals they were describing and were collected as residuals themselves. Evidence bullets get reworded, not exempted — the fence's inability to tell use from mention is already an `[ACCEPTED]` residual in §6ggg, and widening the parser to carve out quotations would trade a false positive anyone can see for a false negative nobody can.
- 2026-08-27 — a relationship used for authorization is re-derived at read time; a relationship used as evidence stays frozen, and the two live in the same row. `settlement_cases.reported_by` records WHO FILED — evidence, and it must never change — but `assertCaseVisible` was also ADMITTING on it, so it answered a question about the past when the question was about now. The executor arm of the same `if` had always called `isExecutorOf` LIVE, two lines away: one estate enforcing a link's lifetime for one party and not the other. M48 PR1 adds the live conjunct to the reporter arm only. The boundary rule: the frozen column may RECORD anything and ADMITS nothing by itself. `CasesRepo.listForUser` deliberately keeps matching it unqualified, because "which reports have I filed?" is the evidence question and docs/03 §6g's acceptance rests on the answer staying yes — a case with no visible author is the thing that argument exists to prevent.
- 2026-08-27 — the four reads M40 PR4 called "case-exists reads" are not case-exists reads, and the vocabulary undersold what the adjudication left ACCEPTED. `listDistributions` returns the beneficiary contact, the asset, the approving operator and whether an amount exists; `timeline` returns the full case history; `listStages` returns the access ladder; `listTasks` returns the administration checklist — each item's title, category, assigned role, due date and who completed it. That is all FOUR of them, named, because a sentence that says "the four" and describes three is the same half-application it is complaining about. Seeing that your own report exists is `getCase` and `listMyCases`, on different gates. So M48 PR1 takes all five reads from an unlinked reporter rather than only the money route — and the alternative was MEASURED, not argued. The argument is M23 PR4b's set equality, encoded in `admits exactly the callers listDistributions admits`: a reader who may see a distribution exists is the reader who may see its amount, so shrinking both sets together is the only option that keeps it. The measurement PR1 cited was real but did not survive PR1's own fixture repair — see the PR1a entry below, which re-arms it.
- 2026-08-27 — a verified count is not a complete count, and asking a verifier for the number gets the number. docs/03 §6g and §6hhh both say "seven survivors behind two gates". A refute-by-default pass confirmed it and was RIGHT — seven do exist behind two gates. It was never asked whether an eighth existed, and one does: `CasesRepo.listForUser`'s `WHERE decedent_user_id = $1 OR reported_by = $1` is a third gate in a third mechanism (raw SQL, not Cedar and not a bare `===`), carrying `listMyCases`. The repo already has the rule — compare SETS, not counts, because mis-attribution preserves counts — and it was applied to the fence's own data while the prose verification asked a confirmation-shaped question. Verification prompts need the anti-vacuity discipline that scans do: ask what ELSE is in the category, never whether N of them are there.
- 2026-08-27 — repair the test double BEFORE writing the proof, or the proof is about the fixture. `FakeCoreReads` had no `unlink` at all, and its `isExecutorOf` consulted only the designation set while the real query JOINs `contacts` with `deleted_at IS NULL` and `linked_user_id` — so an unlinked executor kept the estate in tests and lost it in production. M48's entire subject was inexpressible on that harness, and the control "the executor arm is already live" would have been a false green. Repairing it alone reddened one test, which turned out to be a fixture that designated an executor without linking one; and two more fixtures named a REPORTER without linking one, a state intake cannot produce, because `report` refuses with a uniform 404 unless `isLinkedContact` holds. A double must be faithful about what it REFUSES, and a fixture in a state the product cannot reach makes the test named for a property evidence about nothing.
- 2026-08-27 — a browser drive proves nothing until the probe is shown able to FAIL, and on a containerised stack that means rebuilding the counterfactual. M48 PR1's drive reproduced the defect end to end — an unlinked reporter reading `estateDistributions` and receiving `200 []` — but only because the settlement image was rebuilt with the fix REVERTED and proven pre-fix by reading `dist/admin.service.js` out of the running container — zero occurrences of `isLinkedContact` in the ONE file the fix changes, the service-wide string being no discriminant — rather than trusting a build timestamp. The first observation of `NOT_FOUND` on the fixed container was consistent with the fix working AND with any number of unrelated reasons an unlinked account might be refused; only the same request, same session, same unlinked state, different code distinguishes them. Two constants make the comparison legible: `reportableEstates` was empty in both runs (so the link state did not move) and `settlementCases` returned the case in both (so the refusal was scoped to the gate under test, not a blanket denial).
- 2026-08-27 — a FIXTURE REPAIR can falsify the measurement cited to justify the same PR, and nothing in the toolchain notices. M48 PR1 argued for taking all five reads rather than only the money route on the ground that the money-only alternative REDDENS `admits exactly the callers listDistributions admits`, M23 PR4b's set-equality comparison. That was true when written. The same commit then added `h.coreReads.link(DECEDENT, REPORTER)` to `verifiedCase`, because intake cannot produce an unlinked reporter — and a LINKED fixture reporter is admitted by both routes under the money-only alternative, so the sets stay equal and the comparison stays GREEN. Measured on the tree PR1 shipped: money-only reddens ONE test, the new `an UNLINKED reporter is refused by every route behind the gate`, and the SET comparison passes; measured with the fixture link removed, the SET comparison FAILS, which is the arm the sentence described. **The fix is not to soften the prose but to state the missing arm as data**: PR1a adds `admit the same SET after an unlink`, which compares the two admitted sets with the reporter unlinked. Money-only reddens it, and so does dropping the live link conjunct altogether (the anti-vacuity floor naming REPORTER's absence), while the original M23 PR4b comparison stays green in both — the positive control that the old test was not simply broken. The general rule: when a change repairs a fixture, RE-RUN every alternative the same change's prose claims to have measured. A revert restores code; it does not restore the evidence a sentence was resting on, and a repair does not either.
- 2026-08-27 — a justification can name a sibling that DISAGREES with it, and nobody re-reads the two together. `caseViewed` emitted for operator reads only, and its docstring argued the narrowness was the decision: the other readers are "people reading their own case, which the rest of the product does not audit as a disclosure either". That is true of the decedent and false of the EXECUTOR, who administers somebody else's estate — and the assets service has audited exactly that read as `asset.estate.viewed` with `actorType: 'user'` since M7 PR2 (`031972c`) — a provenance an earlier draft of this entry dated to M23 PR4a, which is in fact the profile sibling `contact.estate.viewed`, not this event. Settlement's own `executorCases` cites that event BY NAME as the reason it needs none of its own, so the counter-example was already quoted inside the service holding the argument. The argument's other half — that a false actor class on an append-only trail is worse than no row — was sound and is SPENT the moment the class is derived from the gate rather than asserted by the emitter. M48 PR2 derives it. The general rule: when a justification cites another mechanism as support, open that mechanism. A citation is a claim about a file you are not editing, and this repo's measured rate of false sentences about other files is high.
- 2026-08-27 — a double that ACCEPTS a field and drops it makes a fix unprovable, not merely untested, and the two are not distinguishable from the outside. `FakeFieldCrypto.decryptField` has always taken an `actorType` in its input type and recorded only `{ userId, field, actorId, purpose }`. So `crypto.field.decrypted` naming every operator's amount read as the estate's own reader could not be caught by any assertion: revert the fix and nothing goes red, which reads exactly like a change that is not load-bearing. M48 PR1 recorded the sibling lesson for a MISSING method and a query that ignored links; this is the quieter form, where the signature says the field is observed and the body throws it away. Reverting the repair now stops the new test COMPILING (`Property 'actorType' does not exist`), which is the sharpest available statement that the repair had to come first. The rule to carry: for any field a caller is JUDGED on, check the double records it before writing the assertion — a double must be faithful about what it OBSERVES, not only about what it refuses and what it returns.
- 2026-08-27 — a vocabulary maintained BESIDE a mechanism drifts silently; derive it or fence it. The case-read surface union existed twice — `CaseReadSurface` in `events.service.ts` and a narrower hand-written copy as the audit wrapper's `surface` parameter — with nothing tying either to the set of reads that actually pass through `assertCaseVisible`. They agreed by hand until M23 PR4b added `distributionAmount` to the gate and to neither union, so from 2026-08-21 (`4e125e1`, #141) to 2026-08-28 the fifth gated read produced no case-trail row and no assertion noticed. A TypeScript union cannot derive itself from "the methods that call the gate", so the derivation is a FENCE rather than a type: the spec scans both source files and compares SETS, failing by name when a gated read records no surface and separately when a declared surface no read produces. Deleting the new `recordCaseRead(..., 'amount')` call — the original defect replayed — reddens both. Neither assertion can be satisfied by editing one file, which is the property a hand-maintained list beside a growing thing never has.
- 2026-08-28 — the audited-volume rule applies to the QUERY's mount point, not to the route's name, and a route can look like a screen and be the app shell. M48 PR2 audited `listMyCases` as a worklist and the review measured where the query comes from: `OpenSettlementCaseBanner` is mounted in `AppShell`, which wraps every page under `(app)/layout.tsx`, with its effect keyed on `pathname`. One append-only row per page navigation, per authenticated user, on a trail with UPDATE and DELETE revoked — and docs/03 §6cc had already refused to let the operator CONSOLE poll for the same reason, on a population of staff. The emit is removed and the reason is written at the line where the next person would add it. Two general rules: before auditing a read, find its CALLER and its mount, because "who reads this and how often" is not visible from the service; and a reuse justified only as "no consumer deploys ahead of this producer" has not yet weighed the member's role as a DETECTION signal — `settlement.queue.viewed` exists to carry cross-case reconnaissance, and ordinary self-reads would have left `detail.worklist` as its only discriminant.
- 2026-08-28 — removing an early return can DISARM a test that never mentioned it. `an operator who is ALSO the reporter is still recorded as an operator read` asserted presence: `expect(auditActions(h.producer)).toEqual(['settlement.case.viewed'])`. While `recordOperatorRead` opened `if (!isOperator) return;`, presence WAS the classification, so counting rows tested the property the name claims. M48 PR2 removed that early return, every reader began emitting exactly one row, and the assertion stopped being able to tell `operator` from `user` — green, untouched by the diff, and no longer testing its own name. Measured: mis-classing the reporter-operator as a user left the entire package green, 277 passed. The rule: when a change removes the mechanism a test's assertion RESTED on, that test is part of the change's blast radius even though the diff does not touch it. Ask, for every behaviour a PR alters, which existing assertions were only working because of it. The repair asserts the class per row; the same mutation now reddens exactly the test named for it.
- 2026-08-28 — a derived fence with one member injected BY HAND is a fence with a hole exactly there, and the hand-injection is where the corpus quietly narrows. The surface fence scanned `admin.service.ts` for `assertCaseVisible` callers and then added `'case'` to the expected set by name, because `getCase` sits on the Cedar gate in another file — so deleting `getCase`'s emit entirely left BOTH named assertions green while the docs claimed "a declared surface no read produces fails the other". The fix is to scan the second file rather than to soften the sentence: one file per gate, and `'case'` derived like everything else. Three smaller lessons from the same fence: strip block comments, because a call commented out with `/* … */` does not start with `*` and was read as live; join a method body before matching, because a call whose arguments wrap over five lines is invisible to a line-at-a-time scan; and floor the CALL-SITE counts as well as the route count, because a gated read whose declaration the scanner fails to recognise is attributed to the method above it and vanishes without changing any set the test compares.
- 2026-08-28 — an audit emit placed below a payload-shaped early return is conditional on the payload, and "who read this estate" is not a question about the payload. `distributionAmount` recorded its case-trail row after `if (row.amount_ct === null || row.dek_id === null) return { amount: null };`, so a caller who passed `assertCaseVisible` and read an amount-less distribution learned that the row exists and that the case is visible to them, and produced no case-trail row at all — where the four list surfaces emit even when they return `[]`. The emit moves above the null return; the DISCLOSURE event (`settlement.distribution.amount_viewed`) stays below it, because that one is scoped to an actual reveal. The rule: an event that records ACCESS is ordered against the authority decision, and an event that records DISCLOSURE is ordered against the disclosure — putting both at the same line answers the second question only in the branch where the first one is.
- 2026-09-02 — a control can be RIGHT when written and falsified later by a different milestone, and nothing in the tree watches for that. Both of M48 PR3's subjects were this shape. `ENCRYPT_ONLY_PREFIXES.distributions` said "settlement amounts are write-only — no read route exists", and that claim was TRUE when written — the map arrived in M18 PR2 (`eef5606`) and its note credits M18 PR1 with correcting an earlier comment that asserted a route which did not then exist; M23 PR4b then shipped `distributionAmount`, so from 2026-08-21 every dual-control amount check an operator performed breached the loudest class in the table at count 1. The same for `attachCaseEvidence`: PR1 made the reads re-derive the reporter's link and the write kept authorizing on the frozen snapshot. Neither was caught by a test going red, because in both cases the mechanism that would have noticed is the one that had been made wrong. The rule that follows: a comment asserting "no X exists" is a claim about the whole tree with no owner, and the milestone that ADDS the X is the one that will not think to look. Anchor such a claim to something derived — here, a bound row per principal class whose membership a spec reads out of the caller's source — so adding the route reddens something instead of silently making a sentence false.
- 2026-09-02 — DELETE a construct whose only correct instance has gone, rather than emptying it. `ENCRYPT_ONLY_PREFIXES` held exactly one member for its whole life. Removing that member would have left a map with no keys, a `boundFor` branch nothing can reach, a `BoundName` union member nothing can produce, and a fence looping zero times — every one of them green, and the zero-iteration loop is this repo's most-named failure. Deleting the construct also STRENGTHENED the invariant next door: `undecidedPrefixes` had accepted "a bound row OR an encrypt-only sentence" as a visible decision, and now demands a reviewed NUMBER for every registered prefix, which is the thing that was missing when the sentence went stale. The cost is real and is stated in the source rather than hidden: a genuinely unreadable prefix no longer gets a distinct alarm token and falls to `unmodeled_principal`. Prefer an absence to a filter — and when the absence loses something, say what, beside the code that lost it.
- 2026-09-02 — a fence that reads ANOTHER package's source must declare it as a turbo input, and the repo's own fence says so before a human does. The new bound test reads `apps/services/settlement/src/admin.service.ts` from `apps/services/audit`, so reverting M48 PR2's `actorType` derivation in settlement reddens a bounds test in audit — a genuinely cross-service claim, and exactly the kind whose inputs turbo cannot see by default. `packages/config/test/turbo-test-inputs.spec.ts` failed by name (`apps/services/audit has turbo.json: false`) the moment the read was added, which is the fence working as designed: the alternative is a cached green replay on the one gate whose only real input just changed. The canonical inputs list is copied BYTE-IDENTICAL rather than tailored per package, because a precise list is a second copy free to drift, and the two failure directions are not symmetric — too broad re-runs a fast test, too narrow replays a stale pass on a disarmed gate.
- 2026-09-02 — the uniform 404 is for hiding EXISTENCE, and where nothing is hidden it becomes a control wearing the face of an outage. An unlinked reporter refused at `addEvidence` gets `reporter_link_revoked` (403), not `not_found`, because that same reporter still SEES the case in their own list — `CasesRepo.listForUser` matches the frozen `reported_by` deliberately, which is docs/03 §6g's evidence argument. Answering "no such case" on the attach would tell them their case had vanished while the list in front of them showed it. A stranger never reaches the check: Cedar denies above it, so the distinct token discloses nothing a caller did not already know. The general form: apply the uniform-refusal rule where an id could confirm a row exists, and check first whether the caller can already see the row through a route you deliberately left open.
- 2026-09-02 — a journey that has never been driven cannot be cited as evidence that nothing fires on it, and the citation is the part that rots quietly. `decrypt-rate-bounds.ts` credited the stack e2e's zero-anomaly gate with keeping fail-closed-loud defaults out of legitimate traffic; for `distributions` that credit was unearned from 2026-08-21 until this PR (twelve days), because no e2e had ever reached the amount route. The gate could not have gone red: it asserts over anomalies raised in its own run's window, and the traffic that would have raised one was never generated. M48 PR3 adds the drive — a full settlement journey to a recorded distribution, read back by BOTH principal classes — and proves it can fail by reverting the operator bound to zero and watching that gate name `distributions_operator`, with the mfa burst green beside it as the positive control. The rule: when a comment credits a gate with covering a path, check that some test walks the path. A gate's reach is the set of journeys that exist, not the set its prose claims.
- 2026-09-02 — the third copy of a number is the one that turns a fence into maintenance. The exact stack-count gate lives in two workflows on purpose (images.yml runs built images, stack.yml runs `dist`, so neither number may be derived from the other), and images.yml's own comment says "if a third consumer of this count ever appears, derive it". PR3 found the third consumer: `assert-stack-counts.test.mjs` fabricated a 33/4 result to prove the workflows' `run:` line still reaches node through bash. Adding one stack test reddened that case — for the shell, which was fine. So the fixture now DERIVES from the numbers images.yml itself passes, and the two profile pairs the unit cases use are read from stack.yml. That removed an accidental tripwire (the pinned fixture had been the only thing catching one workflow's number moving without the other), so it is replaced by an explicit one: the two DEVELOPMENT pairs must AGREE, asserted by name, proven by mutating images.yml to 35 and watching only that case go red while the shell case stayed green. Un-derived is not unrelated — two independent measurements of the same suite in the same profile must match, and nothing had ever said so.
- 2026-09-02 — a `measuredPerMinute` of 1 is not a peak, and the field says so now. PR3's drive gave the two `distributions` rows their first measurement, which moved them off the 0 that the field's docstring defines as "no journey exercises this class". But the drive reveals one amount once, to prove the path is reached and recorded under the right `actor_type`; it does not sample what an executor settling an estate costs. Sizing a bound by arithmetic on that 1 — the file's own "roughly ten windows' worth" starting point — would put the executor ceiling at 50 and red the first real afternoon of work. Both notes state which of the two the number is, and the docstring now warns the next reader before the next reader divides into it.
- 2026-09-02 — a state machine must not audit fewer transitions than it permits, and the record carries the EDGE rather than the target. `setDistributionStatus` accepted three targets and emitted under `if (to === 'completed')`, so moving a distribution into work, and DISPUTING one already paid out, left nothing behind. Counted properly it is five edges audited at one: `disputed` is reachable from `approved`, `in_progress` and `completed`, and only the prior status separates an undo of a payout from a dispute over money that never moved — which is why the row now carries `from` and `to` and the fence compares SETS OF EDGES. A target-level count is satisfied by three rows all attributed to `disputed`. The same question asked of `settlement_cases` found two more: `verified → active` and `verified|active → distributing` are the only rungs with no verb of their own, and both call sites also discarded the boolean `advanceStatus` returns, so a compare-and-set that lost a race changed nothing and reported nothing. The general rule: for any table whose `status` is constrained by a CHECK, the set of transitions the code permits and the set it audits are two sets, and nothing relates them until something is written that does.
- 2026-09-02 — a value the emit depends on is read BEFORE the statement that can change it, and "read the row back afterwards" is the trap that looks correct. Emitting the prior status needs the row as it was; the obvious spellings both fail. Re-reading after the write returns the POST-update row, whose status is already `to`. Holding the `SELECT … FOR UPDATE` row and reading `.status` off it afterwards is correct against a real snapshot and wrong against any store that hands back a live reference — which the in-memory double does. Both produce a valid, hashable, permanent row saying the move went from where it ended up, on a table under `REVOKE UPDATE, DELETE`. The new fence caught it twice, once per call site, reporting a case that moved from `distributing` to `distributing`. The rule to carry: aliasing is a property of the STORE, not of the code reading it, so a fix that depends on the row being a copy is a fix that depends on the driver.
- 2026-09-02 — a closed vocabulary that a VERIFIER re-parses is append-only in the same sense the table is, so a member is never retired. The tidier design for this PR was one `settlement.distribution.status_changed` action with the target in `detail`, retiring `settlement.distribution.completed`. It was rejected on a mechanism rather than on taste: `ChainVerifier.recomputeHash` runs every stored row through the LIVE `AuditEventSchema` and returns null on a parse failure, which the caller reports as `event_hash_mismatch` — the same token real tampering produces — and `audit_events` has UPDATE and DELETE revoked, so nothing can repair the rows afterwards. Removing a member therefore converts history into permanent, indistinguishable evidence of tampering. Widening the vocabulary stays safe in the other direction only because the consumer deploys first; an older consumer rejects unknown actions as `schema_violation` before `BEGIN`, counts, advances the offset and writes no DLQ.
- 2026-09-02 — `onBehalfOf` is a fact about the ROW, not about who held the session, and the argument that nearly justified the opposite was a category error. `distributionCompleted` set `onBehalfOf: asOperator ? decedentUserId : null` while two siblings on the same trail — `distributionApproved` and `distributionAmountViewed` — named the decedent unconditionally, so the ordinary case, an executor moving their own estate's money, wrote a row that could not be found from the dead person's side, which is the query an investigator runs. A FOURTH sibling, `distributionRecorded`, was in the same state and no reading found it: a BROWSER DRIVE did, by showing one estate's real trail with three rows naming it and the row that CREATED the distribution naming nobody. Both are fixed here, so all four agree. The draft justification said the operator arm was unreachable anyway: the console cannot reach the verb, and that is true and pinned (`session-audience.spec.ts` lists `setDistributionStatus` among the routes that must never admit an operator session, and the operator edge carries no `/status` path). It does not support the conclusion. `asOperator` comes from `gate.is`, which asks whether the ACTOR is an active operator, so an operator on an ordinary account session takes that arm on a route their console cannot reach. AUDIENCE and PRINCIPAL are different questions, and a sentence that reasons from a route table to a value computed from a membership check is wrong however carefully the route table was verified.
- 2026-09-02 — an anti-vacuity floor left where it was while the run grows around it stops being a floor and becomes slack. `EVERY operator-actor event carries the decedent, across the whole administration flow` is asserted as a property so that a later emitter is covered without anyone remembering — a claim that holds only where the run REACHES the emitter, and this PR added three actions behind a verb the flow never called. Driving two of them takes the run from four operator events to seven — and only TWO of those three are the driving's doing, which is worth stating because the obvious reading is wrong: the seventh is `settlement.case.activated`, newly emitted by a `decideStage` call the run already made. The floor moves with all of it: had it stayed at four it would have been wide enough to absorb the loss of an emitter the test exists to keep honest. Two halves, and the second is the one that rots: extend the RUN when a property's subject grows, and re-measure the floor rather than leaving a number that was once tight.
- 2026-09-02 — a fence states the corpus its title claims, and keeps the machine that ALREADY passes. `status-audit-fence.spec.ts` opened as "every status a settlement state machine permits is a status it audits" while reading two tables that had been chosen by hand — a fence whose input is narrower than its claim goes green for exactly the reason it is wrong. The corpus is now derived from settlement's own migrations: every table whose `status` carries a `CHECK … IN`, which is three. The third, `settlement_access_stages`, was already total and is kept deliberately as the POSITIVE CONTROL — a fence on which every machine needed fixing cannot distinguish coverage from a matcher that fires on everything, and a fourth such table now reddens the corpus assertion by name instead of joining the two that happen to be covered.
- 2026-09-02 — the two axes of an envelope property are independent, and a test of one is not evidence about the other. `EVERY operator-actor event carries the decedent` quantifies over the ACTOR; `settlement.distribution.recorded` is written by an executor, so that property never looked at it, and it emitted `onBehalfOf: null` beside three siblings on the same resource that named the decedent. Nothing in the suite read an envelope by RESOURCE until M49 PR1 added the second property, and nothing had ever gone red: the null was the shape the emitter shipped with. What found it was DRIVING THE BROWSER and reading the resulting rows out of the audit cluster — five distribution rows about one estate, four naming it and the one that created the distribution naming nobody — which is a defect no unit test in this repo was positioned to see. The rule: for any field that is a fact about a ROW rather than about a caller, assert it over the RESOURCE as well as over the actor, and treat a real trail as something to READ rather than only to poll for the row you already expected.
- 2026-09-02 — a value assigned inside a closure and read outside it is not type-checked at the read, so an outer `let` silently disarms every argument it feeds. `recordDistribution` declared `let caseAdvancedFrom: CaseStatus | null = null` above `withTransaction` and assigned it inside the callback; TypeScript does not track assignments made in a nested function, so the flow type at the guard is `null` and INSIDE the guard it is `never` — assignable to any parameter at all. Measured with a positive control: the same wrong argument written in the sibling shape, returned out of the callback the way `decideStage` does it, raises TS2345; in the outer-`let` shape `tsc --strict` accepts passing a `CaseStatus` to a `number`. So one of the two call sites of this PR's new emitter had no type-checking on the argument the PR exists to add. The rule: return a value out of the closure that computes it. Two spellings of one thing in one file is the smell that found this — and the reason it is worth a log entry rather than a comment is that nothing in lint, `tsc` or the test suite can report it, since the symptom is an error that does NOT appear.
- 2026-09-02 — a milestone that fixes an envelope on one resource has to ask which OTHER resources the same service emits on, and the answer is not the one the review starts with. M49 PR1 fixed `settlement.distribution.recorded`'s missing `onBehalfOf` after a browser drive showed it; the category sweep then found `settlement.stage.requested` in exactly the same state — three siblings on `settlement_access_stage` naming the decedent and the REQUEST naming nobody, so "who asked for access to this estate" was the one question on the stage ladder unanswerable from the dead person's side. Same cause too: the method never took the parameter, though `administrableCaseFor` hands its caller the row that carries it. The durable form is the test rather than the fix: the property now quantifies over the RESOURCE and derives its corpus from the emitters in the source, so an emitter added later is covered whether or not anyone remembers — the first version compared a hand-driven run against a `>= 6` floor, which a later emitter satisfies without ever being exercised.
- 2026-09-04 — a check that cannot distinguish "I found nothing" from "I did not run" reports success by default, and that is the failure direction nobody investigates. `gate-image-scan.mjs` read its grype report as `(report.matches ?? []).filter(…)`; `??` coalesces an ABSENT key to zero findings, so `{}`, `{"error":"database is invalid"}`, `[]` and `{"Matches":[]}` each printed `application (blocking): 0` and exited 0 — measured, all four. The gate now REFUSES anything that does not prove a scan happened, anchored on what grype's own report carries (`matches` is an array, `descriptor.db.status.valid` is true, `built` is inside a bounded window) rather than on a shape we hope for, and every unrecognised shape refuses rather than passes so a future schema rename fails closed. The half that was already fatal — a 0-byte report — was the survivable half, because a crash is loud.
- 2026-09-04 — two outcomes that need different remedies need different EXIT CODES, not just different messages, once a caller has to decide what to repeat. The scan gate exits 1 for a blocking finding, 2 for a refusal, 3 for misuse, and images.yml retries 2 and only 2. Sharing one code would make the retry loop either rescan for real findings three times or never retry at all — the same "cannot tell the two apart" defect moved one layer out, which is why the codes have a test asserting they are pairwise distinct rather than a comment saying so.
- 2026-09-04 — the probe and the verdict must not share an answer. Two probes in images.yml's web smoke test collapsed a failed measurement into the finding they exist to report: `|| true` on an unpipefailed pipeline collapsed a failed curl, a failed grep and a genuinely absent header into one empty string that printed "the image served no Content-Security-Policy header at all". The per-asset probe had the opposite defect, and the first fix missed it: `code=$(curl …)` under `bash -e` takes the substitution's status, so a transient failure killed the step at exit 7 with NO annotation at all, and a 000 retry added above that same unprotected assignment was dead code. Measured in this PR's own review. Both probes now separate the measurement from the verdict, and the curl runs inside `|| true` so its own 000 can reach the variable. An outage must not wear the face of the control, and a probe that cannot report its own failure is the same defect with no face at all.
- 2026-09-04 — a bound whose value depends on a third party's publishing cadence is stated as a judgement with its reasoning beside it, not derived and not omitted. The scan gate refuses a vulnerability database older than seven days, because `valid: true` on a fortnight-old database is true and meaningless. Seven rather than one because the runner fetches a fresh database every job, so the check never fires in the normal case, and because the remedy for a genuinely unreachable database is the caller's retry loop rather than a permanently red pipeline — the argument this file's own header makes against blocking on base-image CVEs.
- 2026-09-04 — M50 opened, "the supply chain the pipeline does not pin", because the category M49 PR2 swept is not M49's subject. Six of §6lll's nine residuals are supply-chain properties of CI itself: twelve distinct third-party actions referenced and none digest-pinned while a throwaway test database IS, both scanners exact-tag but not digest-pinned, `node:22` floating inside the reproducibility job, eight of thirteen jobs without `timeout-minutes`, five unretried `pnpm install` call sites, and `if-no-files-found: warn` on both image analysis artifacts. Recorded under a new owner rather than folded into M49, whose row is about what a state machine says.
- 2026-09-04 — a prose pass that checks the commit message and not the other files carrying the same sentence has done half the job. M49 PR1's message enumerated the residuals it recorded, the pass caught that the list named six of seven, and the identical omission in the docs/04 M49 row went out unfixed because nothing re-derived it against §6kkk. The check that would have caught it is the one already written down for counts: state the corpus and compare SETS. Fixed in M49 PR2.
- 2026-09-08 — the prior status is read AT THE WRITE, not at the top of the caller; whether the read is inside the statement or on the line above it is a preference, and the difference between those two claims was measured. M49 PR1 established "capture the pre-image before the write" for `advanceStatus`, which is sound where nothing changes the status in between. `SettlementService.confirmVerification` is where it stops being sound: `markVerified` moves the case `waiting_period → verified`, and if identity's watermarked interlock then refuses the account lock, the verification unwinds through `markResolved` INSIDE THE SAME OPEN TRANSACTION. The status the service read at the top is `waiting_period` in both arms — the method's own guard admits nothing else — so a pre-read there writes `waiting_period` for a case the database moved out of `verified`, on a table under `REVOKE UPDATE, DELETE`, in the arm where a living owner came one commit from being locked into the settlement of their own estate. **What is NOT true, and an earlier draft of this entry said it was**: that only a statement-internal read is correct, and that the false edge would be undetectable. A `SELECT` on the line above the UPDATE returns the same value — proven by reimplementing the method that way and watching every test stay green, a survivor meaning the edit is not load-bearing — because under READ COMMITTED the CTE is exactly as stale as a pre-read and both rest on the caller's row lock. Demonstrated on 16.15 with two connections: without the lock, a statement that blocks on a concurrent writer proceeds under EvalPlanQual against the new row version and still returns the OLD snapshot's status. And the false edge is recoverable afterwards, badly, by joining the per-statement version rows — the "one event plus an inference" construction this repo rejects, on an unchained table. The rule that survives all of that: read the value at the write, and prefer the spelling that cannot drift back up the method.
- 2026-09-08 — which transitions must record an edge is DERIVED from the from-predicate the runtime compares against, never listed, and the scan that derives it must be attacked before it is believed. A hand-written list of "the events that carry `from`" is true the day it is written and cannot notice the next widened predicate; `apps/services/settlement/test/status-audit-fence.spec.ts` classifies every statement in the service that writes a `status` column by the predicate it compares against, and the counts live there rather than here. Its POSITIVE CONTROL is derived too — `StagesRepo.decide` is exempt because the scan read its predicate, not because the fence says so. **THE FIRST THREE VERSIONS OF THAT SCAN WERE ALL WRONG, AND ITS OWN REVIEW BROKE EACH ONE LIVE**, which is the part worth carrying: it matched only `*.repo.ts` in a directory that already holds a `dek.repository.ts` issuing an UPDATE; it required `RETURNING`, `status` first in the SET clause and uppercase keywords, so a plausible cascade with two prior statuses was simply absent and the count never moved; and it classified by probing for the first `status = '<literal>'` it could find, so `(status = 'a' OR status = 'b')` passed as single-valued, as did an `= ANY($n)` predicate under a `--` comment naming one status. Every one of those went green. A scan that reports a corpus is making two claims — what it classified, and that it SAW everything — and only the first is usually tested.
- 2026-09-08 — when a guard exists at two layers, the mutation that proves it must say WHICH layer, and a battery where every mutation reddens everything has proven less than it looks. Defeating the CTE (`RETURNING prior.status` → `RETURNING settlement_cases.status`) reddens M49 PR3's integration spec and nothing else, because the unit fence drives in-memory doubles that never execute SQL; its mirror image — reporting the caller's earlier read at the liveness site — reddens the unit fence and nothing else. That pair is the evidence for the split, and it is why the integration spec drives the one case where the status a top-of-method read would report, the in-transaction status and the post-update status all differ. The same question asked of `revokeStage`'s dual control gives the opposite answer and is recorded as a residual: deleting the service pre-check alone is green, deleting the double's DDL restatement alone is green, and only deleting both reddens a single test — so two layers are covered by one assertion that isolates neither.
- 2026-09-08 — when a status write carries NO from-predicate, the derivation of which events owe a prior status moves from the SQL to the GUARD CHAIN, and keeps failing closed. M49 PR3 derived settlement's obligation from the `WHERE` clause, which is sound there because every terminal statement restates its from-set in SQL. Every status write in the vault's emergency-access ladder is `WHERE id = $1`: there is nothing to read, and PR3's scan applied unchanged would have demanded a `from` on all five statements — fail-closed and WRONG, because `markRequested` genuinely admits one prior (`blockReason` refuses the other three live statuses). So M49 PR4's fence reads the TypeScript instead, in the three shapes the tree actually contains — an allow-list guarding a throw, a refusal list in the method or in one helper it calls, and no comparison at all — and treats a fourth shape as unreadable, which owes a `from`. The exemption stays DERIVED rather than listed: `request` is exempt because the scan read `blockReason`, the same construction that made `StagesRepo.decide` settlement's positive control. The bound is stated and recorded as a residual: one level of helper inlining, three shapes.
- 2026-09-08 — capture the prior status WITHOUT adding a compare-and-set, where the permissive from-set is itself a control. The obvious companion to "record which arm you took" is "refuse the arms you do not want", and here it would have inverted a rule the same change cites. `deny` and `revoke` admit all four live statuses deliberately: M27 PR3a made deny-on-a-released-policy legal precisely so that the protective action is not harder than the permissive one, release being CallerGuard-only while revoke is step-up gated. A predicate admitting all four is a no-op; one that narrowed would re-invert M27's rule. So the prior is captured and the guard stays where its reasons are written — and the fence asserts that the statements have no from-predicate, so that if one ever gains one, the derivation moves back to the SQL where PR3 put it.
- 2026-09-08 — a `to` key is owed only where one statement serves many targets. Settlement needed one because `setDistributionStatus` writes `status = $3` and the action is chosen by a data map. The vault is the inverse — five statements, five LITERAL targets, one emitter each — so a `to` would be a second copy of a fact the action id already determines, and a second copy is a copy that drifts. The verb-to-target map is asserted by the fence instead, which is what keeps the two action ids whose verb disagrees with the status their write produces (`vault.emergency.requested` writes `waiting`, `rearmed` writes `configured`) visible as data rather than frozen into the wire. Adding `to` would have recorded the mismatch instead of surfacing it.
- 2026-09-08 — a probe with alternatives owes a reachability control PER ALTERNATIVE, and the aggregate control cannot supply it. `threat-model-residuals.spec.ts` already asserted that at least one `PRECONDITIONS` entry reads FILLED and at least one reads ABSENT, which is the standard guard against a scanner that can only ever answer one way. It is satisfied by OTHER entries being healthy, so it saw nothing when M49 PR4's own new entry watched three shapes of which TWO, both spanning newlines, could never match under the line-scoped matcher — a probe reporting clean because it could not look, one level below where the fence was reading. Inspection caught it; inspection does not run in CI. So each entry's alternatives are now derived from its own regex source and checked against the mode it declares. Two consequences worth keeping: the check is MEASURED, not argued — its first draft read the pattern as text and flagged a healthy probe, because `[^'"`\n]` names `\n` in order to EXCLUDE it and reads almost exactly like `[\s\S]`, which admits it, so it compiles each character class and tests it against a newline instead; and the mutation that states its worth is not that removing `wholeFile` reddens something, but that it reddens the new assertion while leaving the old one GREEN.
- 2026-09-08 — a transition with NO event is a different defect from an event that cannot say which transition it recorded, and the second half of M49's subject is the first kind. PR1, PR3 and PR4 all fixed fused edges; the erasure ladder's claim, release and completion moved the row and emitted nothing. It read as covered because the destructive steps between those rungs are audited by CONSEQUENCE — `auth.user.status_changed`, `auth.sessions.revoked_all`, `crypto.dek.destroyed` — and that argument is sound for a FIRST run and false for a resume, where the account is already closed and the DEK already destroyed so every one of the three is skipped. The rule to carry: "audited by consequence" is a claim about a path, and it has to be checked on every path that reaches the same state, not on the one the author had in mind. The resumed leg is now driven end to end and emits exactly two events.
- 2026-09-08 — M49 PR3's SQL derivation transfers to identity where it did not transfer to the vault, and the difference is worth stating once so the next service is not guessed at. The vault writes `WHERE id = $1` everywhere and narrows its priors in TypeScript, so PR4 had to read the guard chain. `erasure_requests` restates its from-set in SQL on every statement, so the fence reads the `WHERE` as settlement's does — on three of the four; the claim's from-set moved into its CTE along with the lock, and §6ooo records that PR5's fence does not read it there, a bound M49 PR6's fence closes for its own service by reading every `WHERE` a statement contains rather than only the one after `SET`, the `UPDATE`'s own, which is where PR5's looks (an earlier form of this sentence said the fence reads it "exactly as settlement's does", flat and unqualified, written before that residual was found; the words "on every statement" in the same sentence were never the wrong ones — they describe the SQL restating its from-set, which it does). The test for which mechanism applies is not the service, it is whether the statement's own predicate carries the from-set — and the answer is worth measuring rather than assuming, because applying the wrong one is fail-closed in a way that looks like a finding: PR3's scan run against the vault would have demanded a `from` on all five statements — right about four of them and wrong about the fifth, and it is the fifth that matters, since `markRequested` genuinely admits one prior and a fence that cannot ever say "owes nothing" has stopped discriminating. §6nnn states it that way; an earlier draft of this entry over-corrected it into "wrong about all five".
- 2026-09-08 — a fence that matches the first `UPDATE` in a block is defeated by `FOR UPDATE`, and the FIRST diagnosis of that defect was wrong in a way worth recording. M49 PR5's fence dropped the one statement its PR was about: `FOR UPDATE OF r SKIP LOCKED` precedes the statement's real `UPDATE`, so the scan read the table name as `OF`, found no `SET`, and moved on. The first theory was different — that prose about SQL, written in backticks, had desynchronised the template-literal pairing — and a comment stripper was written for it. That theory was false: deleting the stripper leaves every assertion green, because the service's code-spans are balanced. The lesson is the one this repo keeps relearning from the other side: a fix that makes a red test green is not evidence that the fix addressed the cause, because the second change (the `FOR UPDATE` repair) landed in the same edit. Prove which change is load-bearing by REMOVING each alone. What NAMED the missing statement was comparing the SET found against the set expected — though a count would have caught this one too, and the distinction from M49 PR4 is the part worth keeping: PR4's defect PRESERVED a count while mis-attributing a statement, so only a set could see it, whereas this defect DROPS a statement and reddens the length assertion as well. An earlier draft of this entry carried PR4's sentence over unchanged; the fence's own header states the difference correctly. Settlement's equivalent scan was checked for the same two defects and has neither — both scans return the same fourteen heads there — so this is recorded as a property of the scan shape, not a repo-wide bug.
- 2026-09-08 — a test named for a branch it never reaches is worse than no test, because it occupies the slot the real one would fill. M49 PR5 added an audit event to the erasure driver's release path, and the drive named `RELEASES the claim, destroying nothing, when the account moved in the window` did not reach a release: it moved the account BEFORE the sweep, so the claim's own `u.status = ANY($3)` refused the row and the leg never started. `runDueErasures` answering 0 was that refusal rather than a claim handed back, and the request staying on 'pending' was trivially true of a request nothing had touched. Measured rather than argued: making `releaseClaim` throw unconditionally left EVERY test in the package green — 561 of them, this PR as it stood before these three drives existed (553 at the base commit, 564 now), a corpus worth naming because re-running the stated mutation on the shipped tree reddens exactly the two drives that closed it, so no test executed the statement at all — not the losing arm, not the winning one. The reachable setup is the RESUME arm, which deliberately carries no status allowlist, so an owner who becomes ineligible AFTER a claim is re-claimed and then stopped by `closeAndUnlinkEmail` restating that allowlist inside its own `WHERE` — the pre-read and the write disagreeing is the boundary the release decides. The drive is renamed for the refusal it does prove. The rule: when a change adds an emit to a branch, check that some test EXECUTES the branch before believing the emit is covered, and an unconditional throw in the method is the cheapest way to ask.
- 2026-09-08 — a compare-and-set and the `if` that reads it are two layers, and each owes its own test. PR5's release boolean is proved in three places deliberately: the STATEMENT answers false on a row that already moved, against Postgres, because a double returning a boolean only restates the belief under test; the GUARD is proved in the unit spec with that double forced to false, which is the layer no SQL can reach; and the end-to-end drive proves the path is reachable at all, which is the thing that was actually missing. Splitting it that way is what makes the mutations diagnostic — ignoring the CAS reddens the integration drive alone, removing the `if` reddens the unit one alone — where a single end-to-end test would have gone red for both and said which for neither. This is M49 PR3's "one mutation says which layer" applied to a boolean instead of a statement.
- 2026-09-09 — When M49 PR3's SQL derivation and PR4's guard-chain derivation AGREE that every status write owes a `from`, the fence must say what it cannot prove: that its exemption set is empty, and that its "owes nothing" answer rests on synthetic controls alone. Rationale: M49 PR5 wrote the warning that a fence which can only answer "owes" has stopped discriminating; PR4 built the guard-chain reading this fence borrows, and an earlier draft credited PR4 with both. On a corpus with no single-prior statement there is nothing to derive an exemption FROM, so the controls are synthetic by necessity — and the honest fence labels each one, states the weaker result in its header, and records the bound as an M45 residual (docs/03 §6ppp). What the plaid fence delivered beyond that is the argument for synthetic controls even where corpus controls exist: one of them found a defect in the fence itself on its first run — the clause cutter stopped at the first `)` and truncated an `IN (...)` list — which no corpus statement in that service could have exposed, because none has a predicate.
- 2026-09-09 — A recovery that is one of a write's several priors is recorded as `from` on the event that records the write, not as a new member guarded by a comparison. Rationale: docs/03 §6kkk rejected "the first `synced` after a `login_required`" as one event plus an inference; a temporal join across two events is an inference, and a field on the event that recorded the write is not. A `plaid.item.recovered` member would need `if (prior !== 'healthy')`, which gives one write two spellings: a reader counting status changes needs both members and the knowledge that they are exclusive. NOT because it resembles the consequence guard M49 PR5 found wrong — that shape was an emit guarded by something OTHER than the fact it records, and a guard on the prior is a guard on exactly what it records; the resemblance was asserted in a first draft of this entry and is withdrawn here rather than in a later correction. The cost is recorded in the fence as data rather than argued away: `synced` names the ACT and writes `healthy`, the same verb/target disagreement PR4 pinned for `requested` → `waiting`.
- 2026-09-09 — A transaction that writes a parent row and its child rows locks the PARENT first, and every writer of that pair takes the same order. Rationale: M49 PR6's review drove plaid's `syncItem` (accounts first, item last) against `revoke()` (item first, accounts last) on two connections and Postgres killed the revoke with 40P01 — a step-up-gated protective action answering `500 internal_error` after Plaid had already removed the token, the item left `healthy` with a dead credential. Putting the item's status write first in the sync's transaction removes the cycle, and because that write is a compare-and-set on liveness it also makes a sync that meets a committed revoke stop before touching an account row — closing, in the same reorder, the resurrection a first draft of §6ppp had recorded as a residual (`accounts.upsert` sets `deleted_at = NULL` with no liveness predicate). The order is proven by a two-connection drive that ends with both accounts live again under the old order; a lock-order rule that lives only in prose is the kind of rule this repo has watched drift.
- 2026-09-09 — A coverage floor is calibrated on the configuration its own header names as the corpus, and the file states BOTH measurements. Rationale: plaid's `jest.config.js` has said since it was written that its floor sits "near the LOCAL number" because the integration suite runs only where `PG_TEST_URL` is set; M49 PR6 ratcheted it against the with-Postgres figure without reading that sentence, and the package went red on every developer's machine — measured by the PR's own review, not noticed by its author, because the author's gate runs always had a database. Two conventions exist in the tree (identity and ai-assistant calibrate on the no-database run so the floor is a lower bound in both configurations; notifications calibrates with Postgres and says so), and either is defensible; what is not is a file asserting one and implementing the other. A count sentence in prose beside the floor said the old floor "had sat between fourteen and twenty-nine points under the no-database measurement", which is neither corpus's range: the no-database gap is eleven to fifteen points and the with-Postgres gap is twenty-one to fifty-two, and the quoted range took the top of the first beside a figure from the second. The same defect class as [[every-count-needs-its-corpus]], one level in — a range can name a corpus and belong to no corpus at all.
- 2026-09-10 — A reader that ATTRIBUTES instances (a statement to a method, a route to a controller, an emit to an event) is paired with a RAW count of the same thing, and the two are asserted together. Rationale: M49 PR6's fence read `UPDATE plaid_items` statements by walking each method's template literals, and its review hid a new writer from it twice over — once in a method whose first literal was not SQL, once in a method without `async` — while all fourteen tests stayed green, because a writer the reader cannot see is indistinguishable from a writer that does not exist. A keyword count of the raw source has no such blind spot and is worthless as a fence on its own; together they make an unattributable instance a FAILURE rather than an absence. This is the anti-vacuity floor one level down: the floor on the total says the scan found things, the floor per LEVEL says it found them everywhere, and this says it can account for every one it found. It applies to a reader of REACH as much as to a reader of statements, which the same PR learned twice: its fence proved "no other file in `src` calls a ladder writer" by resolving the repository's property name once, out of the service file, and hunting `this.<that name>.<writer>(` everywhere else — so a second `@Injectable` binding the same repository under any other name walked through it. The name is resolved per file from the constructor parameter's TYPE now, with the raw floor beside it: the writer's name called on any receiver at all.
- 2026-09-10 — A vocabulary derived from the schema is read from the whole migration SET, never from the `CREATE TABLE` alone. Rationale: migrations here are append-only and checksummed, so a widened enum arrives as `ALTER TABLE … DROP CONSTRAINT … ADD CONSTRAINT … CHECK (…)` in a new file; the `CREATE TABLE` body is the one statement in the set that can never change, which makes it the worst possible source for a value that grows. M49 PR6's review added a fifth `plaid_items.status` with a new migration and the ladder fence — whose entire subject is that vocabulary — stayed 14/14 green. The reader now takes every CHECK that names the column, in migration order, and the last one wins, as it does in the database.
