...
- 2026-09-08 — M49 PR3's SQL derivation transfers to identity where it did not transfer to the vault, and the difference is worth stating once so the next service is not guessed at. The vault writes `WHERE id = $1` everywhere and narrows its priors in TypeScript, so PR4 had to read the guard chain. `erasure_requests` restates its from-set in SQL on every statement, so the fence reads the `WHERE` exactly as settlement's does. The test for which mechanism applies is not the service, it is whether the statement's own predicate carries the from-set — and the answer is worth measuring rather than assuming, because applying the wrong one is fail-closed in a way that looks like a finding: PR3's scan run against the vault would have demanded a `from` on all five statements — right about four of them and wrong about the fifth, and it is the fifth that matters, since `markRequested` genuinely admits one prior and a fence that cannot ever say "owes nothing" has stopped discriminating. §6nnn states it that way; an earlier draft of this entry over-corrected it into "wrong about all five".
