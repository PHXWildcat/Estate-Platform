...
- **No hard deletes.** `deleted_at TIMESTAMPTZ` soft delete; partial unique indexes exclude deleted rows; crypto-shredding performs legal erasure (destroy the row's DEK — see below). **This line said "a privileged retention job (not the app role)" for twenty-four milestones and there has never been such a role** — the repo has no `CREATE ROLE` or `GRANT` outside test files. M25 ships owner-initiated erasure under the app role deliberately (docs/06, 2026-08-21); what stands in for the privilege boundary is a declared caller allowlist in `packages/contracts/test/erasure-domains.spec.ts`, which is weaker and covers a different threat. The gap is docs/03 §6kk.
...
- **Field-level encryption:** sensitive columns are `BYTEA` holding AEAD ciphertext; each row carries `dek_id` referencing a wrapped per-user data key. Equality search uses blind-index columns (`*_bidx BYTEA`, HMAC of normalized plaintext). "Right to delete" = destroy DEK ⇒ ciphertext is irrecoverable, audit structure preserved. **THE SHRED DOES NOT REACH A BLIND INDEX**, and that is the one place this sentence over-promises: a `*_bidx` is an HMAC under a service-wide key, not a `*_ct` + `dek_id` pair, so it survives the destruction of every DEK and stays a deterministic identifier for the value it indexes. It survives in the version tables too, which are INSERT-only. See docs/03 §6kk. **M25 PR1 closes the capture half for the whole category, not one column:** three tables carry both a blind index and a shadow that captures it — `users`, `contacts` and `plaid_items`, in three services — and each capture function now drops every key ending in `_bidx`, derived from the row rather than named. PR0 recorded this as `users.email_bidx` alone; the category was found by deriving it (`packages/contracts/test/version-capture-redaction.spec.ts`), and the member PR0 missed is the more sensitive one, since `contacts.email_bidx` indexes a living third party rather than the account holder. **M25 PR3 closes the live half for `users.email_bidx`:** the erasure UPDATE replaces it with a real blind index of `<uuid>@erased.invalid` — built through `emailBlindIndex`, so width, key and purpose label cannot drift, and under a reserved TLD, so it can never collide with a live address. Random bytes would make every erased row identifiable by its column alone. Note the two halves are one mechanism: had PR1 not landed first, this UPDATE would have copied the OLD index into `users_versions`, where `REVOKE UPDATE, DELETE` puts it beyond any later migration's reach. `contacts.email_bidx` and `plaid_items` keep their LIVE values — those domains are not reached by M25 (docs/03 §6nn), and neither is `document_search_tokens.token_bidx`, which is the sharpest of the four.
