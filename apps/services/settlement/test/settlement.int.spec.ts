...
describeIfPg('settlement service against Postgres (core-cluster co-tenant)', () => {
  ...
  it('migrations are idempotent and the shared schema_migrations holds both owners', async () => {
    ...
    expect(rows.rows.map((r) => r.name)).toEqual([
      ...
      // Profile's M13 link-invitation table. Listed here because the two
      // co-tenants SHARE `schema_migrations` (the 2026-07-22 Plaid-precedent
      // decision): this assertion is what proves the sharing still works, so a
      // co-owner's migration landing is exactly what it should notice.
