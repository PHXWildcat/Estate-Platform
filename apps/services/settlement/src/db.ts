...
/**
 * Thin provider around a pg Pool, plus the transaction chokepoint settlement
 * requires (a case transition must lock the row, mutate it, and perform the
 * cross-service identity-lock call before COMMIT so a failed lock rolls the
 * transition back). Tests can point the pool at a scratch schema via
...
/** Postgres unique-violation detector (the one-open-case index reports here). */
