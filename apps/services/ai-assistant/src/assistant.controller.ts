 ...
 *
 * CallerGuard has already verified this token against identity; what it does
 * not do is hand it back, so the header is read again here. Forwarding it is
 * how the assistant reads assets, documents and profile WITHOUT holding a
 * service credential of its own (src/di-tokens.ts explains why that is the
 * design): it can reach exactly what this user could reach for themselves.
