...
import { z } from 'zod';
import { defaultFetch, readJson, type FetchLike } from './http';
 ...
 * "this read did not happen", never "the user has no profile". The rationale
...
export class ProfileClient {
  ...
  facts(bearer: string): Promise<ProfileFactsView | null> {
    return readJson(this.fetchImpl, `${this.baseUrl}/v1/profile`, bearer, ProfileFactsViewSchema);
  }
