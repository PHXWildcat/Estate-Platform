...
/**
 * Thin provider around a pg Pool, plus the transaction chokepoint the
 * generation pipeline requires (documents row + version row must commit
 * atomically). Tests can point the pool at a scratch schema via
...
/** Postgres unique-violation detector (template uniqueness, DEK uniqueness). */
