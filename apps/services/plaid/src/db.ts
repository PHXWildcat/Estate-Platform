...
/**
 * Thin provider around a pg Pool, plus the transaction chokepoint sync and
 * revocation need (item + accounts must change atomically). Tests can point
 * the pool at a scratch schema via PoolConfig.options (`-c search_path=...`).
 */
@Injectable()
...
/** Postgres unique-violation detector (item_bidx uniqueness, DEK uniqueness). */
