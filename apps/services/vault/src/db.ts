 ...
 * repositories can serve both without knowing which they run under. Every query
 * goes through one of the two chokepoints below - the rule "parameterized
 * queries only" has exactly two enforcement points.
 */
...
/**
 * Thin provider around a pg Pool, plus the transaction chokepoint the vault
 * requires (a keyset replacement must swap the keyset and revoke sibling
 * sessions atomically; a reset must soft-delete every item and replace the
 * keyset in one commit). Tests can point the pool at a scratch schema via
 * PoolConfig.options (`-c search_path=...`).
 */
@Injectable()
...
/** Postgres unique-violation detector (client-supplied item ids collide here). */
