...
import { Db } from './db';
import { SECOND_FACTOR_FAILURES, SECOND_FACTOR_SUCCESSES } from './stepup';
...
export class AuthEventsRepo {
  ...
  /**
   * FAILED SECOND-FACTOR ATTEMPTS since the last successful one, in a window —
   ...
   *
   * IT COUNTS BOTH ROUTES THAT CHECK THE SAME SECRET, and it did not until the
   ...
   * can be made, so the failure kinds and the success kinds are both sets now,
   * and adding a third checker of this secret means adding it here.
   *
  ...
  async failedFactorAttempts(
    ...
    const rows = await this.db.query<{ denials: string }>(
      ...
      [userId, since, [...SECOND_FACTOR_FAILURES], [...SECOND_FACTOR_SUCCESSES], sessionId],
    );
