...
/**
 * Trust boundary (M2, mirrored from the profile service). This service does
 * NOT yet call the identity service to verify a session; instead it trusts
 * the `x-estate-user-id` header, which the BFF / API gateway injects AFTER
 * verifying the caller's session token and STRIPS from any inbound client
 * request. It is therefore gateway-injected, never user-supplied. Real
 * cross-service session verification is a later integration — see README.
 *
