 ...
 * insider-threat chokepoint per docs/03 §5.3) under the financial cluster's
 * own KEK alias; dev/test uses the in-process LocalKmsProvider. config.ts
 * already fails fast if the required settings for the active mode are missing.
 */
function kmsProviderFor(config: AssetsConfig): KmsKeyProvider {
...
@Module({
  controllers: [AssetsController, BeneficiariesController],
  providers: [
    { provide: CONFIG, useFactory: (): AssetsConfig => loadConfig() },
    { provide: CLOCK, useValue: (): Date => new Date() },
    {
      provide: PG_POOL_CONFIG,
      inject: [CONFIG],
      useFactory: (config: AssetsConfig): PoolConfig => ({
    ...
    EventsService,
    {
      ...
      useFactory: (config: AssetsConfig, deks: DekRepository, events: EventsService): FieldCrypto =>
        new FieldCrypto(
