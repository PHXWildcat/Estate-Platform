...
type LoadState =
  ...
  | { kind: 'notFound' }
  | { kind: 'error' }
  | { kind: 'ready'; document: DocumentInfo; versions: DocumentVersionInfo[] };
...
export function DocumentDetailPanel({ documentId }: DocumentDetailPanelProps): ReactElement {
  ...
  const load = useCallback(async (): Promise<void> => {
    ...
    if (document.ok && versions.ok) {
      // A response missing its fields is NOT data (the M11 rule): a version
      // skew must read as "we couldn't load this", never as a document with no
      // history.
      if (
